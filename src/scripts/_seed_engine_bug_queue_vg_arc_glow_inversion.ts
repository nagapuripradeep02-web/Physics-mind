...
const ROWS: Row[] = [
  {
    ...
    title: 'TWO vg pools never stamp and are not brightenOnly, so arcs and right-angle marks are force-dimmed to 40% whenever ANY glow is active — named or not',
    severity: 'MAJOR',
    owner_cluster: 'peter_parker:field3d_surgeon',
    root_cause: 'In the vg lines/planes writer, applyGlowEmphasis matches a glow target against userData.vgId (the vg_lp_* branch of the glow pass). vgId is written by exactly one function, stamp(), and it is called for lines (which also covers the projection shadow and the renderer-injected <lineId>_lambda marker), planes and their .normal, points, segments and vectors. The ANGLE ARC block never calls it — resolve by symbol: the arc pool is built as elementType "vg_lp_arc" and written in the same frame function as the others, with no stamp() line. So an authored glow naming an arc id can never match, and the arc is treated as a non-focal PEER. It is then also absent from the brightenOnly list in the glow branch (which names vg_lp_line, vg_lp_seg, vg_lp_normal, vg_lp_vec, vg_lp_dir), so it takes the touchOp path and is written to GLOW_DIM_OPACITY = 0.4. The failure is therefore not "the glow does nothing" — it is that the ONE object the sentence is about is the ONE object that dims, while its peers hold at full brightness and nothing brightens at all. Measured on lines_and_planes_in_space at CP-B cycle 0: five bindings named arcs (s6_1/s6_4 -> arc1, s7_2 -> arc_normal, s7_3/s7_4 -> arc_plane), on the two states whose entire subject IS an angle arc. Rules 29 and 32e run backwards on 2 of 4 sentences in STATE_6 and 3 of 4 in STATE_7. CORRECTED 2026-08-21 (quality_auditor axis-5 pass), and the correction changes what a surgeon must do: it is TWO pools, not one. stamp() is called at exactly five sites — lines, planes+normals, points, segments, vectors. NEITHER vg_lp_arc NOR vg_lp_right_angle appears, and neither is in brightenOnly. And the severity was understated as a focal-side failure. Removing the five arc bindings did NOT help the arcs: they went from named-and-inverted to unnamed-and-still-hard-dimmed. Measured share of on-screen life spent at 0.4, rate-invariant: STATE_4 arc_par 100%, STATE_7 arc_normal 100%, STATE_7 arc_plane 100%, STATE_3 perp:right_angle 66% at rate 0.7 rising to 100% at 1.1 — the right-angle mark dimmed while perp, its own parent, is focal at 1.0, so ONE object renders at TWO brightnesses on the sentence "That perpendicular is the distance." STATE_7 is the worst: its two arcs display the only two numbers the state exists to teach (55 deg and 35 deg), dimmed on the very sentences that state them while the normal and shadow beside them sit at 1.0. This row therefore describes an ACTIVE CORRUPTION of authored appearance shipping today, not a missing capability. Two aggravating facts: the concept declares focal_primitive_id "arc1"/"arc_normal", i.e. the authored focal is an object the mechanism cannot address (that field is schema-only and consumed by no renderer, so it is inert rather than a second defect, but it is the same tell); and the renderer own comment above stamp() states the invariant the arc pool breaks — "Every pool member carries the RESOLVED object own id ... so glow_focal can name the common perpendicular rather than pool slot 2".',
    prevention_rule: 'EVERY POOL THAT CAN BE NAMED BY AN AUTHORED ID CALLS stamp() — a pool that skips it is not merely unaddressable, it is a trap, because the glow pass treats unmatched members as peers to DIM. Engine fix (founder call, Rule 40 platform file, lands on master through its own PR): TWO stamps and TWO brightenOnly entries, not one of each — add stamp() to the arc block AND to the right-angle block mirroring the other five pools, and add BOTH vg_lp_arc and vg_lp_right_angle to the brightenOnly predicate. A surgeon who fixes "the one pool" this row originally named ships it half-broken. That makes an angle arc glowable fleet-wide, which is the right end state — an arc is the natural focal of any angle state. AUTHORING RULE until it lands: a glow target must be a line, a plane (or <plane>.normal), a point, a segment or a vector; never an arc. GENERAL LESSON: when a mechanism resolves by id, enumerate the id-PRODUCING sites, not the id-CONSUMING ones — a verification that only checks "does this id exist in the authored JSON" passes every one of these five bindings.',
    probe_type: 'js_eval',
    probe_logic: 'AXIS 5 — the probe object is the STATE FULL ELEMENT INVENTORY OVER TIME, never the binding list. A binding-indexed sweep is structurally incapable of finding this: the type census of any binding set enumerates the STAMPABLE universe (line, seg, normal, vec, point) and therefore cannot contain the two pools missing from it. Assert instead: for every state and every ms at which any glow is active, no element that the author did not intend to dim is written below its own authored base opacity — i.e. enumerate what is ON SCREEN and check what the glow channel does to each, including elements no sentence ever names. Negative controls: STATE_7 arc_normal/arc_plane must FAIL at 100% of their life pre-fix; STATE_3 perp:right_angle must FAIL while its parent perp is focal. Then, additionally, the original binding-side check: static, and cheap enough to run on every concept: build the stampable id set per state from the renderer stamp() call sites (lines + projection id + <lineId>_lambda where show_lambda_marker, planes and <plane>.normal, points, segments, vectors, perpendicular, common_perpendicular, intersections) and assert every tts_sentences[].glow and every focal_primitive_id is in it. NOTE the injected lambda marker: the renderer pushes {id: line.id + "_lambda"} into the points array BEFORE the point loop stamps, so a checker that models only AUTHORED vg.points produces a false positive on it — that false positive was observed in this very session. Negative controls: the five pre-fix bindings (arc1 x2, arc_normal, arc_plane x2) must FAIL; L1_lambda must PASS. Runtime form: drive SET_GLOW naming an arc under a frozen clock and assert the arc material opacity does not fall below its peers.',
    status: 'OPEN', concepts_affected: ['lines_and_planes_in_space'], fixed_in_files: [],
    row_type: 'incident', fixed_at: null,
    marker: 'TWO pools, not one: vg_lp_arc and vg_lp_right_angle',
  },
  {
