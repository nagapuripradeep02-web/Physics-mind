/**
 * TEMP seed — electrical_power_in_resistor into simulation_cache so
 * `npm run visual:eyes` / `smoke:visual-validator` can drive it. Seventh 2D
 * (particle_field) diamond (Ch.3 #7): generation does not run through the
 ...
 * _seed_internal_resistance_cache.ts / _seed_combination_of_resistors_cache.ts.
 * Delete after the visual gate.
 *
 * Run: npx tsx --env-file=.env.local src/scripts/_seed_electrical_power_in_resistor_cache.ts
 */
...
const CONCEPT_ID = 'electrical_power_in_resistor';
