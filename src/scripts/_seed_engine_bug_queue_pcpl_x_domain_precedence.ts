...
const rows: Row[] = [
    {
        ...
        probe_type: 'js_eval',
        probe_logic:
            "check_cartesian_plane.ts section 'E-2' — NEGATIVE CONTROL FIRST: a reimplemented pre-fix numeric-wins algorithm is shown to resolve domainMin to the stale numeric (1.75) at the exact STATE_4-style input (min:1.75, min_expr:\"b - 2*(b/round(pow(10,nlog)))\", nlog=3.3625/n=2304 — doc-grounded via docs/MATHEMATICS_PHASE0_CARTESIAN_PLANE.md and the concept's own skeleton doc, never the concept JSON itself), and that resolution is fed through the SHIPPED, unmodified PM_functionPlotSample to show only 2 of 240 samples land inside the plane's own re-zoomed visible window (<1% coverage — the measured 'does not draw' symptom). The SHIPPED PM_planeResolveBound-based resolution is then shown to resolve to the live expression value instead (1.998263...) and land all 240 samples inside the window. STATIC REACHABILITY asserts drawFunctionPlot's source calls PM_planeResolveBound for both min and max, no longer contains the pre-fix numeric-first ternary, and still falls back to ranges.xRange when nothing resolves (degenerate-domain discipline preserved). The dual-bound WARNING is tested via a dedicated sandbox (state reassignment baked as literal statements in one generated function body, since PM_currentState is a value-copied property on the shared test sandbox, not a live binding) proving: fires once for a genuine dual-authored bound naming the primitive id + bound; does not repeat for the same (state, primitive, bound); never fires for a single-authored bound (negative control); DOES re-fire for a genuinely different state. A BLAST-RADIUS SWEEP walks every concept JSON under src/data/concepts/** — SPLIT IN TWO per founder review of PR #76: an ASSERTED regression gate scoped to x_domain ONLY (the one bound E-2 actually changed the precedence of — measured 0 across 165 files) and a REPORTED-ONLY (never asserted) count for plane x_range/y_range co-authoring, which was ALREADY expr-wins pre-E-2 (D3, unchanged) and is legal, on-purpose data (e.g. definite_integral_as_accumulated_area's own STATE_4 magnifier) — the original COMBINED assertion would have red-gated an unrelated branch composing this file with a concept that legitimately co-authors a plane range, a false positive this dispatch never intended to claim.",
        status: 'FIXED',
        concepts_affected: [
