/**
 * TEMP seed — potential_energy_system_of_charges into simulation_cache so
 ...
 * visual gate (mirrors _seed_electric_potential_system_of_charges_cache.ts).
 *
 * NOTE: the `system_pe_assembly` scenario is a NEW config-driven builder in
 * field_3d_renderer.ts (the ENERGY companion to system_of_charges: charges fly in
 * from infinity, pairwise energy bonds light up, a signed meter banks +U/−U). This
 * row must be seeded AFTER that engine change so the cached iframe HTML embeds the
 * new FIELD_3D_RENDERER_CODE. Re-seed (DELETE + re-run) if the renderer or the JSON
 * changes.
 *
 * Surgical: only deletes/inserts this concept's own simulation_cache rows.
 *
 * Run: npx tsx --env-file=.env.local src/scripts/_seed_potential_energy_system_of_charges_cache.ts
 */
...
const CONCEPT_ID = 'potential_energy_system_of_charges';
