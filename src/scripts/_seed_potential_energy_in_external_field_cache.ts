/**
 * TEMP seed — potential_energy_in_external_field into simulation_cache so
 ...
 * visual gate (mirrors _seed_potential_energy_system_of_charges_cache.ts).
 *
 * NOTE: the `pe_external_field` scenario is a config-driven builder in
 * field_3d_renderer.ts — a CHARGE phase (STATE_1..5: charges park at points of a
 * GIVEN potential V, signed U=qV tags, signed energy meter, equipotential
 * shading, qV-vs-mutual bonds, hill/well glyph) and a DIPOLE phase (STATE_6..9:
 * reuses the dipole-in-field engine — p arrow, θ arc, U=−pE·cosθ meter, θ-sweep /
 * oscillation, plus q + θ sliders in STATE_9). This row must be seeded AFTER the
 * engine change so the cached iframe HTML embeds the current FIELD_3D_RENDERER_CODE.
 * Re-seed (DELETE + re-run) if the renderer or the JSON changes.
 *
 * Surgical: only deletes/inserts this concept's own simulation_cache rows.
 *
 * Run: npx tsx --env-file=.env.local src/scripts/_seed_potential_energy_in_external_field_cache.ts
 */
...
const CONCEPT_ID = 'potential_energy_in_external_field';
...
async function main(): Promise<void> {
    ...
    const delFp = await supabaseAdmin.from('simulation_cache').delete().like('fingerprint_key', `${CONCEPT_ID}|%`);
    if (delFp.error) throw new Error(`delete (fingerprint) failed: ${delFp.error.message}`);
