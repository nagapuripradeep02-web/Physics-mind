/**
 * DC Pandey — embed + insert script
 * ===================================
 * Reads dc_pandey_chunks.jsonl (from ingest-dc-pandey.py),
 * generates 768-dim embeddings via gemini-embedding-001,
 * and INSERTs rows into ncert_content with source_book = 'dc_pandey'.
 *
 * Prerequisites:
 ...
 *   node src/scripts/embed-dc-pandey.mjs dc_pandey_chunks.jsonl --dry-run
 *   node src/scripts/embed-dc-pandey.mjs dc_pandey_chunks.jsonl --batch=10 --delay=300
 *
 * Options:
 *   --dry-run     Embed but skip Supabase inserts (prints first 3 rows)
 *   --batch=N     Insert batch size (default: 10)
 *   --delay=N     Ms between embedding requests (default: 250)
 *
 * Safety:
 *   Checks for existing dc_pandey rows before inserting.
...
if (!jsonlPath) {
    console.error("Usage: node embed-dc-pandey.mjs <chunks.jsonl> [--dry-run] [--batch=10] [--delay=250]");
    process.exit(1);
}
...
// ── Safety check: warn if dc_pandey rows already exist ────────────────────────
if (!isDryRun) {
    const { count, error } = await supabase
        .from("ncert_content")
        .select("id", { count: "exact", head: true })
        .eq("source_book", "dc_pandey");

    if (error) {
        console.error("Could not check existing dc_pandey rows:", error.message);
        process.exit(1);
    }

    if (count > 0) {
        console.warn(`\nWARNING: ${count} dc_pandey rows already exist in ncert_content.`);
        console.warn("Re-running will INSERT new rows (ON CONFLICT DO NOTHING).");
        console.warn("To start fresh: DELETE FROM ncert_content WHERE source_book = 'dc_pandey';");
...
async function insertBatch(rows) {
    ...
    // prevents duplicates on re-run. DELETE dc_pandey rows first if re-ingesting.
    const { error } = await supabase.from("ncert_content").insert(rows);

    if (error) {
        // Log and continue — don't abort the whole pipeline on a single batch failure
...
async function main() {
    ...
    if (!isDryRun) {
        ...
        console.log(`  WHERE source_book = 'dc_pandey'`);
