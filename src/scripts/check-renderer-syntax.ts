 ...
 * FIELD_3D_RENDERER_CODE / PARTICLE_FIELD_RENDERER_CODE / PARAMETRIC_RENDERER_CODE
 * template strings, so a stray backtick (terminates the template!) or JS syntax
 * slip ships silently and only surfaces as a blank canvas at runtime. Run after
 * any renderer edit:
...
for (const [name, code] of [
  ...
  ['parametric', PARAMETRIC_RENDERER_CODE],
] as const) {
