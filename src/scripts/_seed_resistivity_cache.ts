/**
 * TEMP seed — resistivity into simulation_cache so `npm run visual:eyes` /
 * `smoke:visual-validator` can drive it. Third 2D (particle_field) diamond
 * (Ch.3 #3): generation does not run through the live Sonnet pipeline, so we
 * assemble the sim HTML deterministically from the JSON's particle_field_config
 * and write one cache row. Mirrors _seed_ohms_law_cache.ts / _seed_drift_velocity_cache.ts.
 * Delete after the visual gate.
 *
 * Run: npx tsx --env-file=.env.local src/scripts/_seed_resistivity_cache.ts
 */
