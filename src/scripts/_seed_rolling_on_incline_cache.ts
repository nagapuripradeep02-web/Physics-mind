/**
 * TEMP seed — rolling_on_incline into simulation_cache so `npm run visual:eyes` can
 * drive it. field_3d generation does not run through the live Sonnet pipeline, so
 * we assemble the sim HTML deterministically from the JSON's field_3d_config
 * (exactly what the admin test page does) and write one cache row. Delete this
 * script after the visual gate (mirrors _seed_rolling_friction_cache.ts).
 *
 * Desk B guardrail 2: this script may only ever name a key this desk owns.
 *
 * Run: npx tsx --env-file=.env.local src/scripts/_seed_rolling_on_incline_cache.ts
 */
...
const CONCEPT_ID = 'rolling_on_incline';
