/**
 * TEMP seed — displacement_current into simulation_cache so `npm run visual:eyes` /
 ...
 * Delete after the visual gate (mirrors _seed_capacitance_cache.ts).
 *
 * NOTE (2026-07-24): the field_3d renderer does NOT yet implement
 * `scenario_type: "displacement_current"` — this script will assemble HTML
 * with no matching scenario case until the field3d-surgeon lands the engine
 * delta (architect skeleton §0b). Do not run THE EYE / visual gates against
 * this concept until that lands (per json_author dispatch instructions).
 *
 * Run: npx tsx --env-file=.env.local src/scripts/_seed_displacement_current_cache.ts
 */
...
const CONCEPT_ID = 'displacement_current';
