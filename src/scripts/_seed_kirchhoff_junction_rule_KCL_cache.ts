/**
 * TEMP seed — kirchhoff_junction_rule_KCL into simulation_cache so
 * `npm run visual:eyes` / `smoke:visual-validator` can drive it. Eighth 2D
 * (particle_field) diamond (Ch.3 #8): generation does not run through the
 ...
 * _seed_electrical_power_in_resistor_cache.ts / _seed_combination_of_resistors_cache.ts.
 * Delete after the visual gate.
 *
 * Run: npx tsx --env-file=.env.local src/scripts/_seed_kirchhoff_junction_rule_KCL_cache.ts
 */
...
const CONCEPT_ID = 'kirchhoff_junction_rule_KCL';
