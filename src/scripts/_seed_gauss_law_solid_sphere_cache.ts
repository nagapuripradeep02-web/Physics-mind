/**
 * TEMP seed — gauss_law_solid_sphere into simulation_cache so `npm run visual:eyes` /
 * `smoke:visual-validator` can drive it. field_3d generation does not run
 * through the live Sonnet pipeline, so we assemble the sim HTML deterministically
 * from the JSON's field_3d_config (exactly what the admin test page does) and
 * write one cache row. Delete this script after the visual gate (mirrors the
 * retired _seed_gauss_law_sphere_cache.ts / _seed_electric_flux_cache.ts).
 *
 * NOTE: the SOLID-sphere case reuses the existing field_3d gauss_law_sphere
 * scenario with gauss_sphere.distribution = 'solid' — the engine branch (linear
 * inside ramp + continuous peak at r=R) lives in field_3d_renderer.ts. THE EYE
 * photographs the assembled HTML; no live generation involved.
 *
 * Run: npx tsx --env-file=.env.local src/scripts/_seed_gauss_law_solid_sphere_cache.ts
 */
...
const CONCEPT_ID = 'gauss_law_solid_sphere';
