/**
 * TEMP seed — kirchhoff_loop_rule_KVL into simulation_cache so
 * `npm run visual:eyes` / `smoke:visual-validator` can drive it. Ninth 2D
 * (particle_field) diamond (Ch.3 #9): generation does not run through the
 ...
 * _seed_kirchhoff_junction_rule_KCL_cache.ts / _seed_electrical_power_in_resistor_cache.ts.
 * Delete after the visual gate.
 *
 * Run: npx tsx --env-file=.env.local src/scripts/_seed_kirchhoff_loop_rule_KVL_cache.ts
 */
...
const CONCEPT_ID = 'kirchhoff_loop_rule_KVL';
