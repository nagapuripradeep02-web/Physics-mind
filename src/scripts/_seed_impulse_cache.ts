/**
 * TEMP seed — impulse into simulation_cache so `npm run visual:eyes` can drive it.
 * field_3d generation does not run through the live Sonnet pipeline, so we assemble
 * the sim HTML deterministically from the JSON's field_3d_config (exactly what the
 * admin test page does) and write one cache row. Delete this script after the visual
 * gate (mirrors _seed_block_on_incline_cache.ts).
 *
 * Run: npx tsx --env-file=.env.local src/scripts/_seed_impulse_cache.ts
 */
