/**
 * TEMP seed — electric_potential_point_charge into simulation_cache so
 ...
 * visual gate (mirrors _seed_electric_potential_meaning_cache.ts).
 *
 * NOTE: this concept's V-vs-r curve panel (v_vs_r_curve) + charge_sign sign-flip
 * are NEW engine work in field_3d_renderer.ts (peter_parker:renderer-primitives,
 * FOUNDER-APPROVED, scenario point_charge_positive / potential_meaning block).
 * This row must be seeded AFTER that engine change so the cached iframe HTML
 * embeds the new FIELD_3D_RENDERER_CODE. Re-seed (DELETE + re-run) if the
 * renderer changes.
 *
 * Surgical: only deletes/inserts this concept's own simulation_cache rows —
 * never touches other concepts (the parallel TTS v3 session is review-site only).
 *
 * Run: npx tsx --env-file=.env.local src/scripts/_seed_electric_potential_point_charge_cache.ts
 */
...
const CONCEPT_ID = 'electric_potential_point_charge';
