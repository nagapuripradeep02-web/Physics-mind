/**
 * TEMP seed — helical_motion_charge_in_uniform_B into simulation_cache so
 ...
 * visual gate (mirrors _seed_circular_motion_charge_in_uniform_B_cache.ts /
 * _seed_cyclotron_period_independent_of_speed_cache.ts).
 *
 * NOTE: do NOT run this until the renderer's `helix_in_uniform_field` scenario
 * exists (executor's next step). Running it before then assembles an
 * unrenderable sim; the executor reseeds after the scenario lands.
 *
 * Run: npx tsx --env-file=.env.local src/scripts/_seed_helical_motion_charge_in_uniform_B_cache.ts
 */
...
const CONCEPT_ID = 'helical_motion_charge_in_uniform_B';
