/**
 * TEMP seed — electric_potential_dipole into simulation_cache so
 ...
 * visual gate (mirrors _seed_parallel_plate_capacitor_field_cache.ts).
 *
 * NOTE: the `dipole_potential` scenario is a NEW config-driven builder in
 * field_3d_renderer.ts (peter_parker:renderer-primitives). This row must be
 * seeded AFTER that engine change so the cached iframe HTML embeds the new
 ...
 *
 * Run: npx tsx --env-file=.env.local src/scripts/_seed_electric_potential_dipole_cache.ts
 */
...
const CONCEPT_ID = 'electric_potential_dipole';
