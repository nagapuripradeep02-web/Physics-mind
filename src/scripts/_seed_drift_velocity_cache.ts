/**
 * TEMP seed — drift_velocity into simulation_cache so
 * `npm run visual:eyes` / `smoke:visual-validator` can drive it. The FIRST 2D
 * (particle_field) diamond: generation does not run through the live Sonnet
 * pipeline, so we assemble the sim HTML deterministically from the JSON's
 * particle_field_config and write one cache row. The physics_config carries
 * BOTH epic_l_path (state ids/durations) and particle_field_config (motion /
 * reveal / hold derivation in deriveStateMeta.ts). Delete after the visual
 * gate (mirrors _seed_magnetic_flux_cache.ts).
 *
 * Run: npx tsx --env-file=.env.local src/scripts/_seed_drift_velocity_cache.ts
 */
...
const CONCEPT_ID = 'drift_velocity';
