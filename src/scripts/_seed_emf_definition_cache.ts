/**
 * TEMP seed — emf_definition into simulation_cache so `npm run visual:eyes` /
 * `smoke:visual-validator` can drive it. Fifth 2D (particle_field) diamond
 * (Ch.3 #5): generation does not run through the live Sonnet pipeline, so we
 * assemble the sim HTML deterministically from the JSON's particle_field_config
 * and write one cache row. Mirrors _seed_combination_of_resistors_cache.ts.
 * Delete after the visual gate.
 *
 * Run: npx tsx --env-file=.env.local src/scripts/_seed_emf_definition_cache.ts
 */
...
const CONCEPT_ID = 'emf_definition';
