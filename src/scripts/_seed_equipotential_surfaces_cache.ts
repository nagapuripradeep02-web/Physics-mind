/**
 * TEMP seed — equipotential_surfaces into simulation_cache so
 ...
 * visual gate (mirrors _seed_electric_potential_point_charge_cache.ts).
 *
 * NOTE: this concept's tangential slide_along_shell rig, the
 * show_field_lines_cross_shells overlay, and the per-state shells_override are
 * NEW engine work in field_3d_renderer.ts (peter_parker:renderer-primitives,
 * FOUNDER-APPROVED, scenario point_charge_positive / potential block, all three
 * OPTIONAL/default-off). This row must be seeded AFTER that engine change so the
 ...
 *
 * Run: npx tsx --env-file=.env.local src/scripts/_seed_equipotential_surfaces_cache.ts
 */
...
const CONCEPT_ID = 'equipotential_surfaces';
