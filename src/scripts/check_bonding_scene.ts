 ...
 *
 * SECTION OWNERSHIP (doc §gate). E1 owns 1, 4, 5, 10, 11, 12.
 * 2/3/6/7/8/9/13/14 belong to E2 (link layer) and E3 (lattice layer) and print
 * as declared SKIPs with their owner — never silently absent.
 *
...
const VARS = [
  ...
  "BS_ARROW_D_PER_UNIT", "BS_MODES_E1", "BS_MODES_DEFERRED", "BS_MODES",
  "BS_CONTROL_IDS", "BS_HUD_LINES", "BS_HUD_LINES_E1", "BS_PLACEMENTS",
...
console.log("\n=== 6. LINK CRITERION, DERIVED AT BOTH ENDS ===");
skip("delta sweep flips link formation; form_pm < break_pm", "E2 (link layer)");
// E1 ships the DERIVED delta the criterion thresholds on, so assert that now:
// the S4 lesson ("sulfur is not negative enough") must be a real number gap.
{
...
console.log("\n=== 9. JIGGLE AMPLITUDE vs TEMPERATURE ===");
skip("full sqrt(T) sweep with the temperature control", "E2 (link layer)");
...
// is either IMPLEMENTED in E1 or explicitly DECLARED-DEFERRED to E2/E3.
{
  ...
  ok("BS_MODES = implemented + deferred, with no overlap and no gap",
    sameSet(E.BS_MODES, [...E.BS_MODES_E1, ...E.BS_MODES_DEFERRED]) &&
    !E.BS_MODES_E1.some((m: string) => E.BS_MODES_DEFERRED.includes(m)),
    `E1=[${E.BS_MODES_E1.join(",")}]  deferred=${E.BS_MODES_DEFERRED.length}`);
  ...
  const unread = E.BS_MODES_E1.filter((m: string) => !upd.includes('"' + m + '"'));
  ok("every IMPLEMENTED mode is read by the frame/apply pass", unread.length === 0, unread.join(" "));
  const leaked = E.BS_MODES_DEFERRED.filter((m: string) => upd.includes('"' + m + '"'));
  ok("no DEFERRED mode is half-implemented in E1", leaked.length === 0, leaked.join(" "));

  // Every E1 hud line is actually rendered; every E1 line is in the closed enum.
  const unreadHud = E.BS_HUD_LINES_E1.filter((h: string) => !upd.includes('"' + h + '"'));
  ok("every E1 hud_line is rendered by the HUD pass", unreadHud.length === 0, unreadHud.join(" "));
  ok("BS_HUD_LINES_E1 is a subset of the closed enum",
    E.BS_HUD_LINES_E1.every((h: string) => E.BS_HUD_LINES.includes(h)));
...
console.log(failures === 0
  ? "\n✅ check:bonding-scene — all E1 sections pass (2/3/6/7/8/9/13/14 are declared E2/E3 stubs).\n"
