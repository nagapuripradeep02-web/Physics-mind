/**
 * TEMP seed — parallel_plate_capacitor_field into simulation_cache so
 ...
 * visual gate (mirrors _seed_equipotential_surfaces_cache.ts).
 *
 * NOTE: the `parallel_plates` scenario was built out from a static stub to a
 * config-driven 7-state capacitor arc (gap bracket, uniform field lines, 3 equal
 * probe arrows, two-sheet superposition, edge-on fringe, gap-widen E=V/d halving,
 * draggable test charge + V/d sliders) in field_3d_renderer.ts
 * (peter_parker:renderer-primitives). This row must be seeded AFTER that engine
 * change so the cached iframe HTML embeds the new FIELD_3D_RENDERER_CODE.
 * Re-seed (DELETE + re-run) if the renderer changes.
 *
 * Surgical: only deletes/inserts this concept's own simulation_cache rows —
 * never touches other concepts.
 *
 * Run: npx tsx --env-file=.env.local src/scripts/_seed_parallel_plate_capacitor_field_cache.ts
 */
...
const CONCEPT_ID = 'parallel_plate_capacitor_field';
