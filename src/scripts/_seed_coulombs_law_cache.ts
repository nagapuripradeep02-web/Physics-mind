/**
 * TEMP seed — coulombs_law into simulation_cache so `npm run visual:eyes` /
 ...
 * from the JSON's field_3d_config and write one cache row. Delete after the
 * visual gate (mirrors _seed_torque_on_current_loop_in_field_cache.ts).
 *
 * Run: npx tsx --env-file=.env.local src/scripts/_seed_coulombs_law_cache.ts
 */
