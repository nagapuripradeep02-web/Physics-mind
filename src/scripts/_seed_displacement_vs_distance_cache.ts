/**
 * Prewarm seed for displacement_vs_distance following the renderer_primitives
 * regen directive (kinematics_1d_track field_3d scenario build + buildScenario()
 * default-case field_lines.count crash fix). field_3d generation does not run
 * through the live Sonnet pipeline, so we assemble the sim HTML deterministically
 * from the JSON's field_3d_config and write one fresh cache row — this IS the
 * prewarm path for field_3d concepts (mirrors _seed_eddy_currents_cache.ts).
 *
 * Run: npx tsx --env-file=.env.local src/scripts/_seed_displacement_vs_distance_cache.ts
 */
...
async function main(): Promise<void> {
    ...
    const ins = await supabaseAdmin.from('simulation_cache').insert({
        ...
        engine: 'threejs',
        fingerprint_key: `${CONCEPT_ID}|understand|${CLASS_LEVEL}|conceptual|none`,
        served_count: 1,
    });
