/**
 * TEMP seed — gauss_law_sphere into simulation_cache so `npm run visual:eyes` /
 ...
 * retired _seed_gauss_law_cache.ts / _seed_electric_flux_cache.ts).
 *
 * NOTE: the gauss_law_sphere field_3d scenario is NEW engine work
 * (peter_parker:renderer-primitives, architect FLAG #R1) — THE EYE will not
 * render correctly until that scenario ships in field_3d_renderer.ts. Seeding is
 * still authored so the visual gate can be run the moment the scenario lands.
 *
 * Run: npx tsx --env-file=.env.local src/scripts/_seed_gauss_law_sphere_cache.ts
 */
...
const CONCEPT_ID = 'gauss_law_sphere';
