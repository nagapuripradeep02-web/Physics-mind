/**
 * TEMP seed — bar_magnet_as_dipole into simulation_cache so `npm run visual:eyes`
 * / `smoke:visual-validator` can drive it. field_3d generation does not run through
 * the live Sonnet pipeline, so we assemble the sim HTML deterministically from the
 * JSON's field_3d_config and write one cache row. Delete after the visual gate.
 *
 * Run: npx tsx --env-file=.env.local src/scripts/_seed_bar_magnet_as_dipole_cache.ts
 */
...
const CONCEPT_ID = 'bar_magnet_as_dipole';
