/**
 * TEMP seed — combination_of_resistors into simulation_cache so `npm run visual:eyes` /
 * `smoke:visual-validator` can drive it. Fourth 2D (particle_field) diamond
 * (Ch.3 #4): generation does not run through the live Sonnet pipeline, so we
 * assemble the sim HTML deterministically from the JSON's particle_field_config
 * and write one cache row. Mirrors _seed_ohms_law_cache.ts / _seed_resistivity_cache.ts.
 * Delete after the visual gate.
 *
 * Run: npx tsx --env-file=.env.local src/scripts/_seed_combination_of_resistors_cache.ts
 */
...
const CONCEPT_ID = 'combination_of_resistors';
