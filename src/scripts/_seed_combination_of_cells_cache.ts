/**
 * TEMP seed — combination_of_cells into simulation_cache so `npm run visual:eyes` /
 * `smoke:visual-validator` can drive it. Generation does not run through the live
 * Sonnet pipeline, so we assemble the sim HTML deterministically from the JSON's
 * particle_field_config and write one cache row. Mirrors _seed_internal_resistance_cache.ts.
 * Delete after the visual gate.
 *
 * Run: npx tsx --env-file=.env.local src/scripts/_seed_combination_of_cells_cache.ts
 */
...
const CONCEPT_ID = 'combination_of_cells';
