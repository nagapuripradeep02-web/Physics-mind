/**
 * TEMP seed — force_on_current_carrying_wire (A15) into simulation_cache so
 ...
 * visual gate (mirrors the retired _seed_solenoid_cache.ts).
 *
 * Run: npx tsx --env-file=.env.local src/scripts/_seed_fcw_cache.ts
 */
...
const CONCEPT_ID = 'force_on_current_carrying_wire';
...
async function main(): Promise<void> {
    ...
    // surgical clear — only this concept's rows (do NOT nuke the whole cache;
    // another session may be working on other concepts).
