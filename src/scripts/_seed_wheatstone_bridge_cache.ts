/**
 * TEMP seed — wheatstone_bridge into simulation_cache so `npm run visual:eyes`
 * / `smoke:visual-validator` can drive it. Tenth 2D (particle_field) diamond
 * (Ch.3 #10, first of the founder's lab trio Wheatstone c20 -> potentiometer
 * c23 -> meter bridge): generation does not run through the live Sonnet
 * pipeline, so we assemble the sim HTML deterministically from the JSON's
 * particle_field_config and write one cache row. Mirrors
 * _seed_kirchhoff_loop_rule_KVL_cache.ts / _seed_kirchhoff_junction_rule_KCL_cache.ts.
 *
 * NOTE: this concept rides NEW engine flags (topology:'bridge',
 * drawGalvanometerC, node readouts, ratio HUD, bridge_r_sweep /
 * bridge_emf_sweep — see the JSON's source_book) that are authored but not
 * yet implemented in particle_field_renderer.ts as of this authoring pass
 * (flagged for peter_parker:renderer_primitives via quality_auditor). This
 * seed script will run fine (assembleParticleFieldHtml doesn't validate flag
 * support), but THE EYE / visual gate will show the pre-engine-upgrade
 * rendering (series/parallel fallback) until that engine step lands.
 *
 * Delete after the visual gate.
 *
 * Run: npx tsx --env-file=.env.local src/scripts/_seed_wheatstone_bridge_cache.ts
 */
...
const CONCEPT_ID = 'wheatstone_bridge';
