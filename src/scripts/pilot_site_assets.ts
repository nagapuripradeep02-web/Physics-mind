...
// profile + unexpired trial OR active subscription — access_end = max(trial_end,
// paid_until); teacher_subscriptions is service-role-written, see the 2026-07-12
// migration). PM.authReady resolves AFTER the profile check, so pages can read
// window.PM_PROFILE / window.PM_TRIAL_END / window.PM_PAID_UNTIL / window.PM_PLAN
// inside .then().
...
(function () {
  ...
  client.auth.getSession().then(function (res) {
    ...
    // ── Profile gate: no profile -> welcome.html; expired trial AND no active
    // subscription -> expired.html. The subscription rides along as a to-one embed
    // (FK on the PK). Fail-open on any error (classroom availability wins over the gate).
    client.from('teacher_profiles').select('*, teacher_subscriptions(plan, paid_until)').maybeSingle().then(function (pr) {
      ...
      // Subscription (service-role-written; teachers can only read it): while
      // paid_until is in the future the teacher is a paying member — access never
      // expires and brand surfaces show the plan instead of the trial countdown.
      var sub = p.teacher_subscriptions || null;
      if (sub && sub.length !== undefined) sub = sub[0] || null;   // embed shape can be object or array
      var paidUntil = 0;
      try { if (sub && sub.paid_until) paidUntil = new Date(sub.paid_until).getTime(); } catch (e) { paidUntil = 0; }
      window.PM_PAID_UNTIL = paidUntil;
...
<div class="wrap"><div class="card">
  ...
  <p class="sub">Everything you set up — your saved lesson layouts, renames, and customizations — is safe
     and waiting exactly as you left it.${PAYMENT_LINK
        ? ' To keep teaching with Viditra, continue on the founding-teacher plan — &#8377;699/month, locked for as long as you stay.'
        : ' To keep teaching with Viditra, message Pradeep and he will activate your founding-teacher plan (&#8377;699/month, locked).'}</p>
${PAYMENT_LINK ? `  <a class="go" href="${PAYMENT_LINK}">Continue — &#8377;699/month</a>
  <p class="hint">UPI, card, or netbanking. Access reopens shortly after payment — usually within the hour.</p>
  <a class="alt" href="mailto:pradeep@viditra.co?subject=Continue%20my%20Viditra%20access">Questions? Email pradeep@viditra.co</a>`
