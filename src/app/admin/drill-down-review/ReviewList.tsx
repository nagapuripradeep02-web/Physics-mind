...
function firstStateTitle(row: DrillDownRow): string {
    const states = row.sub_sim?.states ?? {};
    const keys = Object.keys(states);
    if (keys.length === 0) return "(no states)";
    const first = states[keys[0]] as { title?: string } | undefined;
    return first?.title ?? "(untitled)";
}

function stateCount(row: DrillDownRow): number {
    return Object.keys(row.sub_sim?.states ?? {}).length;
}

export function DrillDownReviewList({ rows }: Props) {
    return (
        <div className="space-y-4">
            {rows.map((row) => (
                <DrillDownCard key={row.id} row={row} />
            ))}
        </div>
    );
}
...
function DrillDownCard({ row }: { row: DrillDownRow }) {
    ...
    async function submit(which: "approve" | "reject") {
        ...
        try {
            const res = await fetch("/api/admin/review-action", {
                ...
                body: JSON.stringify({ table: "drill_down_cache", id: row.id, action: which }),
            });
            if (!res.ok) {
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <div className="font-mono text-sm text-slate-400">
                            {row.concept_id} / {row.state_id} / {row.cluster_id}
                        </div>
    ...
    const protocolBadgeClass = row.protocol === "MICRO"
        ...
        >
            <div className="p-4">
                <div className="flex items-start justify-between mb-2">
                    <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                            <span className={`px-2 py-0.5 text-xs font-mono border rounded ${protocolBadgeClass}`}>
                                {row.protocol ?? "?"}
                            </span>
                        ...
                        <div className="text-xs text-slate-500">
                            {stateCount(row)} state{stateCount(row) === 1 ? "" : "s"}
                            <span className="mx-2">·</span>
                            {row.class_level} / {row.mode}
                            <span className="mx-2">·</span>
                            served {row.served_count}×
                            <span className="mx-2">·</span>
                ...
                <div className="mt-3 bg-slate-950 border border-slate-800 rounded p-3">
                    <div className="text-xs text-slate-500 mb-1">First state title</div>
                    <div className="text-slate-200 text-sm mb-2">{firstStateTitle(row)}</div>
