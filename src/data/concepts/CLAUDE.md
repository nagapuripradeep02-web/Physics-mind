...
- **Pacing rule (Rule 31a/31b)** — guided states are straightforward distinct-motion beats **~15–25s (≈3–4 tight sentences)** (no Socratic predict→`wait_for_answer`→reveal, no `pause_after_ms` prediction pauses on new concepts); no two states identical in motion/teaching; no static state; final explore state = duration 0/open + `interaction_complete`.
