...
- **Pacing rule (Rule 31a/31b, word budget 2026-07-08)** — a guided state = **ONE idea + ONE complete motion**, narration **25–55 EN words (2–4 tight sentences ≈ 10–20s)** counted on `text_en` (>55 = two ideas, split; <~20 = merge/enrich; motion may outrun narration, never the reverse; no Socratic predict→`wait_for_answer`→reveal, no `pause_after_ms` prediction pauses on new concepts); no two states identical in motion/teaching — each state declares a motion archetype + delta line, no archetype repeat except a declared contrast pair; no static state; final explore state = duration 0/open + `interaction_complete`. **Legibility (Rule 32):** cause before effect · one variable moves · caption opens with the ≤5-word delta cue · home-pose continuity · one glow focal.
