...
- **Interactivity rule (UPDATED 2026-06-30 — live-instrument model)** — every EPIC-L state MAY be live: set `show_sliders: true` on its `field_3d_config.states` entry. The sim waits for the teacher; authored demo-motion (`idle_sway` / `oscillation` / `theta_sweep`) keeps running until the teacher grabs a control, at which point that control's axis switches to `manual` (gated on a *trusted* drag in `field_3d_renderer.ts`). This **supersedes** the 2026-05-21 "last-state-only" rule, which fit the auto-advancing guided-lesson model that field_3d sims are migrating off (see `PROGRESS.md` 2026-06-30 + plan `cached-crafting-platypus`; `electric_dipole_in_field` is the reference conversion). Guardrails when making states live: keep ≥2 distinct `advance_mode` values across `epic_l_path.states` (Gate 12); **never** set `advance_mode: 'auto_after_animation'` on a static-but-live state (it trips THE EYE motion heuristic → false "motion died"); re-seed the concept's cache after restructuring so `deriveMotionExpectations` reads the new `field_3d_config`.
