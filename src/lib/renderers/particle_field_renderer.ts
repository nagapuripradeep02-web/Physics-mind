...
function drawGasKinModeA(state, gx, gy, gw, gh, dim) {
  ...
  for (s = 0; s < seriesIds.length; s++) {
    ...
    // Two series coinciding by construction is drawn dashed on the second —
    // the existing convention at :5088-5092 (drawGasConcGraph), not re-derived.
    dashed = s > 0;
    noFill(); strokeHex(col, 0.95 * dim); strokeWeight(2);
    if (dashed && drawingContext && drawingContext.setLineDash) drawingContext.setLineDash([6, 4]);
    beginShape();
    for (i = 0; i < gasKinBuf.length; i++) {
      e = gasKinBuf[i];
      if (e.t > ax.x1) break;
...
function drawGasKinMarks(state, gx, gy, gw, gh, dim, px, py) {
  ...
  for (i = 0; i < marks.length; i++) {
    ...
    if (rep) {
      ...
      fit = gasKinFitWindow(seriesSel, dt0, dt1);
      // Still filling: nothing to show yet, stay off (distinct from a window
      // that CLOSED with too few samples, which falls through and honestly
      // prints "measuring…" rather than a number it cannot support). Gated on
      // dt1, not t1: a converge_from mark's effective window right edge is dt1
      // while it is animating, and only equals t1 once the slide finishes.
      // A repeat-governed mark (rep truthy) skips this gate entirely — see above.
      if (!fit && PM_simTimeMs < dt1) continue;
    }
