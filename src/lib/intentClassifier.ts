...
export const VALID_CONCEPT_IDS: ReadonlySet<string> = new Set([
    ...
    // Work, Energy and Power #5 (Class 11 Ch.6.6 — same newtons_laws_body
    // field_3d engine + SEAM K/L/M/N energy layer, and the FIRST fleet
    // exercise of a checkpoint seeded exactly at the home pose: it adopts side
    // without firing, then fires once on the RETURN crossing and latches). A
    // force is CONSERVATIVE when its total work around any closed path is
    // exactly zero — equivalently, its work depends only on the endpoints, not
    // the path taken (gravity: a round trip's work bar falls then climbs back,
    // and the start-line point reads "W gravity = 0.0 J"). Friction is
    // NOT conservative: it always opposes the instantaneous motion, so its
    // work is negative on the way out AND the way back, and a round trip ADDS
    // the two losses instead of cancelling them (the start-line stamp reads
    // "W gravity = 0.0 J, W friction = −125.3 J" — the PRIMARY aha). The
    // friction arrow debuts flipping 180° at the turnaround (the mechanism:
    // friction re-aims with the motion, gravity never does), and two points on
    // a frictionless slope stamp an identical gravity reading on both passes
    // (the endpoint-only property potential_energy_definition builds on next).
    // Does NOT define potential energy, energy conservation, or where
    // friction's lost joules go, and does not re-teach the work definition,
    // the sign taxonomy, or kinetic energy — all prerequisites. Alex pipeline,
    // 2026-08-07.
    'conservative_vs_nonconservative_forces',
    // Work, Energy and Power #4 (Class 11 Ch.6.2 — same newtons_laws_body
    // field_3d engine, and the FIRST concept in the fleet to author BOTH the
    // SEAM L energy_layer AND SEAM M work_accumulators together, which is
    // what turns the engine's own panel header to "Energy and work bars").
    // The NET work done on a body equals the change in its kinetic energy,
    // W_net = ΔK = ½mv² − ½mv₀²: a steady pull from rest fills K in lockstep
    // with the net-work bar (the PRIMARY aha, both landing on 40.0 J);
    // friction alone removes the same 40.0 J, signed negative; a pull
    // exactly balancing kinetic friction does +98.0 J and −98.0 J that sum
    // to a net of zero while K never moves (only the NET work changes K);
    // a cart already moving through a labelled point A shows ΔK is
    // measured from the STARTING energy, never the final ½mv² alone — the
    // scene HOLDS at point A and at point B so both can be read and subtracted. Does NOT cover what
    // work is or W = F·d·cos θ (work_done_by_constant_force), the sign
    // taxonomy of work (positive_negative_zero_work — assumed prerequisite,
    // not re-taught), what kinetic energy is (kinetic_energy_definition —
    // assumed prerequisite), path-dependence of work, potential energy of
    // any kind, where removed kinetic energy goes (heat/sound — a later
    // concept's content), or power. NOTE: this id COLLIDES with a legacy,
    // never-registered mechanics_2d entry of the same name (old Ch.9
    // catalog); that dead entry has been removed from CONCEPT_RENDERER_MAP
    // and CONCEPT_PANEL_MAP in favour of this live field_3d atomic — see the
    // removal comments at both sites (2026-08-05, mirrors the rolling_on_incline
    // precedent 2026-08-04). Alex pipeline, 2026-08-05.
    'work_energy_theorem',
    // Work, Energy and Power #6 (Class 11 Ch.6.7 — same newtons_laws_body
    // field_3d engine, the FIRST concept in the fleet to render the U_grav
    // energy bar, and the first to pair the SEAM L energy_layer with a work
    // accumulator where the claim IS their numeric CHANGE-identity). U, the
    // potential energy, is defined by ΔU = −W_conservative: when gravity does
    // −128.0 J of work climbing a frictionless slope, U climbs by exactly
    // +128.0 J in the same frames (the PRIMARY aha); on the descent gravity's
    // work turns positive and U falls back by the same joules — stored, not
    // destroyed. On a rough slope a third bar (friction) falls on BOTH legs
    // with no partner rising to meet it — friction's joules become heat, and
    // only a conservative force (path-independent work) can have a U at all.
    // Two marked points, held under a physics-freezing dwell, show that the
    // LEVELS of U and W differ (offset by the reference) but only the CHANGE
    // matches: 135.2 − 88.2 = 47.0 J, exactly minus the work gravity did
    // between them. Does NOT teach U = mgh, the freedom to choose h = 0, or
    // path independence (gravitational_potential_energy, #7 — the dashed
    // U = 0 line is named once and never discussed as a choice); does NOT
    // teach spring PE, K, E_total, conservation, or the accounting of where
    // friction's heat goes — all later concepts. Alex pipeline, 2026-08-10.
    'potential_energy_definition',
    // Work, Energy and Power #7 (Class 11 Ch.6.8 — same newtons_laws_body
    // field_3d engine). Near the Earth's surface a body's gravitational
    // potential energy is U = mgh, measured from a height you CHOOSE to call
    // zero — the choice moves every U value but never the difference
    // ΔU = mgΔh, which depends only on the start and end heights, never the
    // path between them. One apparatus (a cart on a ramp, a U bar, a dashed
    // h = 0 line) carries three invariances across five guided states: same
    // place gives the same U (a launched-and-returned round trip), a MOVED
    // zero line shifts every U but never ΔU (the PRIMARY aha — the bar opens
    // at 0.0 J instead of 8.8 J, yet the stamped gain still reads 29.4 J), and
    // a steeper ramp to the SAME height gives the SAME U (path independence —
    // d = 1.40 m instead of 2.40 m, still 44.1 J). The final state derives
    // U = mgh by applying potential_energy_definition's own ΔU = −W_conservative
    // relation to gravity, with two latched checkpoint stamps proving the
    // subtraction on screen (8.6 J each way). Does NOT teach the general
    // definition of potential energy or ΔU = −W_conservative (that is
    // potential_energy_definition, a shipped prerequisite, cited by
    // concept_id only, never by state number); does NOT teach spring
    // potential energy, K, total mechanical energy, energy conservation, or
    // friction losses; and does NOT touch the general −GMm/r form. Alex
    // pipeline, 2026-08-10.
...
════════════════════════════
  ...
  conservative_vs_nonconservative_forces ← the ROUND-TRIP TEST: a force is CONSERVATIVE when its total work around any CLOSED PATH is exactly zero, equivalently when its work depends only on the ENDPOINTS never the path taken — gravity is the exemplar, a work bar that falls then climbs back to "W gravity = 0.0 J" the instant a block re-crosses its own start line; friction is NOT conservative, because it always opposes the instantaneous motion its work is negative on the way out AND the way back, so a round trip ADDS the two losses instead of cancelling them (latched "W gravity = 0.0 J, W friction = −125.3 J", the PRIMARY aha); the friction arrow FLIPS 180° at the turnaround while the weight arrow never changes direction (the mechanism WHY); and on a frictionless slope the same gravity reading repeats EXACTLY at the same spot on the way up and the way down (path independence, the property potential energy is built from). Does NOT define potential energy, energy conservation, or where friction's lost joules go, and does NOT re-teach the work definition, the sign taxonomy, or kinetic energy.
  work_energy_theorem ← the NET work done on a body equals the change in its kinetic energy, W_net = ΔK = ½mv² − ½mv₀²: a steady pull from rest fills K in lockstep with the net-work bar, both landing on the same 40.0 J (the PRIMARY aha — one meter, two readings); friction alone removes the same 40.0 J, the net bar signed negative as K falls to exactly zero; a pull authored EXACTLY equal to kinetic friction does +98.0 J and −98.0 J that sum to a net of zero while K never moves — only the NET work changes kinetic energy, not any one force's own work; a cart ALREADY MOVING through a labelled point A shows the change is measured from the STARTING kinetic energy, never the final ½mv² alone — the scene HOLDS at point A and again at point B so both readings can be read off and subtracted (K 18.0 J then 36.0 J, net 18.0 J). Does NOT cover what work is or W = F·d·cos θ (work_done_by_constant_force), the sign taxonomy of work (positive_negative_zero_work — assumed prerequisite, not re-taught), what kinetic energy is (kinetic_energy_definition — assumed prerequisite), whether work depends on the path, potential energy of any kind, where removed kinetic energy goes, or power. First concept in the fleet to author work bars AND the K bar together in one panel.
  potential_energy_definition ← potential energy U is DEFINED by ΔU = −W_conservative: a block launched up a frictionless slope shows gravity's work bar falling to −128.0 J while U climbs from 49.0 to 177.0 J — equal joule changes, opposite directions, every frame (the PRIMARY aha); on the descent gravity's work turns positive and U falls back by the same joules — stored, not destroyed. On a rough slope a THIRD bar (friction) falls on BOTH legs with no partner rising to meet it — friction's joules become heat, and only a CONSERVATIVE force (one whose work is path-independent) can have a potential energy at all, which is why "friction potential energy" does not exist. Between two marked points, held under a physics-freezing dwell, the LEVELS of U and W differ (offset by the reference height) but only the CHANGE matches: 135.2 − 88.2 = 47.0 J, exactly minus the work gravity did between them — ΔU is a difference, never a level read alone. Does NOT teach U = mgh or any formula for U's value, the freedom to choose the h = 0 reference, or path independence (gravitational_potential_energy — the dashed U = 0 line is named once as where the count starts and never discussed as a choice); does NOT teach spring potential energy, kinetic energy, total mechanical energy, energy conservation, or the accounting of where friction's heat goes; and does NOT re-teach the work meter, work's sign, or the round-trip test (all assumed prerequisites).
  gravitational_potential_energy ← near the Earth's surface a body's gravitational potential energy is U = mgh, measured from a height you CHOOSE to call zero — the choice moves EVERY U value but never the difference ΔU = mgΔh, which depends only on the start and end heights, never the path taken. A cart driven up a ramp: the U bar climbs in step with height, and two point-stamps make U = mgh a number twice (14.7 J then 44.1 J); a launched-and-returned cart stamps the SAME U at departure and at return — the place sets the energy, not the trip; the PRIMARY aha moves the dashed h = 0 line to the cart's own start height — the bar now opens EMPTY at 0.0 J and every stamped U changes (5.9 J, 35.3 J), yet the stamped gain still reads the same 29.4 J; a steeper ramp to the SAME height needs less travel distance (d = 1.40 m instead of 2.40 m) but stamps the identical 44.1 J — path independence; and a final derivation state applies ΔU = −W_conservative to gravity with two latched checkpoint stamps, proving 8.6 J gained exactly mirrors 8.6 J of gravity's work lost. Does NOT teach the general definition of potential energy or ΔU = −W_conservative itself (potential_energy_definition, assumed prerequisite, never re-taught), spring potential energy, kinetic energy, total mechanical energy, energy conservation, friction losses, or the general −GMm/r form (a later chapter).
...
- "what is a conservative force" / "conservative vs non conservative force" / "why is friction non conservative" / "why is gravity a conservative force" / "does the path matter for work done" / "work done over a closed path" / "round trip work done by gravity" / "why doesn't friction cancel on the way back" / "shouldn't going back undo the loss from going out" / "why does friction add up instead of cancelling" / "is a closed path just going back to where you started" / "why does friction flip direction" / "does friction always point the opposite way you're moving" / "why doesn't gravity change direction like friction does" / "why doesn't the path matter for gravity's work" / "does it matter how you get from a to b for gravity" / "why is work by gravity only about start and end point" / "test for a conservative force" → conservative_vs_nonconservative_forces (the CLOSED-PATH TEST: total work zero around any loop = conservative, gravity; friction is NOT conservative because it opposes the motion on every leg, so a round trip adds the losses. Does NOT cover potential energy, energy conservation, or where friction's lost joules go — NOT work_done_by_constant_force, positive_negative_zero_work, or kinetic_energy_definition, all of which this concept assumes as prerequisite and never re-teaches)
- "work energy theorem" / "work-energy theorem" / "W = delta K" / "net work equals change in kinetic energy" / "how is work related to kinetic energy" / "why does net work change the speed" / "does the work done by one force become kinetic energy" / "is the applied force's work the same as the net work" / "can a force do work with no change in kinetic energy" / "net work when a force cancels friction" / "moving at constant speed but a force is acting is there work" / "why doesn't net work equal the final kinetic energy" / "do I subtract the starting kinetic energy" / "what is net work when the object turns around" / "does the work energy theorem hold when velocity reverses" / "derive work energy theorem from F = ma" / "prove W = delta K" → work_energy_theorem (the RELATION between net work and kinetic energy — W_net = ΔK = ½mv² − ½mv₀². Covers: the theorem itself shown positive (a lone pull fills K) and negative (friction alone empties it); the confrontation that only the NET work changes K, never one force's own work, even when that force does large nonzero work; ΔK as a CHANGE measured from the STARTING energy, not the final ½mv² alone, including through a velocity reversal; and the derivation from F = ma. Does NOT cover what work IS or W = F·d·cos θ — that is work_done_by_constant_force; does NOT cover the sign taxonomy of work by angle — that is positive_negative_zero_work; does NOT cover what kinetic energy IS — that is kinetic_energy_definition; does NOT cover path-dependence, potential energy, where the energy goes, or power)
- "what is potential energy" / "potential energy definition" / "delta U equals minus W" / "why is there a minus sign in potential energy" / "if gravity does negative work where does that energy go" / "does negative work mean energy disappeared" / "why does potential energy increase when work is negative" / "why can't friction have a potential energy" / "why does the definition need the word conservative" / "what makes a force conservative or not for potential energy" / "does pushing against friction store energy the same way as lifting" / "how do I calculate delta U between two points" / "is delta U always equal to minus the work by gravity" / "why doesn't U equal minus W at every point" / "is it the level or the change that has to match" / "does potential energy depend on speed" → potential_energy_definition (the DEFINITION ΔU = −W_conservative: negative work by gravity is stored, not destroyed — U rises by exactly the joules gravity's work falls, and falls back by the same joules on the way down; only a force whose work is path-independent (conservative) can have a potential energy at all, which is why friction has none — its removed energy becomes heat instead; between two points the LEVEL of U and W differ but the CHANGE always matches. Does NOT cover U = mgh or any formula for U's value, the free choice of the h = 0 reference, or path independence itself — that is gravitational_potential_energy; does NOT cover spring potential energy, kinetic energy, total mechanical energy, or conservation; does NOT re-teach the round-trip test — that is conservative_vs_nonconservative_forces, assumed prerequisite)
- "U equals mgh" / "gravitational potential energy formula" / "potential energy of an object at a height" / "why can you choose the zero of height" / "does changing the reference height change the real potential energy" / "why did the potential energy become zero when nothing moved" / "is potential energy real if you can choose where zero is" / "does it matter if I lift something straight up or push it up a ramp" / "why does a longer ramp not store more energy" / "why does a gentler ramp need less force but the same energy" / "does the path taken change the potential energy" / "how do you derive U equals mgh" / "why is U equals mgh only true near the Earth's surface" → gravitational_potential_energy (U = mgh measured from a height you CHOOSE to call zero — the choice moves every U value but never ΔU = mgΔh, which depends only on the start and end heights, never the path between them; a moved zero line is the PRIMARY aha (the bar opens empty yet the stamped gain is unchanged); a steeper ramp to the same height needs less travel distance but stores the identical energy — path independence; the final state derives U = mgh from ΔU = −W_conservative applied to gravity. Does NOT cover the general definition of potential energy or ΔU = −W_conservative itself — that is potential_energy_definition, assumed prerequisite, never re-taught; does NOT cover spring potential energy, kinetic energy, total mechanical energy, conservation, friction losses, or the general −GMm/r form)
