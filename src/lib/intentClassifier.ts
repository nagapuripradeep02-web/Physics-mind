...
export const VALID_CONCEPT_IDS: ReadonlySet<string> = new Set([
    ...
    // AC map (ac_voltage_inductor/ac_voltage_capacitor now built, see below;
    // phasors/series_lcr_circuit/ac_power_factor/lc_oscillations/transformer
    ...
    // concepts — ac_voltage_capacitor (now built, see below)/phasors/
    // series_lcr_circuit/ac_power_factor/lc_oscillations/transformer follow —
    // the latter four NOT yet built, do not invent routing for them).
    'ac_voltage_inductor',
    ...
    // AC map (phasors/series_lcr_circuit/ac_power_factor/lc_oscillations/
    // transformer follow — NOT yet built, do not invent routing for them).
...
- "AC voltage on a resistor" / "resistor in an AC circuit" / "is current in phase with voltage for a resistor" / "i = v/R for AC" / "why is power never negative for a resistor" / "average current in AC is zero but the resistor still heats up" / "why doesn't an AC ammeter read zero" / "what is rms value" / "root mean square" / "Vrms = Vm over root 2" / "why is the peak voltage higher than the rated voltage" / "is 230 volts the peak or rms" / "square mean root" / "why is average power half the peak power" / "why is the average of sine squared one half" → ac_voltage_resistor
  (DISAMBIGUATION ac_voltage_resistor vs ohms_law vs ac_generator: ohms_law is the STEADY, DC-only V = IR relationship — no time-varying voltage, no rms, no phase; route plain "V=IR" / "resistance" / "why does current reduce after a resistor" questions there, not here. ac_generator is about HOW the sinusoidal EMF is produced (a coil rotating in a field, slip rings, flux-linkage phase lag) — it never mentions a resistor's OWN response, in-phase current, rms, or power; if a coil/rotation/slip-ring/flux-linkage is the subject, route to ac_generator. ac_voltage_resistor is about what happens ONCE that sinusoidal voltage is APPLIED ACROSS a resistor — in-phase current, power never negative, the zero-average paradox, and the rms/DC-equivalent rating. Sibling concepts phasors / series_lcr_circuit / ac_power_factor / lc_oscillations / transformer are NOT YET BUILT this authoring pass — never route a phase-angle/impedance/resonance/transformer-ratio question to ac_voltage_resistor; a LAG/reactance/coil question routes to ac_voltage_inductor (now built, see below); a LEAD/capacitive-reactance question routes to ac_voltage_capacitor (now built, see below); if no other Ch.7 concept is advertised in VALID_CONCEPT_IDS for that question, fall through to the nearest advertised sibling per the general unknown-concept fallback, do not invent a routing here.)
