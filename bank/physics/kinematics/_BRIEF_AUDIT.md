# Audit brief — worked solutions for the kinematics bank
...
Input:   the slice file named in your dispatch, under `bank/physics/kinematics/_audit_slices/`
Output:  `C:\Tutor\physics-mind-eapcet-corpus\bank\physics\kinematics\_audit\<audit_file>` — one file per
...
- **WEAK** — quality: register, a step with neither content nor equation, duplicated prose, a vague
  mistake entry, an off `difficulty` or `mistake_type_hint`.
...
- g = 10 m s⁻² and g = 9.8 m s⁻² are both used; the question or options tell you which. Not a
  finding unless only the other value fits.
