# Second-reader brief — an independent answer to each projectile-motion item
...
Input:   the slice file named in your dispatch (under `pdfs/books/.../projectile/_slices/`)
...
`transcripts.jsonl`, and never anything under `bank/physics/projectile/_solutions/`. Worked examples
...
1. Read the question, its options if any, and the figure description (open the crop for exercises).
   ...
   teaches: equations of motion, projectile equations, vectors, relative velocity, calculus as taught
   in Class 11–12). Never Lagrangian or Hamiltonian mechanics, Laplace transforms, Jacobians, tensors.
2. **Compute every number with python** (plain arithmetic or sympy); never in your head. Name every
   scratch file with your agent label (`W01-B-prj-03_item.py`); the scratchpad is shared.
3. Report the result:
...
```json
{"item_id": "prj_3f9a1c02", "final_option": 2, "final_value": "5 s", "method": "Vertical motion with u sin θ upward, s = −70 m, solve the quadratic", "solvable": true}
```
