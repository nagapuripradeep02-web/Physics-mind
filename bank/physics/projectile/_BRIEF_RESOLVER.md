...
Each item in your slice is a physics problem in OUR wording (Class 11, projectile motion). You solve it
...
printed answer — do not open anything under `pdfs/` or under `bank/physics/projectile/_solutions/`.
...
2. Use only Class 11–12 methods (NCERT plus JEE Main coaching techniques). Compute every number with
   python (name scratch files with your agent label, e.g. `W01-F-prj-02_item.py`).
...
```json
{"item_id": "prj_3f9a1c02", "final_option": null, "final_value": "(a) 2 s, (b) 20 m", "method": "Time of flight from the vertical motion, then range = u cos θ × t"}
```
