# Authoring brief — worked solutions for the projectile-motion bank

You write the worked solution for each of ~15 projectile-motion problems (Class 11, JEE Main level). Your
dispatch names your SLICE file. Everything else is here.

Input:   the slice file named in your dispatch (under `pdfs/books/dcp_m1/projectile/_slices/`)
Output:  `C:\Tutor\physics-mind-eapcet-corpus\bank\physics\projectile\_solutions\<item_id>.json` — one file per item
Refusal: `C:\Tutor\physics-mind-eapcet-corpus\bank\physics\projectile\_solutions\_refusals\<item_id>.json` — instead of a solution
...
2. **Use only Class 11–12 methods** — the NCERT syllabus plus what JEE Main coaching teaches
   (equations of motion, projectile equations, vectors, relative velocity, calculus as taught in Class 11–12,
...
3. **Compute every number with python** (plain arithmetic or sympy). Never do the arithmetic in your
   head. Name every scratch file with your agent label (`W01-A-prj-03_item.py`); the scratchpad is
...
```json
{
 ...
 "authored_by": {"model": "sonnet", "wave": 1, "agent": "W01-A-prj-03", "at": "2026-09-11T10:00:00+05:30"}
}
```
...
{"question_id": "prj_3f9a1c02",
 ...
 "authored_by": {"model": "sonnet", "wave": 1, "agent": "W01-A-prj-03", "at": "..."}}
```
