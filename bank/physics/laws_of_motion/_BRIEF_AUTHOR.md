# Authoring brief — worked solutions for the laws-of-motion bank

You write the worked solution for each of ~15 laws-of-motion problems (Class 11, JEE Main level). Your
dispatch names your SLICE file. Everything else is here.

Input:   the slice file named in your dispatch (under `pdfs/books/dcp_m1/laws_of_motion/_slices/`)
Output:  `C:\Tutor\physics-mind-eapcet-corpus\bank\physics\laws_of_motion\_solutions\<item_id>.json` — one file per item
Refusal: `C:\Tutor\physics-mind-eapcet-corpus\bank\physics\laws_of_motion\_solutions\_refusals\<item_id>.json` — instead of a solution
...
2. **Use only Class 11–12 methods** — the NCERT syllabus plus what JEE Main coaching teaches
   (free-body diagrams, Newton's laws, constraint equations for strings and pulleys, pseudo force in an
   accelerating frame, static and kinetic friction, equations of motion, vectors, calculus as taught in Class 11–12,
...
3. **Compute every number with python** (plain arithmetic or sympy). Never do the arithmetic in your
   head. Name every scratch file with your agent label (`W01-A-lom-03_item.py`); the scratchpad is
...
```json
{
 ...
 "authored_by": {"model": "sonnet", "wave": 1, "agent": "W01-A-lom-03", "at": "2026-09-11T10:00:00+05:30"}
}
```
...
{"question_id": "lom_3f9a1c02",
 ...
 "authored_by": {"model": "sonnet", "wave": 1, "agent": "W01-A-lom-03", "at": "..."}}
```
