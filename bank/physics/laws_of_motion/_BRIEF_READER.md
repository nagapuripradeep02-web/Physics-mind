# Reader brief — transcript, restatement and givens for the laws-of-motion bank

You read ~12 cropped page images of physics items (Class 11, laws of motion). For each one you
...
Input:   the slice file named in your dispatch (under `pdfs/books/.../laws_of_motion/_slices/`); each
         item names its `crop` image, an optional `context_crop` (a "Directions" block shared by
         several items, or a neighbouring item whose data this one depends on: a note such as "in the
         next three exercises the car accelerates at ..." or the question that "repeat the above"
         refers to) and its `out` path. Fold that data into the transcript's `question_text` in
         square brackets and into the restatement as plain givens, so the item stands alone.
...
- `figure_description`: our own description of the figure with every value on it, so the problem can be
  ...
  A blind solver reads ONLY your words, so the description must fix the geometry completely: which
  way an incline rises (left to right or right to left) and its angle; where each pulley is fixed
  (ceiling, table edge, wedge top) and whether it is movable; which block hangs and which rests on a
  surface; which surfaces are rough (with μ) and which smooth; the direction of every applied force
  (up the slope, horizontal to the right, along the string); which body a stated acceleration
  belongs to and its direction. Name the bodies as the figure labels them (A, B, m₁, m₂) so the
  question, the parts and the options refer to the same names.
- Keep the quantity the original asks for: "velocity" stays velocity (direction included), "speed"
  stays speed, "net force" is not "force", "tension in the string between A and B" names the segment.
- Assertion–reason: keep the form `"Assertion: ... Reason: ..."`, both sentences reworded, AND set
  `options` to the four standard choices in this exact order, one per line as printed in the section's
  directions: `(a)` both true and the Reason is the correct explanation of the Assertion; `(b)` both
  true but the Reason is not the correct explanation; `(c)` Assertion true, Reason false; `(d)`
  Assertion false, Reason true. Also write "(e) both false" only if the section's directions print a
  fifth choice. The solutions refer to these option letters, so an assertion–reason item without
  them is served incomplete.
