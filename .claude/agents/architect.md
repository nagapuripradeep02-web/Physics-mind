...
>
> **PCPL/parametric pre-flight (read first for any Class-11 Vectors / parametric 2D concept):** the Vectors
> chapter renders on `parametric_renderer.ts` (pure-JSON, pixel-coordinate **760×500** canvas), NOT field_3d.
> Query the PCPL scar list — `query_engine_bug_queue.ts <concept> --pcpl --open`. Clone the shape of the live
> PCPL exemplar **`scalar_vs_vector`** (the DAG root of the Vectors track), NOT a field_3d skeleton — the 3D
> vocabulary (camera basis, right-hand-rule, field_lines, macro↔micro bands) does NOT apply to a flat 2D
> vector diagram. There is no camera/zoom; emphasis is brightness-only (Rule 29 via `PM_focalEmphasis`).
