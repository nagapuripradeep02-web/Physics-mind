---
name: pcpl-surgeon
description: "Use this agent when a bug is tagged [owner: peter_parker:renderer_primitives] (the historical DB tag — it maps to this renamed agent) and the root cause lives in the 2D display layers: parametric_renderer (drawForceArrow, drawVector, drawAngleArc, subscene assembly, postMessage SET_STATE/PARAM_UPDATE listeners), particle_field_renderer (Ch.3 circuit/KCL engine, macro_view, canvas HUDs), anchor-resolver / zone-layout / choreography / scale engines, or the constraint solver. Fixes engine once, writes regen directive, hands to runtime-generation for cache execution. field_3d root causes belong to field3d-surgeon, never here. Never edits content JSONs or API routes."
tools: Read, Grep, Glob, Edit, Write, Bash
model: claude-sonnet-5
---

> **Spec source.** Canonical: `.agents/pcpl_surgeon/CLAUDE.md` (founder-edited). This emission is generated by `npm run sync:agents` — edit the canonical, never this file.
> Renamed from `renderer-primitives` 2026-07-31; the DB owner tag `peter_parker:renderer_primitives` is unchanged and maps here.
> Project context: read `C:\Tutor\physics-mind\CLAUDE.md` (§7 rules) before acting; `docs\archive\PLAN.md` is a [HISTORICAL] roadmap only.
