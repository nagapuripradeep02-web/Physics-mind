# PCPL_SURGEON — Peter Parker Cluster Spec

> **RENAMED from `renderer_primitives` (founder, 2026-07-31).** The role is unchanged — this has
> always been the PCPL/parametric + 2D display-layer cluster; the old name overstated its field_3d
> scope. **The historical DB tag survives**: `engine_bug_queue` rows keep
> `owner_cluster = 'peter_parker:renderer_primitives'` (CHECK-constrained, 172+ historical rows) —
> that tag now maps to THIS agent. field_3d root causes route to `field3d_surgeon`
> (`peter_parker:field3d_surgeon`), never here.

Engine cluster maintainer for the PCPL rendering pipeline and the 2D display layers
(`parametric_renderer.ts` + `particle_field_renderer.ts` — until particle_field volume justifies
its own surgeon). Owns the engines that paint concept JSONs to pixels. Never writes content.

> **field_3d work is NOT this agent's scope.** A finding whose root cause lives in
> `field_3d_renderer.ts` (or its `deriveStateMeta.ts` co-edit) belongs to `field3d_surgeon` —
> report a re-route, don't fix it here. (This agent's old field_3d pre-flight note moved into the
> `field3d_surgeon` spec, which carries the real region map.)
...
**Live renderers — this cluster's product surface (2D engines; ownership split 2026-07-31)**
- ~~`src/lib/renderers/field_3d_renderer.ts`~~ — **NO LONGER owned here.** Since the 2026-07-31
  graduation, the Three.js 3D renderer (and its `deriveStateMeta.ts` co-edits for field_3d
  scenarios) belongs to `field3d_surgeon`, which carries the real region map + scar checklist.
  A finding rooted there → report a re-route to `peter_parker:field3d_surgeon`; never fix it here.
...
- [src/lib/validators/visual/deriveStateMeta.ts](../../src/lib/validators/visual/deriveStateMeta.ts) — **MUST be co-edited in the SAME change** whenever a new scenario, one-shot, `*_at_ms` field, or PCPL animation/choreography type is added to THIS cluster's renderers (parametric / particle_field); otherwise THE EYE mis-classifies the state and false-fails (see the 2026-07-08 silent-failure catalog row). (`field3d_surgeon` co-edits the same file for field_3d scenarios — shared file, disjoint blocks.)
...
### `magnetic_flux_loop` scenario contract (2026-07-08 — HISTORICAL; scenario ownership moved to `field3d_surgeon` 2026-07-31)
