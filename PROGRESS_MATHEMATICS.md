...
- **⏭ NEXT SESSION — start here.** The desk is `Physics-mind-mathematics-definite-integral` (branch `feat/mathematics-definite-integral`, PR **#70** open and HELD). **Do NOT re-baseline until Checkpoint B approves** — baselines have now been approved twice on builds a live drive found broken, and two current ones certify unreachable frames.
