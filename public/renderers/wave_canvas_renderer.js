...
function WC_drawStandingWave(cfg, st) {
  ...
  // If two waves active, draw them individually with low alpha, then show standing wave
  if (waves.length >= 2) {
    // Draw individual traveling waves faintly
    for (var wi = 0; wi < 2; wi++) {
      var wave = waves[wi];
      var col = wave.color || (wi === 0 ? '#42A5F5' : '#FF9800');
      var amp = wave.amplitude * 60;
      var wl = wave.wavelength * 80;
      var freq = wave.frequency;
      var ph = wave.phase || 0;
      var dir = wave.direction === 'left' ? 1 : -1;
      WC_drawSineWave(40, yCenter, amp, wl, ph + dir * WC_time * freq * TWO_PI, col, '', 0.3);
    }
  }

  // Draw standing wave envelope + instantaneous shape
  if (waves.length > 0) {
    var sw = waves[0];
    var swAmp = sw.amplitude * 60;
    var swWl = sw.wavelength * 80;
    var swFreq = sw.frequency;

    // Envelope (max amplitude at each point)
    WC_stroke('#66BB6A', 0.25);
    strokeWeight(1);
    noFill();
    beginShape();
    for (var ex = 40; ex <= w - 40; ex += 2) {
      var envY = swAmp * abs(sin((TWO_PI / swWl) * (ex - 40)));
      vertex(ex, yCenter - envY);
    }
    endShape();
    beginShape();
    for (var ex2 = 40; ex2 <= w - 40; ex2 += 2) {
      var envY2 = swAmp * abs(sin((TWO_PI / swWl) * (ex2 - 40)));
      vertex(ex2, yCenter + envY2);
    }
    endShape();
    ...
    for (var sx = 40; sx <= w - 40; sx += 2) {
      ...
      var sy = yCenter + swAmp * spatialPart * temporalPart;
      vertex(sx, sy);
    }
    endShape();

    // Nodes and antinodes
    if (cfg.show_node_antinode) {
      WC_drawNodeAntinodeMarkers(swWl, w, WC_CH, yCenter, swAmp);
    }
  }
}
...
function WC_drawDopplerEffect(cfg, st) {
  ...
  if (WC_state === 'STATE_3' || WC_state === 'STATE_4') {
    ...
    textSize(11);
    textAlign(CENTER, CENTER);
    text('v\u2098 (wave speed) = same in both directions!', WC_CW / 2, arrowY - 45);
  ...
  text("f' = f\u2080(v \u00B1 v\u2092) / (v \u2213 v\u209B)", WC_CW / 2, WC_CH - 20);
}
