...
// Standalone version for public/renderers/
...
function updatePlotly(stateId) {
    ...
    return Plotly.restyle('plotly-panel', { x: [allX], y: [allY] }, [0]).then(function() {
...
function drawGasContainer() {
    ...
    for (var i = 0; i < TD_particles.length; i++) {
        ...
        if (p.x > boxLeft + boxW - 6) { p.x = boxLeft + boxW - 6; p.vx *= -1; }
