...
const files = fs.readdirSync(dir).filter((f) => f.startsWith('ts_ipe_z1_lr_') && f.endsWith('.json'));
let bad = 0;
for (const f of files) {
  ...
  const noIn = raw.insider_note ? 0 : 1;
  const nvt = raw.verification?.needs_teacher_verification === true ? 0 : 1;
  if (noWhy + noCm + noMn + noMt + noMg + noIn + nvt + longest.length) bad++;
  console.log('OK  ', f, '·', raw.qtype, raw.marks_total + 'M · steps', raw.answer.steps.length,
    `· gaps why:${noWhy} cm:${noCm} mark_note:${noMn} memory_tip:${noMt} margin_note:${noMg} insider:${noIn} nvt:${nvt}`,
    longest.length ? `· LONG LINES ${longest.length}` : '');
  longest.forEach((l) => console.log('     >52  ', l));
}
console.log(bad ? `${bad} file(s) with a problem` : `all ${files.length} valid and complete`);
