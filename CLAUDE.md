...
constants neutral or parameterized)** · **Rule 38 (depth rings core|extended|advanced with a coherent-when-cut check; explore state surfaces CORE-ring content only; algebra-only formula surfaces outside the advanced ring; cross-board dialect + decided graph axes; `curriculum_tags` authored as claims with `needs_teacher_verification` on every unverified cell)** · **Rule 39 (a new scenario with DOM overlay widgets declares them in SIM_READY + routes display through the widget-vis resolver + reports WIDGET_VIS_STATE + supports WIDGET_PING)** · plain English · **THE EYE** (§5 ③) with zero new `engine_bug_queue` rows.
