...
40. **[A]** **Engine files are PLATFORM, not chapter work (2026-07-26).** `parametric_renderer.ts`, `field_3d_renderer.ts`, `particle_field_renderer.ts`, `premium_primitives.ts`, `deriveStateMeta.ts`, `build_review_site.ts` and the `validators/visual/*` tooling are shared by every chapter and BOTH subjects (physics + chemistry). Land a change to them on master **separately and immediately** — never bundled inside a chapter branch, where it stays invisible until merge. (40a) Before adding any engine-level mechanism, run `git fetch origin && git log --all -S "<symbol>" --oneline` to confirm it does not already exist. (40b) This is the JUDGMENT half and is enforced by nothing automatic — the mechanical half (verification, backup) is now GitHub Actions `verify.yml` + the `.githooks/` auto-push hooks (`docs/GIT_WORKFLOW.md`). Do NOT add rules for anything CI already checks; duplicating machine-checkable rules here dilutes the ones that need a human. Origin: the PCPL focal-glow fix (`PM_focalEmphasis` vs `PM_focalPulseBoost`) and the parametric player clock were each built TWICE, independently, in one week — on branches neither side had pushed. (CLAUDE_RULES.md)
41. **[A]** **Plain-language law — every word a teacher or student can read is basic, literal English (founder 2026-07-30; friction_force/normal_force review).** Scope: EVERY reader-facing string — concept names, state titles (the rail), delta cues, on-canvas captions, bottom-left state labels, narration/subtitles (`text_en` and every language), HUD/slider labels, annotations, feedback prompts. (41a) No idioms, no metaphors, no personification: never "fate", "grip", "ceiling" (say "maximum"/"limit"), "answers/asks/wins/lets go/gives back/budges/rides on/lurches", "All yours". Forces do not want, know, answer, or care. (41b) Physics vocabulary is NOT jargon — "static friction", "kinetic", "normal force", "maximum static friction μₛN" are the plain words; use the word the formula uses ("friction increases to match the push", never "friction finds the answer"). (41c) The test: a Class-11 student with textbook English (English as a second language) must understand every word without asking; if a word needs explaining and is not a physics term, replace it. (41d) State titles short and literal — the rail truncates, so the FIRST words must carry the meaning. (41f) **ONE exception, TEMPORARY and narrow (founder, 2026-09-19): Vidi's chat replies on the MATHS papers may use ONE everyday comparison per reply** — one sentence, something an average student has really handled, then straight back to the mathematics. It exists because the founder asked for analogies while tuning the drag-to-ask replies ("use analogy with simple things if needed but make it simpler to understand"), and it is what produced lines like "think of it like cancelling before you multiply fractions — you do the cheap move first so the big move stays small". Everything else in 41a still holds inside that reply: no idioms, no personification, no naming a step with a figure of speech. Scope today is the four maths papers only (`MATHS_STYLE_PAPERS` in both chat servers); the sims, every authored string, and every other subject's replies stay under the full ban. **The founder expects to loosen this and extend it to the other subjects later — treat it as a live experiment, not settled doctrine, and do not "fix" a comparison out of a maths reply on the strength of 41a.** (41g) **SECOND exception, same experimental footing (founder, 2026-09-21): Vidi's chat replies may carry ONE small joke.** One sentence, literal, the size of a passing remark, then straight back to the answer. NEVER when the student is stuck, worried, short of time, or has just got something wrong — those get a plain, warm reply; never about the student, their mistake, their teacher or the marks they could lose; never as a sign-off. It lives in PERSONA in BOTH chat servers, and it is taught by example there because the permission alone did nothing: measured over 6 probes with only the rule present, the model made zero jokes; with the worked example added ("five times is more than the examiner will ever ask for" is the right size, "at this point the proof owes you rent" is too much) it fires correctly and still stays silent on the worried and stuck probes. Scope is Vidi's REPLIES only — every authored string, the sims and all four papers' content stay under the full 41a ban. Like 41f this is a live experiment, not settled doctrine: **do not "fix" a joke out of a Vidi reply on the strength of 41a.** (41e) Relation to other rules: 24/34 govern how MUCH text; 35 bans cultural localisms; 41 bans literary/idiomatic register — all three apply together. Enforced at: architect (titles + delta cues), physics-author (narration), quality-auditor (plain-language probe over every rendered string). (CLAUDE_RULES.md)
