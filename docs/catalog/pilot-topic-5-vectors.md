...
> **Retro-catalog, authored 2026-06-13 (Session 67).** This page was reconstructed AFTER the fact from
> the 19 vector simulations already shipped in `src/data/concepts/`. T5 was the catalog's single largest
...
## Build status (this chapter is ALREADY SHIPPED)

All 19 concepts below are **built and registered** (`src/data/concepts/<id>.json`, all 19 ids in
`intentClassifier.ts` `VALID_CONCEPT_IDS`). **Disk id = catalog id (1:1)** — no aliasing needed for T5
(unlike the kinematics/forces chapters; see proposal D2). The four legacy bundle names
`vector_basics → unit_vector`, `vector_addition → resultant_formula`, `vector_components → vector_resolution`,
`scalar_vs_vector → current_not_vector` redirect via `CONCEPT_SYNONYMS` and are NOT separate concepts.
...
| **V-G7** | **Nano convention note:** the 5 nanos here were each shipped as independent multi-state sims (3–4 states) BEFORE the "nano = 2 states" convention; they are retained as independently-launchable and marked nano by ROLE (sub-concept of a parent), not by state count. |
...
| ↳ special_cases | The three shortcuts: θ=0° → R = A+B; θ=90° → R = √(A²+B²); θ=180° → R = \|A−B\|. | nano | ✅ | ✅ | [resultant_formula] | — | parent: resultant_formula; built as a fuller sim (V-G7) |
| ↳ range_inequality | The resultant is bounded: \|A−B\| ≤ R ≤ A+B. Maximum when parallel, minimum when antiparallel. | nano | ✅ | ✅ | [resultant_formula] | — | parent: resultant_formula; built as a fuller sim (V-G7) |
| ↳ direction_of_resultant | The resultant's direction: tan α = B sin θ / (A + B cos θ), measured from A. | nano | ✅ | ✅ | [resultant_formula] | — | parent: resultant_formula; built as a fuller sim (V-G7) |
...
| ↳ inclined_plane_components | Resolving weight on a slope: mg sin θ along the incline, mg cos θ perpendicular. The canonical resolution application. | nano | ✅ | ✅ | [vector_resolution] | [block_on_incline(T11), friction_block_on_inclined_plane(T12)] | parent: vector_resolution; built as a fuller sim (V-G7) |
...
**All 19 are already shipped** — V1 flags here are retrospective priority notes, not a build queue.
...
2. **Nano state-count vs convention.** The 5 nanos were each shipped as 3–4-state sims before the
...
- Atomic count: **14**. Nano count: **5**. Total: **19 entries** — all already shipped in `src/data/concepts/`.
- Disk id = catalog id (1:1) — first slice of the proposal-D2 alias map.
...
*The catalog's biggest hole was the chapter we finished first. T5 = a DAG root with 14 atomics + 5 nanos,
all shipped; cataloguing it resolves ~30 downstream prerequisite edges. One real gap remains: the cross
product.*
