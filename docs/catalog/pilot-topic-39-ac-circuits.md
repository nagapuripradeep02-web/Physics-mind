...
| **lcr_series_circuit_atomic** | Impedance Z = √(R² + (X_L − X_C)²); phase φ = arctan((X_L − X_C)/R); I = V₀/Z | atomic | ✅ | — | [ac_resistor_circuit, ac_inductor_circuit, ac_capacitor_circuit, phasor_representation_atomic] | [resonance_in_lcr_atomic, power_in_ac_circuit_atomic, impedance_triangle_nano] | Combines all 3 previous atomics via vector addition of voltage phasors. **Cycle break (2026-06-13):** removed `impedance_triangle_nano` from Requires — a nano cannot precede its parent atomic; the triangle is DERIVED from the LCR result (now in Required-by). |
...
| **transformer_in_ac_atomic** | **AC-grid extension of `transformer_atomic` (T35)** — the core V_s/V_p = N_s/N_p turn-ratio principle is owned by T35; THIS row adds only the AC-specific physics: soft-iron core minimises hysteresis loss, lamination cuts eddy losses, and step-up/step-down in the 50 Hz grid context. | atomic | ✅ | — | [transformer_atomic(T35), hysteresis_loop_atomic(T37), soft_vs_hard_magnetic_materials_nano(T37)] | [transformer_losses_nano, ac_traction_application_nano] | AC-G7; **closes T37 hysteresis forward-edge**. **De-dup 2026-06-13:** was a near-duplicate of T35 `transformer_atomic`; re-scoped to the AC extension (depends on T35, no longer restates the turn-ratio principle). Kept as a thin atomic — NOT demoted to nano because it owns 3 child nanos (losses / step-up-down / traction); demotion would create nano-under-nano. |
