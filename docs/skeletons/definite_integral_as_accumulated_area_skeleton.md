# ARCHITECT SKELETON — `definite_integral_as_accumulated_area` — **AMENDMENT ROUND 2 (FINAL — Checkpoint A cycle 1 applied; cycle budget EXHAUSTED → ESCALATES TO FOUNDER)**
...
**What the founder must decide (genuinely open after this round):**
- **F2 ruling (mine, reviewable):** S4's one formula surface shows the EXACT `gap = 4/n − 4/(3n²)` rather than `gap ≈ 4/n`. Argument: the state's claim is the size of the gap; the exact form makes "never zero" provable and keeps the 6-dp HUD consistent with the surface at every n; an `≈` on the error formula feeds M1. The cost: a slightly heavier surface on a core state. Overriding to `≈ 4/n` is a one-line change plus a §10g HUD-consistency note.
- **F9 approach (mine, reviewable):** the narration constraint is now published per state as `words_max = ⌊2.5 × motion_window⌋` (the Rule-31 band floor), with S2's motion lengthened to fit its budget — chosen over lengthening every window to absorb a 55-word worst case. If the founder prefers headroom for 55-word states everywhere, every guided state needs a ≥ 22 s motion window.
- **F17 argument (mine, reviewable):** `Σ` / `xᵢ` stay on S2's core surface `Sₙ = Σ f(xᵢ)·h`, argued as read-off-the-drawing (38c) with ledger rows added. The fallback is the wordier `Sₙ = sum of f(xᵢ)·h`.
- **Contract questions routed to CP dispatches (decided there, not here):** the F11 tracking-label combination contract (CP-A) · the F5 plot_point seizure clause + Gate 9(d) seizable-union extension (CP-B) · the F6 publication scope map (CP-C). Listed in §13 "Dispatch scope additions".

*Handoff: **ESCALATED TO FOUNDER** — Checkpoint A's cycle budget (2) is exhausted; this document does not return to founder_proxy. Not handoff-ready to `mathematics_author` until the founder rules, and not buildable until CP-A…CP-D land on master.*
