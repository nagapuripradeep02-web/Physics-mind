...
>    — **NOT** this document's `{at_ms, ramp_ms, mode:"once"|"ping_pong"}`. There is **no `ping_pong`**.
...
| M2 | "`n·d = 0` means the line is perpendicular to the plane" (the normal is confused for the plane itself) | S4 | The cyan `n·d = 0` line is shown doing what that condition actually forces: it glides past the patch, parallel, and never touches — with the readout pinned at `0.000` the whole time. The amber line that DOES cut through has ~~`n·d = 0.624`~~ **⚠ AMENDED: `n·d = 0.574`** (the HUD prints the normalized cosine), plainly not zero. `one_line_fix`: "`n·d = 0` means the direction is perpendicular to the NORMAL, which puts the line parallel to the plane." |
...
| the test value | `n·d` | HUD row (live) — **`0.000` for the parallel line, ~~`0.624`~~ ⚠ AMENDED **`0.574`** for the cutting line** | S4 | S4, S7, S9 |
...
| S2 | 18 s | 0–4000 the patch unfolds (`half_extent` 0.4→3.0) with the normal `n` up from the start; 6500–10500 the test vector **`v`** sweeps ACROSS the patch and `n·v` holds 0; **11150 `v` hands off IN PLACE** (`grow_ms: 0`, 50 ms before its predecessor hides, so the picture never goes empty); 11200–14000 it tips OUT of the plane and `n·v` leaves zero; hold *(pacing swept against the shipped JSON 2026-08-20 — 5 of 9 rows drifted; this is one)* — the old cell described a breathing patch that is not authored and omitted the `n·v` demonstration entirely, which is the state's whole lesson | 3 s (17 %) | `eye_capture_ms: 12000` → normal + full patch | 5500 |
| S3 | 24 s | 0–400 `q` and the sweep foot appear; **2000–7000 the foot sweeps `s = −2.2 → +1.6` along ONE straight path, readout `3.11` → dips to `2.20` at `s = 0` (≈ t 5100) → rises to `2.72`**; 7000–9000 the foot returns to `s = 0` and the readout falls `2.72 → 2.20`; **8950 the segment LOCKS GREEN IN PLACE** — instant (`grow_ms: 0`) and 50 ms before `cmp` hides at 9000, so it recolours rather than vanishing and regrowing; its right-angle mark lands on the same beat *(pacing swept against the shipped JSON 2026-08-20 — 5 of 9 rows drifted; this is one)* *(was 9000–9600 grow + 9600–11000 mark: `hide_at_ms` equalled `reveal_at_ms`, and `vgRevealFrac` returns 0 for BOTH at that instant, so at t=9000 the aha state showed no segment and an empty HUD)*; 11000–12500 11000–12500 formula surface writes; hold | 4 s (17 %) | `eye_capture_ms: 16000` → green perpendicular locked, `2.20`, right-angle mark, surface | **3500** after the last reveal ✓ |
| S4 | 24 s | 0–2000 patch + parallel line enter; **2000–8000 the `n·d = 0` line slides over and past — no marker ever**; 8000–9500 it retires; 9500–15000 the cutting line arrives and punches through; 15000–15600 marker snaps on at `X`; hold | 3.5 s (15 %) | `eye_capture_ms: 18000` → marker present, ~~`n·d = 0.624`~~ **⚠ AMENDED `n·d = 0.574`**, `λ = 2.600` (λ correct as authored — scale-invariant) | 2400 ✓ |
| S5 | 26 s | 0–2000 both lines draw **and `shortest distance = 1.80` is already live**; **2000–3500 the crossing marker pulses — 1.5 s, and the contradicting number is on screen for every frame of it**; 3500 marker removed; **0–1800 the green common perpendicular grows between the true feet — it is up BEFORE the marker, not after** *(pacing swept against the shipped JSON 2026-08-20 — 5 of 9 rows drifted; this is one)*, and that is FORCED, not a slip: since PR #93 a readout waits for its subject to ARRIVE, so `skew_distance` can only be live at ~1620 (the whole point of the counter-timing fix) if the geometry reveals at 0. Restoring the old 3800–6000 ordering would silently move the number off the misconception beat and re-open a closed CRITICAL; 7500–16500 **`camera_steps` (F24)** eases to the S8 pose; hold | 3 s (12 %) | `eye_capture_ms: 16500` → common perpendicular + `1.80`, no crossing marker, camera arrived | **10500** after the last reveal ✓. **`camera_steps` is closed-form on state-local ms, so the pin reproduces byte-identically and the DoD MAY now assert the camera arrival** — ROUND 0 could not, and had to raise a FLAG |
| S6 | 22 s | 0–2000 directions re-drawn from one origin; 2000–8000 anchors slide, arc HOLDS at 69.4°; 8000–19000 `d₂` rotates 69.4°→115° (amended 2026-08-10 — the third stale cell; two were caught earlier, this one by the pass-3 audit), arc tracks; hold | 2 s (9 %) | 13200 → arc mid-rotation with its live readout | 5200 |
| S7 | 22 s | 0–1800 **`camera_steps` eases from the shared home pose to S7's own pose (az 140 / el 26)** as the cutting line returns; 1800–2000 settle; 2000–7000 it splits into shadow + normal component; 9000–10200 arc to normal (55.0°, **radius 0.62**); 13000–14500 arc to shadow (35.0°, **radius 0.95**) and the sum row fills with it *(pacing swept against the shipped JSON 2026-08-20 — 5 of 9 rows drifted; this is one)* — **the radii are STEPPED as of 2026-08-20**: at the shared default 0.9 the two arcs were contiguous at one apex and read as a single unbroken ~90° curve, so the sum the state exists to teach could not be seen as two parts, and the inner arc also overran the drawn normal arm (~208 px arc vs ~180 px arm); hold | 4 s (18 %) | `eye_capture_ms: 16000` → both arcs, both readouts, `35.0 + 55.0 = 90.0` | 2000 ✓ |
| S8 | 24 s | 0–2000 S5 scene returns with the common perpendicular already present; 2000–4000 `d₁ × d₂` builds at the origin *(pacing swept against the shipped JSON 2026-08-20 — 5 of 9 rows drifted; this is one)*; 7000–12000 it translates onto the common perpendicular and coincides; 12000–15000 formula surface + three HUD terms; hold | 4 s (17 %) | `eye_capture_ms: 17500` → the two overlaid, surface, `1.685 / 0.936 / 1.80` | 2500 ✓ |
...
| S6 | `theta_deg` 69.4 → 115 (amended 2026-08-10; slider floor stays 25); each anchor's slide | **NO** |
