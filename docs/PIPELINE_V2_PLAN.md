...
| **4** | **Exemplar rubric** — distill WHY `faraday_law_induction` / `resistivity` / `magnetisation_and_intensity` are excellent into a scored sheet → becomes founder-proxy's Checkpoint A/B scoring rubric. Every founder rejection thereafter feeds it (the ceiling counterpart of the scar list). | **UNRATIFIED DRAFT 2026-08-01** — `docs/EXEMPLAR_RUBRIC.md` v0.1, **governs nothing** | Agent-drafted, founder has NOT reviewed it. Its §1 (measured exemplar structure) / §2 (6 founder rejections quoted from their fix commits) / §4 (the fence: all three exemplars predate Rules 31/38/41 — do not clone their titles or narration) are evidence and usable now. Its §3 scored sheet is a PROPOSAL: dimensions arguable, **thresholds invented with zero founder input**. `.agents/founder_proxy/CLAUDE.md` is untouched — nothing scores against it. 4 taste calls close it (§6 there): thresholds + weighting, is the exemplar set still current, retrofit `resistivity`'s Rule-41 titles?, does a 0 block at Checkpoint A? |
