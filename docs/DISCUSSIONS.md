...
**1. The 2026-27 board landscape, researched and verdicted (~15 sources; full detail in `docs/patterns/answer_book.md` §Board landscape).** TS and AP stopped being variants of one exam. **TS:** the written Physics theory paper is EXPLICITLY unchanged at 60M (A 10×2 compulsory · B 6-of-8×4 · C 2-of-3×8) — March 2026 ran exactly this; the 2026-27 reform adds 15M first-year practicals *outside* the written paper (the old 30M second-year practicals split 15+15, external), and "60+15 ABL" is the MATHS formula (75→60 written + 15 activity marks), not Physics. Second-year reforms land 2027-28. The real TS exposure is the SYLLABUS: NCERT-aligned textbooks for 2026-27 (new AI-in-Physics chapter, non-JEE/NEET cuts) → per-unit inventory diff when they publish. **AP:** already switched (March 2026, Yr 1): 85M, four sections — A 9×1 and B 14×2 both fully compulsory, C 8-of-12×4, D 2-of-3×8 — NCERT syllabus, CBSE-style; Maths 1A+1B merged into ONE 100M paper (was 75+75), Botany+Zoology merged into one Biology paper, pass marks raised. Yr 2 follows in March 2027. **The finding that matters most: the AP 2026 paper asked OUR authored questions** (inertia B-13, KE–momentum B-14, friction methods C-26, avg/inst velocity C-24, door torque B-15, vector product C-27) — a full pattern + syllabus change did not change the questions, only their packaging. One NCERT bank, different lengths → exactly what `cuts[]` + the `board` field model.

**2. The marks arithmetic (the product's value proposition, quantified).** TS: Sections B+C = 40M with generous choice (dodge 2-of-8 SAQs, 1-of-3 LAQs) → full-bank mastery ≈ 38-40 choice-protected marks; the full VSAQ bank historically covers most of Section A's 20 → honest total ~50-56/60. AP: C+D = 48 choice-protected (dodge 4-of-12!), but A+B = 37 marks with ZERO choice → the complete VSAQ/2M bank is load-bearing in AP, optional garnish in TS. The founder's "guaranteed 45" is conservative for the choice sections; the caveats: first-year-of-new-syllabus bank-draw risk (absorbed by choice, resolved by TGBIE model papers), knowing ≠ producing (the checking loop is the enforcement mechanism of the guarantee), and mark splits stay claims until the teacher pass.

**3. The three-layer sourcing doctrine (replaces "trust the Fastrack stars").** Layer 1 — the textbook's enumerable derivations (an 8M question MUST be a derivation and each chapter has 3-6; deterministic, available now; our authored units are ~all this layer). Layer 2 — the boards' own voice (model papers + each real paper; AP March 2026 is data point #1 of the new era, `appearances[]` is its home). Layer 3 — the old star ranks as a discounted prior, never a fact (they compile 2004-2014 frequency under the old syllabus). The first post-reform guide-book editions will themselves be guesses; a digital bank updates the week model papers drop — the disruption is an opening, not a threat.

**4. The enumeration thesis — the VSAQ/SAQ space per chapter is CLOSED and countable.** A 2M answer must fit 2-4 lines, so every VSAQ is one of ~8 archetypes (define · state-the-law · explain-with-law · yes/no+reason · what-if/special-case · relation-between-two · 2-step numerical · condition/comparison) applied to a chapter's finite object inventory (~15-20 objects) → ~30-40 askable VSAQs per chapter, of which the books carry the asked core. SAQs enumerate too (~10-15/chapter, 8 archetypes incl. sub-derivations and distinguish-tables); the numerical leak closes at the formula level (boards lift numericals from textbook examples). **Trustability is back-tested, not asserted**: every question actually asked (2019-2026, both boards) must fall inside the enumerated grid, else the archetype set gets fixed. Honest ceiling ~95% — "every question of the last N years plus the full predicted space", never "impossible to be surprised". Empirical support this session: the two source books are NOT supersets of each other even within one unit (Unit 7: BLM has 2 VSAQs the Fastrack lacks; Fastrack has 6 SAQs the BLM lacks) — the union is the starting bank and enumeration extends past the union.

**5. Tiered authoring resolves the "he could just study the textbook" paradox.** Full enumeration ≈ 46 answers/chapter ≈ 600/year — comparable ITEM count to the textbook, but a different unit of study: the bank item IS the deliverable (answer + marks + figure), the textbook is prose the student must convert himself. Enumeration creates a BOUNDARY, not more homework: Tier 1 (asked, star-ranked core ≈ the old Fastrack size) = the crammer's 40-45-mark path; the full map = the first honest basis for a 60/60 promise ("there is nothing else this chapter can ask you at 2 or 4 marks" — a sentence no textbook or guide has ever been able to say). The same tiering applies to OUR cost: full cards for the asked core, LEAN cards for the predicted tail (steps+marks+short rubric, no figures), promoted only when a paper season or the teacher pass validates the cell.

**6. The term-pack design (the year-long companion).** IPE students sit unit tests → quarterly (late Sept) → half-yearly (Dec) → pre-finals → IPE (March); every internal exam is the board paper over a smaller chapter window. Student declares `{exam_date, chapters[]}` (a two-tap picker; a chat shell can parse the same later) → deterministic filter of the bank → term practice paper in board format, day-by-day countdown plan, readiness meter scoped to the term. AI boundaries fixed: explain-a-step is DeepSeek grounded in the authored `why`/`common_mistakes` (never composes physics — Rule 18 as the trust position, "every answer human-verified, AI only checks YOU"); memory tips are AUTHORED and measured (which tip preceded a passed recall), never improvised live. Chatbot = doorman, never the building: the offline single-file product must work fully without it. Cost basis already proven: DeepSeek ₹0.045/check (₹0.014 cached), Web Speech free — a ₹299-499/yr price carries a heavy user.

**7. TS/AP differentiation (founder decision this session): board tags + badges, not a full AP edition yet.** `appearances[]` gains an optional `board` field; every known hit filled (TS years from the Fastrack's printed citations, the six AP-2026 hits); cards show "Asked: TS 2011 · AP 2026" and enumerated entries a "Predicted — not asked yet" chip. The full AP edition (a qtype for the 1M objective Section A, AP cuts, board toggle) is a separate founder go/no-go after AP model papers settle — scoped in the pattern doc as re-heading + re-cutting over the same step lists, plus the one genuinely new build (a Section-A recall drill).

**8. Pan-India (context for the asset's worth, not this year's work).** Nearly all boards' science syllabi have converged or are converging on NCERT (Kerala/Karnataka/Gujarat/Rajasthan/MP/Bihar/UP-belt; TS was among the last holdouts); own-syllabus outliers: TN (Samacheer Kalvi), Maharashtra (Balbharati), WB (semester system — a different exam species), Odisha. The axis that gates OUR product is who examines Class 11 publicly: AP, TS, Kerala, Karnataka, TN, WB, J&K, Jharkhand. Expansion ladder: AP → Kerala/Karnataka (NCERT + Yr-1 public exam + answer-writing culture; Rule-35 culture-neutral content needs zero rework) → TN/Maharashtra later (own textbooks = content re-verification) → the Hindi belt only as a second-year/Hindi play. The authored step-list bank is potentially a 6-8-state asset with marginal cost per NCERT state ≈ headers + cuts + inventory ranks.

**9. Product ladder beyond authoring (ranked, founder-facing):** (1) "Asked in…" tags (this session); (2) the retrieval loop hardened (Learn→Hide→Attempt→check + re-ping); (3) the model-paper simulator (timed 6-of-8 / 2-of-3 choosing practice); (4) AP edition + Section-A drill; (5) second-year book (old pattern valid through March 2027; ch.4-10 = the 61-sim block, answer cards deep-link into sims); (6) Maths Answer Book (strongest top-N-LAQ culture; TS Maths just lost 15 written marks); (7) practical-record helper (the new 15+15M — a brand-new anxiety with zero guide coverage); (8) Telugu-medium answers. Sequencing: 1-2 now, 3 at breadth, 4-6 founder calls, 7-8 parked.

## Session 88 — The first real student review reframes the student product: crammers, not learners; the Exam Notebook; voice latency engineered away (2026-08-19)

**1. Context — the trigger was a real student, the first outside-founder user of Quick Learn.** A student who has finished intermediate and is entering B.Sc. first year used the Quick Learn lesson and reported: *"I'm able to understand the concept, but it's really boring — it's taking a lot of time where speech is produced."* Two findings in one sentence, and they route differently: the **learning worked** ("I understand"), while the **packaging and pacing did not**. The founder's first instinct was a cost/latency swap (replace Sarvam `bulbul:v3` with `bulbul:v2`, which is half price). That instinct was half right and is recorded in §2; the deeper half of the review opened the strategy question in §3, which is the substantive output of this session. Full design document: `docs/QUICK_LEARN_EXAM_STRATEGY.md` (written this session, PROPOSED status). Session log: `PROGRESS.md` 2026-08-19.

**2. Latency: diagnosed, then engineered away — and the model swap was NOT the fix.** Measured chain per spoken answer: DeepSeek non-streaming (~2.5 s) → the whole message synthesized as ONE Sarvam REST call (~3–5 s) → returned as **base64 WAV at ~57 KB per spoken second** (a 30 s answer ≈ 1.7 MB download before the first word plays) → played. Nothing overlapped. **The founder's proposed v2 swap changes the price tag, not the process** — the wait is caused by whole-clip synthesis, oversized payloads and a fully serial chain, so switching model alone would have left the student waiting roughly as long, at half the cost. Two fixes shipped instead (commits `347db1a`, `b5911b4`): (a) **every scripted line is pre-rendered at build time** — 79 lines across the 3 lessons (welcome, mission instructions, hints, celebrations, replay prompts) synthesized once into committed mp3 clips the page plays instantly, leaving only free-form questions on the live path; (b) **the live path now returns mp3**, ~⅓ the bytes. Deferred by decision: sentence-chunked TTS, streamed DeepSeek, and the v2 switch — **`bulbul:v2` has no `priya` speaker** (its voices are anushka/manisha/vidya/arya), so adopting it is a product-voice change requiring a by-ear founder decision across both the clip and live paths, not a config edit. **Probe finding worth carrying to any Sarvam work:** the REST API **silently ignores `audio_format`** (the worker had been sending `'wav'` to no effect); the honoured parameter is **`output_audio_codec`**.

**3. The market question the founder put directly: do intermediate students in Telangana/AP actually want simulations?** Founder's framing, unprompted and self-critical: of ~20 lakh TS/AP intermediate students per year, perhaps **0.5% (5,000–10,000)** genuinely want to understand a concept via AI + simulation. The majority "play around the whole semester" and study only in the last month or last week — for **IPE** and college internals (quarterly/half-yearly), not EAMCET or JEE. A minority prepare seriously from the start of college. His question: how do we pull *all* these students, become famous in two states first, and make the product something students feel dependent on?

**4. Position taken: students do not buy understanding — they buy marks, rank, and relief from fear.** The 0.5% learner segment is a real beachhead (10k students is viable revenue), but a product framed as *"understand the concept"* stays niche permanently. **The move is NOT to convert crammers into learners** — no product has ever achieved that, and trying is how ed-tech burns money. **The move is to serve the cramming moment and smuggle the understanding inside it.** Three segments, honestly sized: (A) JEE/EAMCET grinders ~5–10%, already owned by Sri Chaitanya/Narayana/Aakash material overload — entered through their unserved gap (the 11 pm doubt, the concept coaching rote-fed but never made click); (B) **IPE last-minute crammers — the majority**, active 1–4 weeks before an exam; (C) internal-exam-only, same behaviour in smaller spikes. **The wedge is that IPE is famously predictable**: papers repeat patterns, and every student already hunts "important questions" PDFs. The same sims therefore go behind an exam door — *"the 7 questions that come every year from this chapter, each explained in 2 minutes with a moving picture, plus the exact answer format that gets full marks."* Note the mechanism already exists: `pyq_compare` (built 2026-08-15) is this exact trap-then-truth pattern for JEE; the IPE 8/4/2-mark bank is the same pattern at ~50× the audience.

**5. The Exam Notebook — the founder's product concept, jointly designed this session.** A student uploads a photo of (or picks) a question. A **ruled-notebook UI** with a handwriting font renders the model answer **writing itself line by line**, while **marks accumulate in the left margin like a teacher's red pen** (+1 state the law, +1 write the formula, +2 complete the derivation, +1 diagram → 8/8). If the question demands a diagram it **draws itself stroke by stroke**, synchronized with the step that earns its mark. Any line is clickable for an explanation plus a memory tip, and each step deep-links into the concept sim that shows *why* it is true — the smuggled understanding. Founder's stated goal for the feature: the student "feels confident I can write the exam tomorrow morning."

**6. The architectural inversion that makes it buildable — the bank, not the brain.** The founder's central worry was *"how does the AI always know the 8/4/2-mark format and the step-by-step answer for any random question?"* **Answer: it cannot do so reliably, and it does not need to.** A live LLM writing mark schemes will be wrong some of the time, and a crammer who memorizes a wrong answer the night before IPE never trusts the product again — so Rule 17/18 is the *design* here, not a constraint on it. **The IPE corpus is finite and repetitive: roughly ~400 entries (~50 LAQ 8-mark, ~150 SAQ 4-mark, ~200 VSAQ 2-mark) across Class 11+12 Physics**, with marking steps already encoded in BIE schemes and every Vikram/Deepthi-style guide. Each entry is authored ONCE as structured step-JSON (marks, type, memory tip, diagram ref, sim link), agent-drafted and human-verified — the existing pipeline doctrine. Serving becomes recognition and retrieval: **vision extract (Gemini Flash, already in stack) → pgvector match (already in stack) → serve the verified entry**; a miss returns "we'll add this" and **enters the authoring queue as demand telemetry**, so the bank grows exactly where students actually ask. Cost of a hit ≈ one embedding plus an occasional vision call. This is the same pre-solve inversion already priced on 2026-08-18 (uncapped photo-solve ₹350–1,164/student/month vs pre-solving the finite corpus ≈ ₹400 one-time). Verified entries carry a ✅ teacher-verified badge; anything live-generated must wear an explicit "AI draft — not verified" badge, because the crammer's trust is the entire asset.

**7. Diagrams: drawn live on screen, never invented live.** The founder asked whether diagrams could be drawn in real time on the paper or must be pre-drawn in the database. **Both, correctly split: real-time *drawing* (the experience) always; real-time *inventing* (AI deciding the shape at view time) never.** Each diagram is stored as an ordered stroke list (SVG paths) authored offline — agent drafts, human approves once, frozen — and the browser's `stroke-dashoffset` animation reveals each path end-to-end at pen speed, which is visually indistinguishable from a hand sketching it, deterministic, pixel-perfect and ₹0 at serve time. Because strokes are data rather than an image, the diagram stays alive: the force-arrow stroke draws at the exact moment its "+1" step writes, clicking the θ arc explains just that part, and **the stroke ORDER itself becomes a memory aid**. Library size is small — IPE questions massively share diagrams (AC generator, cyclotron, meter bridge, Young's double slit, transformer, galvanometer), so **~100–150 diagrams cover the entire two-year bank**. Any out-of-bank fallback should have the model *place* shapes from a reviewed primitive vocabulary rather than freehand SVG — conductor, not composer, the same pattern as the voice professor.

**8. The differentiation question the founder raised himself: "the solutions are already in the Sri Chaitanya / Narayana books — why use our AI?"** **If the product is the guide's answer on a screen, it loses** — the guide is ₹200, offline, lecturer-endorsed and already on the desk. The product must be what paper physically cannot do: (i) **it checks whether the student can reproduce the answer** rather than merely showing it; (ii) it answers "why?" on any line at 11 pm; (iii) the diagram draws in memory-aiding stroke order; (iv) it can say "12 hours left, 3 weak chapters → these 9 questions in this order"; (v) it measures which steps a student keeps forgetting. **The decisive one is (i).** Reading feels like studying; retrieval *is* studying — the strongest effect in the learning-science literature. The crammer's real terror is not "I don't have the answer" but "will it come out of my pen at 9 am?" Hence the core loop: **Learn → Hide → Attempt → Score** ("Now you try": steps hidden, the student writes/ticks, revealed against the mark scheme — *"you'd have scored 5/8; you missed the diagram mark and the final substitution"*), plus a **6 am re-ping** ("90-second check: can you still write the 4 formula lines?"), since one overnight re-test roughly doubles retention. Scoring 8/8 on the second attempt at 11 pm *is* the confidence the founder is trying to manufacture.

**9. Photo-of-the-guide mode — the founder's idea, endorsed, and strategically stronger than it first appears.** The student uploads the *printed guide's own* question and answer and asks "teach me to write and remember this." **This reframes the competition entirely: we stop racing Sri Chaitanya on content and become the layer that makes their content stick** — returning the step-marks breakdown, memory hooks, the self-test loop and the drawn diagram, on material the student already trusts. It needs no authoring to work, and it works day one on any question in any guide. Run it alongside the verified bank (bank = instant and badged; photo-mode = universal fallback). **Hard caveat recorded as a constraint, not a footnote: a student processing their own photo for their own study is their use, but our stored bank must be authored from the syllabus and past papers — publisher guide text must never become our stored content.**

**10. Memory-tip expertise is engineered, not hoped for.** The founder's requirement — the AI must be "really expert" at tips and tricks so the student does not forget — is met by encoding the craft rather than trusting model instinct: **a tip taxonomy authored once** (first-letter chains for step sequences · story-method for derivation flow · *the diagram as memory palace*, i.e. drawing order · formula triangles · unit-check recovery for a forgotten formula · contrast pairs for commonly-confused derivations · examiner-eye flags naming the mark most students lose), with bank tips written at authoring time and human-reviewed, and live generation constrained by the taxonomy in photo-mode only. **The compounding asset: the self-test reveals which tip actually worked** — did the student recall the step that tip covered? Failing tips get rewritten, and within one exam season the product holds measured memorability per tip per question. That is a moat in the same family as `student_confusion_log`. Honest limit recorded: mnemonics excel at sequences and facts but are weak on derivations, where the strongest aid is seeing the pivotal move happen — which is precisely where the sim link earns its place *even for crammers*.

**11. Distribution and "dependence" (TS + AP first, other states after).** Dependence = being present at the moment of panic and reliably faster than YouTube at 1.5×, a senior's notes, or a chatbot's wall of text. Four levers: (a) **the exam calendar IS the marketing calendar** — quarterlies Sept, half-yearlies Dec, pre-finals Jan, IPE Feb–Mar, EAMCET May, with chapter-wise "last-week plans" pushed on that rhythm; (b) **WhatsApp is the channel** — Quick Learn is already a single shareable link, so the first 30 seconds of every page should be optimized for the "bro this explained the 8-mark question in 2 minutes" forward; (c) **lecturers are the trust layer** (the existing students-through-teachers thesis, doubly true in exam month, where one lecturer's "watch these 5 before the internal" converts a whole section); (d) **free during exam fever, paid for depth** — the IPE crammer wave rides free because that is where two-state fame comes from, while the grinder segment (EAMCET/JEE depth, photo-solve beyond a daily cap) is monetized. Supporting features designed: chapter sprint ("Chapter 6, 45 minutes left" → auto-playlist by expected marks), **choice math** ("learn these 5 of the 8 LAQs and you're statistically covered" — a sentence crammers screenshot), writing-time mode (an 8-mark answer must fit ~10–12 minutes of real handwriting), and **the morning sheet** (auto-generated one-page formula + diagram skeleton per chapter, printable and born to be forwarded — the growth loop disguised as a feature). **Explicitly recorded as what NOT to do:** no gamification (streaks/coins/leaderboards) before exam-moment fit is proven; do not try to make crammers study in October; do not confuse famous with used — in TS/AP student circles "famous" means one sentence, *"it saved me before the exam."*

**12. Evidence note on the triggering review.** The student said she *understood* and found it boring. That is evidence **for** this reframe rather than against the product: the pedagogy is landing, and what failed was the framing (built for the 0.5%) plus the pacing (now fixed). The correct next test is not more concept polish but the same lesson behind an exam door.

**13. Validation ladder agreed (cheap → committed), nothing beyond step 0 started.** (0) A self-contained **demo page** — notebook paper, an 8-mark answer writing itself, mark chips accumulating, one diagram drawing itself, then "Now you try" scoring the attempt; no backend, hours of work; offered and awaiting a founder go. (1) A **one-chapter vertical slice** (~15 bank entries on a chapter that already has a sim): bank schema + pgvector matcher + notebook renderer + click-to-explain + one drawn diagram, wired into existing Quick Learn infrastructure. (2) The queued **drip test, re-aimed across segments** — 5 students recruited deliberately as 2 crammers / 2 average / 1 aspirant, crammers given the exam-framed link and the aspirant the concept-framed one; completion and dwell then answer the 0.5% question **with numbers instead of estimates**.

**14. Open founder decisions carried out of this session.** (a) Green-light the demo page? (b) Next authoring effort — an IPE important-questions layer over the existing 3 lessons, **or** a 4th concept? (c) Latency fixes 3–5, including whether to accept a **voice change** for `bulbul:v2`'s 50% saving. (d) Recruit a **lecturer reviewer** for the bank's mark schemes — the ✅ verification badge depends on a human gate, exactly as Asmi gates the sims. (e) **Doctrine hygiene:** Rule 20 suspends board/exam mode for the *teacher* product; a student-facing exam-answer surface needs its own explicit carve-out ruling rather than an assumed exemption.

**15. Not done this session.** No Exam Notebook code exists — §5–§10 are design, not implementation. Nothing was deployed (`wrangler deploy` for the mp3 live path and `deploy:cf-site` remain founder-gated under Rule 17), and the ear-walk of both lessons needs ears rather than a static check. Also unchanged and still open: the `TeacherPlayer.tsx:302+322` cancel/speak scar on a live student-facing surface, and the quick-learn desk's `website/` divergence from the office's uncommitted marketing edits, which must be reconciled before any `deploy:cf-site`.

---

## Session 87 — Customer discovery call: Sailaja (founder, prepenroll.com; IB DP/IGCSE physics tutor) — sim-licensing-into-third-party-notes question, NCERT-only site copy flagged, TeacherOn confirmed as a live outbound channel (2026-07-24)

**1. Context.** A founder sales/trial call with Sailaja, an independent IB Diploma Programme + IGCSE physics tutor (also runs prepenroll.com) who was cold-contacted via TeacherOn (see #6). Not a research session — a live customer conversation; captured here per the strategic-discussion routing rule (CLAUDE.md §9), not implemented.

**2. Feature request: post-sim assessment + revision tools, benchmarked against an EdTech competitor's roadmap.** Sailaja's ask: once a simulation is shown, surface auto-generated assessment questions the tutor doesn't have to write, ideally with an inline chatbot so the student answers each question right after the sim (removes lesson-prep load from the tutor); alternatively, flashcards for spaced revision of the concept. She named her own reference product, *Save My Exams*, whose stated growth arc was notes → smart lessons → flashcards — i.e. she's watched a competitor add exactly this kind of layered assessment/revision tooling and is asking PhysicsMind to follow the same arc. **Cross-reference:** this lands close to, but is not identical to, the already-planned `application_ladder` (near/far/novel questions) + E29 Feynman-mode grader (DISCUSSIONS Session 34/35, Topics 4/5) — those are graded-answer/transfer tools for the AI-tutor (V2) path; Sailaja's ask is teacher-facing, tutor-in-the-loop, and explicitly wants a lightweight MCQ-chatbot + flashcard layer sitting right after the V1 sim, not a full step-graded rubric. Not actioned — flagged as a V1 feature request distinct from the existing V2 assessment plan; needs a founder decision on whether it fits the teacher-facing mission (§3) as a post-sim add-on.

**3. New, unresolved business-model question: licensing simulations into a tutor's own EXTERNALLY-SOLD content.** Sailaja is authoring her own paid IB DP physics notes (sold externally to students, not just her own class) and asked, explicitly: can she link to or embed PhysicsMind simulations inside those notes, and if so, what would the pricing/plan look like? She was clear that competitors' "free" videos/links are commoditized — students pay for the *structuring* around them — and that if PhysicsMind's sims are good, her notes would be built around them, which is a differentiator for her business. She is weighing a subscription model where a student buys her notes, then runs the linked sims, then reads on. **This is a genuinely new question, not covered by any existing pricing/licensing discussion in this file** (prior GTM material — Sessions 85/86 and the B2B sketch elsewhere — covers teacher subscriptions and school licenses, not third-party redistribution/embedding of sims inside another paid product). Founder's live response: needs to think about it, asked whether her notes stay internal to her own institution or go external (she confirmed external), and said he'll raise it with co-founders. **Open, unresolved — needs a founder + co-founder decision** before quoting her a plan: whether external embedding is allowed at all, and if so, whether it's a per-view/per-link fee, a revenue share, or a distinct "content-partner" tier.

**4. NCERT-only site framing flagged as a live mis-scoping — same issue Session 86 already diagnosed, now heard from a real customer.** Sailaja, unprompted, pointed out the website presents the product as NCERT-based, which reads as CBSE-only to a curriculum outsider; her own teaching (IB DP Grade 11-12) is more calculus/derivation-heavy than NCERT. She connected this directly to the founder's own earlier point about teacher-reorderable states: if the product can reorder/hide per curriculum (Rule 38), the copy shouldn't claim NCERT as the scope. Founder agreed on the call to change the site copy "immediately." **Action item, not yet done in this session:** update public-facing copy (the `website/` NCERT/CBSE framing already flagged as overclaiming in Session 85 item 2) to reflect curriculum-flex rather than NCERT-only — no file was touched this session, this is a carry-forward TODO.

**5. Rollout timeline confirmed verbally:** Physics not yet officially launched; founder estimates ~1 month to official rollout, aiming to have most/all of Class 11-12 covered by then (consistent with the Session 85 "coverage ≈25/30 chapters" buy-trigger, not a new number). Chemistry and Math to follow Physics, expected to move faster once the Physics-authoring pipeline's lessons carry over. No new commitment made beyond restating the existing roadmap.

**6. TeacherOn confirmed as a working outbound lead channel — validates a channel previously only sized, not proven live.** Sailaja was sourced by a co-founder finding her contact on TeacherOn. DISCUSSIONS.md already sized TeacherOn as an outbound pool for the UAE segment (line ~112, "~1,500 online physics tutors listed on TeacherOn") but this call is the first confirmed instance in this file of TeacherOn actually producing a real inbound conversation for the India/founding-teacher motion. Her own read, worth carrying into the outbound playbook: individual/solo tutors tend to ignore cold outreach unless shown a product demo video — reinforces the existing "video, not just a link" instinct already implicit in the demo-site work (Session 86/PROGRESS.md `viditra.co/demo`).

**7. Trial/commercial handling (operational, not strategic — logged for continuity only):** founder extended her free trial from 1 week to 2 weeks on request, and separately offered her a "founding teacher" seat for Physics now, extending to Chemistry/Math on their launch — consistent with the founding-25 motion referenced in Session 85/86. No pricing was quoted on the call; she explicitly said she's open to paying and is waiting on the founder's answer to item 3 above before committing to how she'd use sims inside her own paid notes.

**8. Not done this session:** no code, docs (other than this log), or config changed. Three follow-ups now sit with the founder: (a) decide the third-party sim-embedding/licensing model (§3, new and unresolved), (b) fix the NCERT-only website copy (§4, a repeat finding), (c) decide whether/how a lightweight post-sim MCQ-chatbot + flashcard layer fits the V1 roadmap alongside the already-planned V2 application ladder (§2).

---

## Session 86 — International market sizing (verified) → curriculum-flex doctrine: Rules 38+39 codified from a full capacitance proof-run; per-widget teacher toggles shipped (2026-07-20/21)

**1. The question chain.** Founder: "who can use this product if we complete the whole NCERT Class 11+12 catalog — India AND USA/UK/Canada/international?" Extended Session 85's domestic segment map to the international layer, then drilled into a segment insight and ended in codified authoring doctrine.

**2. Verified international numbers** (two deep-research workflow runs, ~210 Sonnet agents total, every claim 3-vote adversarially verified; refuted claims logged and NOT used):
- **USA — best-verified single market:** ~1.53M high-school physics students/yr (2023-24), **~27,600 physics teachers** (AIP 2018-19, a floor), 86–91% of seniors attend a school offering physics. Source: AIP Statistical Research Center (primary).
- **UK:** **44,957 A-level physics entries** Summer 2025 (6th most popular subject, +4.3% YoY; 42,025 England). Concentration: 70% of A-level physics students sit in 30% of schools; a quarter of English state schools have NO specialist physics teacher (IOP) — the non-specialist teacher is a natural buyer.
- **India (CBSE only):** 16.93L Class 12 examinees across **19,299 schools** (2025); NEET-UG 2.3M aspirants for 108,940 MBBS seats (2024); coaching market $7.2B → $17.8B by 2034 (IMARC).
- **International schools worldwide:** **15,075 K-12 schools** (2026, +2% YoY), 7.7M students, teaching IB/IGCSE/Cambridge — zero-localization market by construction (Rule 35).
- **India's international-curriculum layer:** **258 IB World Schools** (+41% in 5yrs) + **700–800+ Cambridge/CAIE schools** (India = 75%+ of South Asia's 1,034, 81% of net new); **972 international schools** total (ISC broad definition, world #2 after China); **physics verified top-3 subject** at IGCSE AND AS/A-level in India; 91,000+ exam entries in the March-2026 series alone (+7% YoY). Teacher headcounts are paywalled (ISC Research report) — estimate ~1,500–3,000 senior-secondary international-curriculum physics teachers in India. **Every private-tutoring-market claim (rates, platforms, IGCSE-vs-CBSE premium) was REFUTED 0-3** — no citable numbers exist; the ~28 UrbanPro tutor replies are better primary data than any published source. Canada: nothing survived verification — unsized.
- Refuted, never reuse: 174,401 AP Physics 1 takers (2025) · 1.2M JEE for 17,385 IIT seats · 7.4M int'l-school students (7.7M is verified) · IB 11→245 growth · CAIE 398→698 · "550+ IGCSE schools" · AS+16%/A2+8% split.

**3. The segment insight (founder's own):** many INDIAN teachers teach international-curriculum students (IB/IGCSE pay better) — and **payment rails follow the teacher's bank, not the curriculum**. An IGCSE tutor in Hyderabad pays via the existing Razorpay/₹699 flow. So "serve international curricula" and "sell internationally" are SEPARATE problems: the first is a domestic premium segment reachable inside the founding-25 motion today (IGCSE-tagged tutors already sit in the UrbanPro reply list, and they double as the free curriculum-mapping reviewers Rule 38g requires); the second (foreign-banked teachers, Stripe/entity) stays deferred.

**4. Catalog classified per curriculum (ten-year-tutor exercise, 48 concepts):** AP Physics C: E&M ≈85–90% usable (the Gauss sextet is its crown jewel — and dead weight for IB/A-level/IGCSE); CAIE 9702 ≈70–75% (circuits AS + fields/induction A2); IB DP ≈60–65% (HL-skewed; no Gauss; 2023 reform dropped the AC generator); AP Physics 2 ≈55–60%; Ontario ≈70% split across grades; IGCSE ≈30–35% **via the hide rail** (motor effect, simple circuits, ac_generator is IGCSE core). Structural reads: the circuits chapter is the universal passport (only cluster in all six curricula); Ch.5 magnetism-&-matter (χ/M) is NCERT-only — exclude from international views; the depth a curriculum can't use is HIDDEN by the teacher, not re-authored (the product mechanics — reorder/hide/rename — ARE the localization layer).

**5. Product decisions:** curriculum dropdown → catalog filtered per curriculum + per-curriculum PRESETS that pre-hide states via depth rings (pure data, the Rule 31 presets-over-one-engine pattern one level up). **ONE sim per concept, never per-curriculum copies** (384-file rot scenario explicitly rejected). Seven authoring guidelines were drafted and — per prove-then-harden — tested on a REAL concept before codification.

**6. The proof-run (capacitance, Ch.2 #9, isolated worktree `feat/curriculum-flex-pilot`):** full pipeline + new field_3d scenario. Friction report verdict: guidelines cost ~45–60 min/concept, ZERO compromise to the CBSE/JEE lesson; tag-at-birth (syllabus tags) cannot be executed without teacher verification — tags ship as claims. **One structural finding the guidelines missed and the auditor caught:** a ring-neutral explore state inherits content from every ring (S7 displayed ε₀A/d, taught only in hide-able S5) → founder chose the free fix: **the explore state may only surface core-ring content** (over engine-heavy ring-adaptive explore). Also caught en route: a Rule-34d chrome collision reproducing an OPEN scar on the PRIMARY-aha readout (invisible to THE EYE by construction — it captures the raw sim, not the review chrome), and a founder-reported slider bug whose root cause (ramp modes never checking the drag-seize flags) silently affected FOUR states, two unreported.

**7. Per-widget teacher toggles (founder request → shipped same session):** teachers individually show/hide each slider row/graph/formula/readout — the granular sibling of Clean mode, the widget-level analogue of Rule 25d. Sim DECLARES its widgets (SIM_READY); chrome ⚙ panel is built-once generic. UX iterated on founder feedback: Auto/On/Off tri-state buttons → plain switches that always mirror what's on screen (WIDGET_VIS_STATE live-sync) + hover-ping (hover a row → the widget pulses on-canvas) + in-panel ✓ Save/↻ Defaults riding the teacher_layouts account sync.

**8. Codified as law (founder-approved):** **Rule 38** (curriculum-flex authoring: depth rings + coherent cuts, core-only explore, notation ladder, dialect, decided graph axes, widest-overlap anchors, tags-as-claims + no-unverified-preset, presets=data) and **Rule 39** (teacher widget contract for every new scenario). Propagated same-session into CLAUDE.md §6/§7, CLAUDE_RULES.md, and all five canonical agent specs (quality_auditor gains Gate 3h) — the 2026-07-13 doctrine-debt lesson applied on day one. **Deliberately deferred, each behind a named gate:** fleet retrofit of the 48 existing concepts (founder-named delta), shipping any curriculum preset (Rule 38g real-teacher verification), international school-license motion (coverage + entity/Stripe), maths/chem (physics-first doctrine reaffirmed), universities (not the market — product soul is the school teacher's live surface).

**9. School-licensing sketch (for when coverage justifies it):** Indian international schools all have smart boards (product needs only Chrome — vendored deps doctrine pays off); pricing ladder sketched: teacher ₹699/mo (exists) → school small ₹60k–1L/yr → school mid ₹1.5–3L/yr (≈₹500–800/science-student/yr, 2–3× the old Cat-2 B2B anchor); niche TAM ≈₹4–7 Cr/yr across 300–500 core-eligible schools (grade 11-12 international physics), ~900 stretched via IGCSE hide-depth. Land the teacher inside first; school license when 2+ teachers in one school are active. Coverage remains the gate, not price.

---

## Session 85 — Product scope research: who beyond Class 12 JEE/NEET can use PhysicsMind today, and what a full Class 11+12 NCERT build would unlock (2026-07-17)

**1. The question.** Founder noticed the marketing site's "Class 11th and 12th, JEE/NEET" heading may not reflect the real addressable teacher base — asked for ground-level clarity on whether CBSE Class 8/9/10, ICSE, and state-board (Telangana/AP/Maharashtra/TN) teachers can use today's catalog, then extended the question to a full Class 11+12 NCERT build hypothetical ("what if we create every simulation for every chapter").

**2. Reality check on today's catalog: it's 100% Class 12, not "11–12."** Every shipped concept JSON is tagged `class_level: 12`; there is zero Class 11 mechanics content in the product (the old ~60 kinematics/forces JSONs are retired architecture per CLAUDE.md §0). The marketing site's "Class 10–12 · JEE / NEET / CBSE" eyebrow (`website/index.html`) already overclaims on the Class-11 side — a correction needed regardless of any scope decision below.

**3. Today's real segment map** (multi-agent deep-research workflow + 6 targeted follow-up agents, ~20+ primary/secondary board sources verified):
- **Strong as-is:** ICSE Class 10 (uniquely teaches emf + internal resistance quantitatively — CBSE defers these to Class 12; `emf_definition`/`internal_resistance` serve ONLY this segment outside Class 12) and **Telangana SSC Class 10** (surprise finding — TS still runs the legacy SCERT textbook, deeper than NCERT, reaching drift velocity, Kirchhoff's loop law, and F=BIL at Class 10; AP SSC switched to a shallower NCERT-derived book in 2024-25 and diverged sharply from TS despite shared ancestry).
- **Board-examined slice:** CBSE Class 10 (4 sims map onto Unit IV, 13/80 board marks).
- **Qualitative-mode fit** (formula/derivation states hidden — a config change, not new engineering, since `formula_overlay` is already per-state data and hide/reorder already ships per Rule 25d): AP/Maharashtra/TN SSC magnetism, CBSE Class 10 magnetism. Maharashtra oddly RETAINS EM induction + generator qualitatively where current CBSE dropped them.
- **No real fit today:** CBSE/ICSE Class 9 and Grade 8 — not just a depth mismatch; the catalog has ZERO kinematics/motion sims, so Class 9's one strong topical overlap (Motion) has no product to offer regardless of syllabus depth.
- **No board below Class 11 teaches** electrostatics, magnetism & matter, or quantitative AC — 17 of 41 shipped sims stay Class-12-exclusive by construction, full stop.
- **Market sizing:** no board publishes a subject-teacher census anywhere (verified negative — CBSE's SARAS portal down, Parliament declined to estimate the coaching sector). Only school/candidate counts exist: ICSE Class 10 ≈2,957 schools (2026, small but high-WTP/English-medium), CBSE Class 10 ≈26,700 schools vs CBSE Class 12 ≈19,300–20,000 (Class 10 is the BIGGER CBSE footprint, inverted from where content fits best), state boards collectively the largest raw TAM but fragmented/vernacular.

**4. The "build all of Class 11+12" extension.** Confirmed against live 2026-27 primary NCERT/CBSE PDFs: **28 total chapters** (14 Class 11 + 14 Class 12, structurally stable across 2 consecutive academic years). Current catalog = 6/28 chapters, 41 sims — **this is not a new idea, it's the founder's own already-declared buy-trigger** (CLAUDE.md §3: "coverage ≈25/30 chapters"), now given real numbers. Estimated full-build size: **~150–220 sims** (working midpoint ~180), scaled from current per-chapter density (rich chapters ~9–11 sims each, e.g. Current Electricity/Moving Charges & Magnetism; thin ones like Magnetism & Matter ~2).

**5. What a full build unlocks (verified, not assumed):**
- **Class 11 becomes a native segment for the first time** — currently zero product exists for it.
- **Class 10 optics is the single biggest structural upgrade, and a newly-confirmed finding.** Class 10 Light/optics is genuinely QUANTITATIVE across all six boards checked (CBSE, ICSE, Telangana, AP, Maharashtra, TN) — mirror formula, lens formula, magnification, refractive index, dioptre eye-defect correction, all with real numericals. Combined with existing current-electricity coverage, this roughly doubles the on-syllabus physics-mark-share for Class 10 on every board, not just CBSE. Wave optics (interference/diffraction) is confirmed absent everywhere at Class 10 — permanent overshoot, hide unconditionally. Optical instruments (telescope/microscope) are absent/qualitative on five boards but **genuinely Class-10-taught with real content in Tamil Nadu** — the one board-specific exception worth its own toggle.
- **Class 9 becomes a partial, uneven upgrade — not a clean unlock.** Straight-line kinematics and core Newton's-laws demos match well on both CBSE and ICSE. But Motion in a Plane (vectors/projectile) has zero Class 9 equivalent on any board — permanent overshoot. Work-Energy-Power fits CBSE-9 but ICSE defers it entirely to Class 10. **Live curriculum risk flagged:** NCERT's brand-new Grade 9 textbook (*Exploration*, First Edition, printed April 2026) has cut standalone Gravitation to a two-paragraph footnote of F=ma — the national trend is moving AWAY from where a Class-11 Gravitation sim would land, not toward it. ICSE's Fluids/Archimedes (two dedicated, quantitative chapters) is a genuine win specific to ICSE Class 9.
- **Grade 8 and ~8 of the 28 chapters** (Thermodynamics, Kinetic Theory, Dual Nature, Atoms, Nuclei, Semiconductor Electronics, plus Wave Optics/instruments outside TN) **stay JEE/NEET-exclusive no matter how complete the catalog gets** — no sub-Class-11 curricular hook exists anywhere for these, full build or not.

**6. Open tension flagged, not resolved.** The English+Hindi-only decision (Rule 30i, made the same day) works against the strongest state-board finding here — Telangana, AP (Telugu-medium), Maharashtra (Marathi-medium), and TN (Tamil-medium) are all language-mismatched with the new plan, even where their syllabus fit is strong or improving. This directly conflicts with the still-fresh 2026-07-02 "first customers = local Telugu teachers" plan on record. Not resolved this session — needs a founder call before any GTM copy locks in.

**7. Not yet researched — flag before acting on it:** ISC (CISCE's Class 11–12 board) is a DIFFERENT syllabus from ICSE Class 10 under the same council; don't assume it maps as cleanly without a dedicated pass. State-board Class 9 mechanics/optics depth (Telangana/AP/Maharashtra/TN) — only their Class 10 was researched; Class 9 depth was checked for CBSE/ICSE only.

**No code or CLAUDE.md changes made this session — pure research/discussion, captured here per the strategic-discussions convention. See `PROGRESS.md` top entry for the session pointer.**

---

## Session 84 — GLOBALIZATION PIVOT: Rule 35 "globally neutral content" SUPERSEDES Session 81's culture-is-data; Rule 34 uncluttered canvas codified; first demo live on viditra.co via Cloudflare Workers (2026-07-10)

*(Numbering note: "Session 83" is cited by Rule 33 (macro↔micro, 2026-07-08) but was codified straight into CLAUDE.md without a DISCUSSIONS entry — the number stays reserved for that conversation.)*

**1. Globalization pivot — Rule 35 (founder directive).** The product will ship to overseas syllabi (USA, UK, …) by dividing/tagging the SAME simulations per syllabus — and that is only cheap if sims carry **zero India-specific culture to strip later**. Founder: "there should be no Indian context or Indian culture included in the simulations … so that in the future we'll get less work to do for other countries." This **supersedes Session 81 decision (3)** ("KEEP Indian anchors as swappable `real_world_anchor` data"): anchors are now authored **universal in the first place** (ceiling fan, phone charger, elevator, MRI, speaker magnet) — no swap step ever needed. Region constants (50/60 Hz mains, 230/120 V) phrased neutrally or parameterized. Scope guard: this governs RENDERED/NARRATED sim content only — NCERT remains the syllabus backbone for coverage/sequencing, and the `student_confusion_log` moat remains Indian-student-sourced (data ≠ content). Existing concepts de-localize on next touch (retrofit delta; re-voice only then). Codified: CLAUDE.md §5/§6/§7 Rule 35 + CLAUDE_RULES.md + architect/quality_auditor canonical specs (the auditor's anchor probe INVERTED — country-specific culture is now the FAIL, where non-Indian anchors used to be) + emissions re-synced. Known live violation to fix on next ac_generator touch: the demo's STATE_1 narration "the machine behind the fifty-cycle power in every Indian home" (re-voice when touched, not now).

**2. Rule 34 — uncluttered canvas (founder, from viditra.co/demo screenshots).** The sim screen must show the machine, not walls of text: ≤5-word delta-cue caption on-canvas; narration prose in a subtitle strip BELOW the canvas (global `#capStrip` chrome); ONE math-serif Unicode formula surface per state; HUD = compact value-only instrument; overlays never collide/clip; all on-canvas math real Unicode across all THREE text paths (DOM overlays / canvas graph text / 3D sprite labels). Reference implementation: the ac_generator de-clutter retrofit (2026-07-10, THE EYE-verified).

**3. First public ship on the brand domain + hosting clarity.** The de-cluttered demo deployed to **https://viditra.co/demo/** — discovered viditra.co is a **Cloudflare Worker (`viditra`) serving `website/` as Static Assets**, not Pages/Netlify; deploy = `npx wrangler deploy` (root `wrangler.toml`, wired as `deploy:cf-site`). Netlify hosts ONLY the pilot teacher app and keeps credit-blocking on audio-heavy deploys → decision: migrate the app to a second Worker (`viditra-app` → app.viditra.co) mirroring the marketing site; runbook ready (`docs/notes/cloudflare-migration-runbook.md`), migration not yet executed.

---

## Session 82 — Rule 32 legibility doctrine + Rule 31 word-budget pacing + motion-archetype taxonomy codified; Fable-5 architect / Sonnet-5 fleet model split (2026-07-08)

**The ask.** Founder: tighten CLAUDE.md so upcoming sims are decisively better — every state under ~20s, no two states alike, every state alive with choreography, each state teaching one really important thing, and — the new idea — a teacher clicking through should *notice* the difference in every state. Also: run the architect on Fable 5, the rest of the fleet on Sonnet 5. Full brainstorm → approved design at `docs/superpowers/specs/2026-07-08-rule32-legibility-rule31-tightening-design.md`.

**Diagnosis.** ~80% of the ask was already Rule 31 — the real gaps: (1) three contradictory pacing numbers (~15–25s / ~28–35s / ~20s) with nothing counting at the gate; (2) "no two states alike" enforced by judgment only; (3) *observability of difference* — states being different ≠ each state making its delta legible — had no home in doctrine.

**Decision 1 — Rule 31a pacing = word budget, not a stopwatch.** A guided state = ONE idea + ONE complete motion cycle; narration **25–55 EN words (2–4 tight sentences ≈ 10–20s)** counted on `text_en`; >55 = split flag ("two ideas"), <~20 = merge/enrich flag; the motion may outrun the talking, never the reverse. Kills all three old numbers. Ground truth found during implementation: **faraday itself runs 67–94 words/state** — the exemplar's narration predates the budget; all doctrine now says "clone the shape, not the sentence length"; first true word-budget exemplar = `magnetic_flux` (next authored).

**Decision 2 — Rule 31b distinctness is DECLARED via a motion-archetype taxonomy.** 10 named archetypes (`translate-through · rotate/flip · densify/rarefy · oscillate/track · align/scatter · flow-along-path · reveal-build · cycle-compare · null-result-hold · drag-sandbox`); the control table gains `archetype` + `delta` ("what changed vs previous") columns; **no archetype repeat except a declared contrast/reversal pair** (faraday S2/S3 = the canonical pair — same translate, delta = sign flips); `drag-sandbox` = explore only. Stays a design artifact + auditor gate, NOT a Zod field — schema hardening only after the taxonomy survives ~5 concepts (same staging Rule 31 used).

**Decision 3 — NEW Rule 32, the legibility doctrine** (founder picked exactly the recommended cut from a 9-item menu): **32a** cause moves before effect (readable ~0.5–1s beat); **32b** only the taught variable moves (explore exempt); **32c** every caption OPENS with a ≤5-word delta cue; **32d** same apparatus, home pose, no teleport-rebuild; **32e** exactly ONE glow focal at any instant. Deferred-with-triggers (recorded in the rule body): progressive formula assembly, money frame, live readout law, full-range sweep (guidance only).

**Enforcement shipped.** New quality_auditor **Gate 3f** (word count + archetype table + delta cue mechanical; cause→effect + one-variable + continuity judgment; single-focal); Gate 3e#5 pacing superseded → 3f#1. eye_walker verdict table gains a **"delta visible?"** column (frozen-frame N vs N−1 diff + cue readability). Architect/physics_author/json_author specs updated end-to-end (table columns, cause-first timelines, caption convention); all 5 emissions re-synced same session. Asmi's per-state review grows a 7th question: *"Did this state feel clearly different from the previous one — worth its own click?"* (CSV column `clearly_different_YN`) — the teacher-pulse check on the whole doctrine.

**Decision 4 — model split.** `architect` pinned **`claude-fable-5`** (the creative pedagogy/choreography role — runs once per concept, cost marginal; watch the first dispatch, fallback = revert pin); `json_author` + `quality_auditor` (upgraded from sonnet-4-6) + engine cluster pinned **`claude-sonnet-5`**; physics_author/eye_walker/retrofit_surgeon already sonnet-5; shipper stays haiku.

**Rollout discipline.** Upcoming sims only — NO fleet retrofit (later = founder-named retrofit-surgeon deltas). Prove on `magnetic_flux` → `lenz_law` → `magnetic_materials`, then Asmi pulse-check, then consider Zod hardening. New specs/models dispatch natively from the NEXT session. Known skip-list: PROGRESS history + old per-concept specs keep old pacing strings (history, not doctrine); the 2026-07-03 engine_bug_queue seed's "~18-22s" prevention_rule row is superseded by the word budget (optional DB-row text refresh later).

**Follow-up decision (same day) — how old Socratic sims convert: the SAME pipeline, no new agent.** Founder asked whether old-strategy→new-strategy reconstruction (which must be free to DELETE/merge states) needs a dedicated subagent. Answer: no — deciding which states live is architect-class work, and the new archetype rubric makes deletion mechanical (two old states share an archetype with no contrast pair → merge; no archetype-worthy motion → delete; a predict→reveal pair → one straightforward beat; Session 78 audit precedent 309→~75 states). Codified as a **"Reconstruction mode" section in the architect spec**: input = existing JSON + measured word counts + founder verdicts; every old state re-earns its control-table row; PRIMARY aha moves with its surviving state; deletion-mechanics checklist (state_count/entry_state_map/field_3d_config renumbering, Gate 12, deep-dive flags, full EN+TE re-voice, baselines-after-approval); Class A (scenario has per-state modes → pure JSON pass) vs Class B (static-poses → renderer_primitives live-instrument delta FIRST — the Ch.1 coulomb class). Division of labour stands: full reconstruction = pipeline via Reconstruction mode; ONE named delta = retrofit-surgeon (its binding-count invariant forbids state deletion by design); engine gaps = peter_parker. A new "reconstructor" agent was rejected: duplicate-architect antipattern + the 2026-06-18 stage-matching rule (phase-limited task ≠ permanent role). The pending coulombs_law spec's pacing section was patched to the word budget.

---

## Session 81 — Launch strategy: one app + roles (not two products); ship-to-everyone with a reviewer-first flag for novel sims; culture-is-data so the product is already global; UAE market is real; name→domain→website→dashboard sequencing. Umbrella-brand naming session run — decision DEFERRED (2026-07-07)

A no-code strategy session covering five linked "how do we launch" questions, plus a naming session the founder chose to pause.

**Topic 1 — One production app + roles, NOT two apps.** Founder's question: build a second production app for paying teachers separate from the reviewer pilot app? **Decision: no — one app, one Supabase project, one catalog; the difference between a reviewer and a paying teacher is a ROLE on their account (`reviewer` = free forever + screen recordings; `subscriber` = paid, telemetry only), never a separate deployment.** Two apps = double deploys + drift. The "sims reach reviewers before paying teachers" pipeline the founder wants is a **content flag, not a second app**: extend the existing `PILOT_CONCEPTS` allowlist idea into an early-access tier (new sim → reviewers only in-catalog → after their classroom feedback, flip the flag → visible to all subscribers, badged "New"). Feedback types map onto what already exists: event telemetry = the live `pilot_events` pipe (everyone); screen recordings = the existing Asmi/reviewer human loop (reviewers only, no upload feature needed for ~3 people).

**What actually must change for a paid launch (the real work, ~1 month):** the stack (static site + Supabase + Netlify/Cloudflare) scales to hundreds of teachers — no re-platform. Needed: (1) **Payments** — Razorpay subscriptions + a Supabase Edge Function on the webhook writing subscription status to a `subscriptions` table; login checks reviewer-role OR active-subscription. (2) **Auth hardening** — current gate is client-side fail-open (right for a free trial, wrong for paid); make it fail-closed, disable public signups, provision accounts only via the payment webhook. Honest caveat: sim files are static assets, so DRM is not worth chasing for v1. (3) **Hosting** — the Netlify credit-block (see `project_netlify_deploy_credits_blocked`) must be resolved before marketing drives traffic: paid Netlify plan or a Cloudflare Pages migration ($0). (4) **Self-serve onboarding** — landing page → pay → auto-create account → welcome email.

**Topic 2 — Ship "decent and improving" to everyone, with a reviewer-first exception for NOVEL sims.** Founder's question: send every new sim to reviewers first, or ship a decent version to paying teachers too and iterate on feedback? **Decision: ship to everyone by default; reviewer-first ONLY for genuinely novel work.** Reasoning: the reviewer gate is a 1–2 week classroom cycle on the reviewer's schedule — gating every sim behind it caps shipping rate at the reviewers' teaching calendar, and coverage (~25/30 chapters) is the real bottleneck. A sim reaching production has already passed tsc + validator + THE EYE + quality-auditor + founder visual approval — "reviewed by everything except a live classroom." And teachers **preview** sims during lesson prep, so a weak sim is "teacher skips it," not "humiliated mid-class" — a tolerable cost for speed. **The exception:** anything with a **new scenario type / renderer engine / interaction pattern** goes reviewer-first (early-access flag → one classroom cycle → flip to all), because that's where the unknown-unknowns live; a sim built on already-proven engines ships straight to everyone. The exception shrinks over time as each reviewer-caught defect becomes an `engine_bug_queue` scar. **Doctrine note:** this makes the Asmi professor gate a POST-release loop for engine-proven sims and keeps it PRE-release only for novel ones — a real change to pipeline stage ④ (was pre-release for all). Founder's explicit call.

**Topic 3 — The product is already global; the GO-TO-MARKET should not be yet.** Founder wanted to also market to Western (EU/US/Gulf) teachers from day one, and asked whether to strip Indian culture and/or build per-country syllabus versions. **Verified fact that reframes it:** the live diamonds are already culture-neutral by architecture — Rule 24 forces canvas = labels/equations only, so the VISUAL has zero culture; the Indian anchor lives in ONE swappable metadata field (`real_world_anchor`) + occasionally a narration sentence (checked `faraday_law_induction`: primary anchor = magnet down a copper pipe, universal; "India" appears in exactly one grid sentence). The mango/cricket/rickshaw hits are all in the ~60 OLD-architecture JSONs, not the product. **Decisions:** (a) **Do NOT strip anchors** — they're data in a text field, not baked in; a Western variant is a one-sentence anchor + narration swap (minutes of agent work per concept), so stripping now only makes the product blander for the actual Telugu customers to please customers who don't exist yet. (b) **One product, one catalog — never per-country builds;** NCERT/AP/A-level/IB overlap almost fully at the concept level (SI units everywhere), and the hide/reorder/rename teaching layer IS the syllabus adapter ("the teacher localizes" — the EXIST argument). A later **syllabus-tag layer** (filter catalog by "AP Physics C Unit 11" etc.) is cheap metadata and can wait; en-US TTS waits even longer (TTS off by default). (c) **Staged GTM, not day-one Western marketing** — marketing to strangers during the highest-defect phase acquires the least-forgiving customers with the slowest feedback loop; the "ship decent, iterate fast" model only works where feedback is fast and forgiveness high (local teachers). The bridge the founder was missing: **Gulf CBSE schools + diaspora teachers** = Western-level (AED/USD) revenue with ZERO product change (they teach the exact NCERT curriculum), reachable through the same Indian networks — step 2, before native US/UK. **Recommended sequence:** now→~3mo: 10–20 Indian (Telugu-first) teachers, build the payment/auth/hosting layer, spin the flywheel; in parallel at $0: be *available* globally (English landing page, don't block foreign signups) without *marketing* globally; at ~15–25 chapters: active push to Gulf/diaspora + add Stripe; after: native West + syllabus tags. Marketing DATE driven by coverage + trial telemetry, not the calendar; a soft launch (payments live, sold personally at a founding rate) fits better than a broad 1-month "official" launch.

**Topic 4 — The UAE market is real and physical (not just India-based remote tutors).** Researched (sources logged in-session). Four segments, easiest→hardest: (1) **India-based online tutors** teaching UAE students remotely (~1,500 online physics tutors listed on TeacherOn for UAE) — AED-funded WTP, identical sales motion to local teachers, the zero-effort bridge; (2) **licensed freelance private tutors physically in the UAE** — now formal/legal via the MoHRE 2-year free private-tutor work permit (2023–24), mostly Indian expats tutoring CBSE/JEE/NEET, findable on MyPrivateTutor.ae / TeacherOn — the core UAE segment (individual buyers, AED pricing); (3) **local tuition centres** (ATP STEM, Meccademia, long tail) — target the INDEPENDENT centres, not the big Indian chains (Allen Overseas, PW Gulf are already there = demand validation but build in-house, won't buy); (4) **CBSE school physics teachers** (18+ CBSE schools in Dubai alone, plus Abu Dhabi/Sharjah) — the Asmi profile, many also tutor privately under the new permit. Motion: seed a couple of UAE-teaching online tutors into the first Indian cohort → then licensed freelance tutors IN the UAE via diaspora connections → then independent centres + school teachers once coverage is respectable. Flag for later: selling to centres as businesses raises UAE VAT/invoicing questions (not a now-problem).

**Topic 5 — Sequencing: name → domain → website → dashboard; and personalization = synced customizations, not the profile pic.** (a) **The name is the unblocking decision** — it goes into the domain, the Razorpay bank-statement descriptor, invoices, login page, word-of-mouth; renaming after teachers subscribe is expensive, renaming now is free. Chosen strategy: **umbrella brand now** (subject-neutral, subjects as product lines under it) over a PhysicsMind/ChemMind family. (b) **The website is NOT a later milestone — it's part of the 1-month payment work:** the marketing/landing + subscribe flow IS the payment layer; and move the app OFF `physicsmind-review-13308.netlify.app` onto `app.<brand>.com` immediately (with 2 users it's painless; later it breaks every bookmark/saved login). (c) **Teacher dashboard = the retention surface, ranked by what creates ownership:** #1 (highest value) **sync the state hide/reorder/rename customizations from localStorage to the teacher's Supabase account** so her lesson setup follows her from home laptop to school smartboard — that's real, good lock-in (her invested labor lives in the product); #2 saved lessons / "my sims"; #3 profile + photo (cheap, warm, but garnish — build it last). Keep the V1 dashboard purely teacher-facing; student accounts/linking is V2 (voice-professor) territory (avoids minors'-privacy/moderation now).

**Naming session — run, then DEFERRED by founder.** Style chosen: evocative real/fused word (not "Mind"-family, not descriptive, not coined-from-nothing); roots = universal English/Latin; domain rule = **exact .com or nothing**; core idea = the **trio relay** (concept flows teacher → simulation → student → "I got it!" flows back; the sim is the bridge). 114 candidates checked live against the Verisign RDAP registry; verified .com-available include `viditra`, `vidtria`, `kenlume`, `brightken`, `videtia`, `illucesco`, `dilucesco`, `sophitra`, `tridya`, `phanotra`, `trisophia` (of these the strong, collision-clean ones ↓). **Top two (anagrams of each other, both clean):** **Viditra** (`vid-` = see/know, shared PIE root *weid-* → Sanskrit vidya + Greek eidos/idea + Latin videre + German wissen; `-tra` = Sanskrit instrument suffix as in yantra/mantra → "instrument of seeing/knowing"; matches the live-instrument doctrine; best say-once-spell-right) and **Vidtria** (`vid` + Greek `tria` = three → "the seeing three" = encodes the trio literally; slight `dt` cluster; Vitria Technology *sounds* near). Recommendation on the table: **Viditra as brand + register vidtria.com as its shadow** (same 7 letters, ~₹2k/yr, reversible). Runners-up with weaknesses noted: Kenlume/Brightken ("Ken" reads as a man's name), Videtia (pronunciation splits), Illucesco/Dilucesco (beautiful meaning — "it dawns on me"/"daybreak" — but 4 syllables, poor referral ergonomics), Trisophia (weak collision: a South African distributor). **Nothing registered. Founder taking more time to decide.** When decided: register the .com (+ .in/.co + social handles) same day; formal India + EUIPO trademark search before marketing spend.

---

## Session 80 — Doctrine CODIFIED as Rule 31 after `faraday_law_induction` validated it: rules, agent specs, emissions, exemplars all rewritten; Gate 3c → legacy, new Gate 3e (2026-07-02)

**The gate fired.** Sessions 78/79 defined the model; the deal was "codify only after a good output." `faraday_law_induction` (Ch.6, authored NATIVELY in the model — 6 distinct-motion states, per-state controls S1–S4 none / S5 speed+turns / S6 all, flowing induced-current beads, two-phase Lenz pole face after founder review) passed every gate and the founder approved. The strategy is now **Rule 31** (index + full body), with Rule 15 (advance_mode) and Rule 16a (misconception delivery) amended, and the entire prose layer that used to *mandate* Socratic authoring rescoped to legacy: architect's session-33 "Socratic reveal — always-on" section superseded by "Straightforward motion beats"; physics_author's reveal-timeline → motion-timeline + control spec; json_author's "sliders in the last state only" → per-state contextual controls; quality_auditor's **Gate 3c now legacy-only** with a new **Gate 3e** (distinct motion, control-table match, one-panel discipline, no Socratic artifacts) as the new-model counterpart; the blessed static hook un-blessed. All 5 emissions regenerated same-session (SYNC-verified); 14 proof_run exemplars bannered [OLD MODEL]; a new `faraday_law_induction_skeleton.md` is the canonical clone target. Machine layer untouched on purpose — the Zod schema keeps `wait_for_answer` so the 104 existing JSONs keep validating; per-state control declarations remain renderer convention + Gate 3e judgment (a `visible_controls` schema field is the noted future hardening). **Next test of the codification:** Ch.5 #6 `magnetic_materials` — the first concept the pipeline agents author under Rule 31 specs rather than by founder-guided session.

**The TTS question (same session) → Rule 30f.** With narration off by default the founder asked what TTS is even FOR, whether regional voices (Telugu now; Punjabi/Gujarati/Tamil/Malayalam someday) help or hurt, and whether it fights the new model. Ground-level answer: in the LIVE classroom TTS adds ~nothing — the teacher is the voice and the localization layer (that's Rule 24's whole design); TTS earns its keep in exactly three places — the first-demo "it speaks Telugu" moment, teacher lesson-prep preview, and student home-revision replay (recorded-lecture value, NOT the AI professor). Fully aligned with Rule 31 (Rule 26 keeps narration a pure overlay; straightforward beats are cheaper to translate than Socratic ones). The catches: each language multiplies every future edit; each needs a native reviewer (Rule 17) — the founder can gate Telugu/Hindi, nobody gates Tamil yet; Sarvam credits are the practical gate. **Decision (founder — first customers are 2–3 LOCAL TELUGU TEACHERS):** ship EN + Telugu audio per concept, rendered as the LAST DoD step (post-founder-approval, pre-handoff); Hindi text-only until a Hindi market exists; further languages only after ~25-chapter coverage + a native reviewer each. Codified as Rule 30f.

---

## Session 79 — The "straightforward, non-Socratic, per-state contextual controls" authoring rule: each state shows only the sliders it needs; the last state shows them all. First built natively on `magnetisation_and_intensity` (§5.5); founder video-review → 6 engine_bug_queue fixes (2026-07-01)

**The context.** After the live-instrument formalization (Session 78) the founder pushed the model further from the *teaching-sequence* side: build new sims like the original 40 (a guided sequence of states + a final interactive state) but **strip the Socratic** predict→wait→reveal (pointless without a real student in front of the sim), make **every state straightforward** — one aspect, real motion/choreography, **no two states alike, no state static** — and shorten durations (the old ~45s/state was padded with Socratic waiting). This was agreed as the default shape for all upcoming sims (not a retro-fit of the 40 yet — same discipline as Session 78: prove it on a few, with a real teacher, before scaling).

**The new rule that crystallized this session — per-state contextual controls (progressive disclosure).**
Each state exposes **only the interactive slider(s) relevant to the concept it is teaching** — not the full control set. The **last (explore) state exposes ALL sliders** (current, material, field, whatever the concept has). Intermediate teaching states carry only their own live control. Rationale: the control *is* part of the teaching (one slider present silently says "this is the variable that matters here"), it kills "what do I touch?" paralysis, and by the last state the student has met each control individually so the full set reads as synthesis, not a cockpit. This is essentially the live-instrument model + Rule 27 (explorer pattern) refined into: *"each state exposes only its own live controls; the final explore state exposes the full set."*

> ### The four catches (so the rule doesn't backfire) — laid out in layman terms for the founder
> 1. **Build the control surface ONCE, show/hide per state — never rebuild sliders per state.** (One TV remote, buttons grey out per channel — not a new remote per channel.) The trap is the same slider built in 6 places → 6 edits every change, one missed → silent break. Handle: one panel built at load, per-state flags toggle rows. (Done for magnetisation: one `mag_sliders` panel + `show_material`-style flags.)
> 2. **"No slider" ≠ "static." Motion and interactivity are separate axes.** A state can be alive by *moving on its own* (auto-demo) OR by being *touchable* (slider). The real floor is **"not a frozen slide,"** NOT "must have a slider." Don't bolt a pointless slider onto a watch-this beat (e.g. "dipoles jitter randomly") just to tick an interactivity box — keep it auto-animating with zero sliders.
> 3. **A repeated slider stays in the SAME screen position across states.** (Brake pedal is always in the same place.) A control that appears in multiple states must not move around — the teacher hunts for it on a projector in front of 30 students otherwise. Handle: new controls *add a row below* the persistent one; they don't displace it. (Magnetisation: bottom-right panel, current always the top row, material appears underneath.)
> 4. **Each state DECLARES its live controls in the spec, up front — not ad-hoc while coding.** (Architect marks which switch goes in which room on the blueprint.) Author a per-state control table *before* coding, e.g. S1 current-only · S2 none(auto) · S3 current-only · S5 current+material · S6 all. The code follows the table; it becomes a reviewable design decision, not an accident.

**Where it was proven — `magnetisation_and_intensity` (NCERT §5.5), the first concept built natively in the straightforward model.** New `magnetisation` field_3d scenario (solenoid supplies intensity H = nI; an insertable core develops magnetisation M as atomic dipoles align; B = μ₀(H+M); χ/μ_r classify dia/para/ferro). 6 states, each a distinct motion: S1 current beads flow · S2 dipoles jitter random · S3 dipoles align + M grows (aha) · S4 dense field-lines fade in + B climbs · S5 dia/para/ferro cycle · S6 explore. tsc 0 · validate 103/0 · THE EYE clean · runtime Playwright probe (zero console errors) — the probe caught a real per-frame dispatch bug THE EYE missed (the update call had landed inside the earths_magnetism `if`-block).

**Founder video-review → 6 fixes (logged to `engine_bug_queue`, all FIXED).** The founder screen-recorded a review; every issue was logged OPEN first, fixed, then flipped to FIXED (the recording-as-bug-report workflow):
1. **Current not visibly moving** → bright amber beads now spiral along the coil wire, speed ∝ I.
2. **Material slider shown where irrelevant** → hidden on S1–S4 (current only), shown on S5 + S6 — *this is exactly the per-state-controls rule above, and is what generalized it into a rule.*
3. **Readout showed material/M/μ_r on the vacuum state** → state-aware (S1 shows only H + B₀=μ₀H).
4. **Legend listed dipoles/M/B on states with no dipoles** → trimmed to one state-appropriate line.
5. **Formula too small** → prominent big per-state formula panel on the right.
6. **S5 material slider label lagged the auto-cycle** → synced every frame.

**Status / next.** Rule agreed as the default authoring shape; candidate to **codify** as a one-line discipline in `src/data/concepts/CLAUDE.md` (the per-state control table as the first authoring step + the four catches as a checklist) — pending founder sign-off on wording; root `CLAUDE.md` untouched without approval. Magnetisation is on the main working branch, uncommitted, served for review at `http://localhost:8092/magnetisation_and_intensity/`. Not yet: TTS audio, Asmi review, the assessment quiz block, commit.

## Session 78 — The live-instrument model formalized: a "state" is a saved VIEW, not a teaching step; state-count = distinct-pictures (archetype-graded); the views + editable-beats two-layer reconciliation; 41-sim audit (309 → ~75 states); magnetic_force prototype 8→4 in a worktree (2026-06-30 → 2026-07-01)

**The dilemma the founder kept circling.** Across the session the founder asked, repeatedly and from different angles: for a *teacher*-facing product, do we need a sequence of states (teacher shows each state), or just ONE interactive state where an expert teacher alters parameters and explains? Are we mixing two things — an "instrument" and a "teaching sequence"? How many states does a sim really need — one, two, three? "I'm really confused and in a dilemma — reality, no illusion."

**The resolution — there is only ONE instrument; "states" are its saved VIEWS, not lesson steps.** The confusion came from conflating two meanings of "state": (a) the OLD auto-advancing reveal-step (a slide the sim performs — the teacher-in-a-box we're killing), vs (b) a **saved view of the live instrument** = a camera angle + which elements are visible + a label + parameter presets. Properly built, **the states ARE the instrument** — there is no separate "sequence layer," so we are NOT mixing two things. The teacher's expertise means we don't need a narration/sequence; it does NOT mean one screen, because the limiting factor is the *student's eyes and the camera*, not the teacher's brain.

> ### The grading rule (how to count states for ANY sim)
> **Start at 1. A new view (state) is forced ONLY by one of five triggers**, otherwise it's a control: (1) a genuinely different **camera** (can't see both well at once), (2) a **frozen-vs-running** mode split (pause-and-inspect vectors vs let-it-run motion), (3) a **second object / contrast** that can't overlay, (4) an **incompatible operation** (two setups/zoom-scales that can't share a frame), (5) **unavoidable clutter** (the weakest — bias against). **Always controls, never states:** a parameter value (θ=0/45/90), a shape morph (sphere→cube→blob), element visibility, a sign flip, a build-up (assemble/sum), a predict→reveal beat. **Test:** would the teacher JUMP to a new camera/decluttered view, or just DRAG? Jump = state; drag = control. Result is almost always **1, 2, or 3**. Archetype → count: SINGLE-INSTRUMENT/MORPH-FAMILY/ASSEMBLY → 1; TWO-VIEW(two-cameras or direction+motion)/CONTRAST → 2; OTHER(two setups/scales) → 2–3.

**The 41-sim audit (electrostatics + electromagnetism, 6 parallel analysts).** Current: **309 states across 41 sims (avg ~7.5), only ~10% interactive** (one buried sandbox each). Under the rule: **~75 states (avg ~1.8), 100% interactive — ~4× compression.** Distribution: 12 sims → 1 state, 26 → 2, 3 → 3; none > 3. The signature waste everywhere: a long predict→reveal teaching chain authored as states when it's one camera + a slider + a few toggles (the θ=0/45/90 and sphere→cube→blob "states" are the textbook should-be-a-slider collapse). Even the just-built `earths_magnetism` (4→2) and `electric_dipole_in_field` (6→3) over-segment; `potential_energy_in_external_field` is still the old 9-state/1-interactive form (never converted).

> ### The two-layer reconciliation (vs ChatGPT's advice the founder brought in)
> ChatGPT independently reached ~85% of the same conclusion and added real value: the **two-markets reality** (most paying teachers are NOT expert improvisers — a blank sandbox loses them; they want "what do I say next?"), **external evidence** (PhET/PhysPort/meta-analysis: interactivity + teacher guides + scaffolding-varies-by-prior-knowledge), and **progressive control disclosure** (early beat fewer controls). BUT it contradicted its own slider-rule by prescribing "**4–7 beats**" — re-importing the over-segmentation under a new word. **Resolution — these are TWO LAYERS, not a contradiction:** **VIEWS** (structural, physics-driven, 1–3 — the camera/mode/clutter the engine can't combine) and **BEATS** (a UX layer = cheap, editable bookmarks over those views: camera + param preset + a "say this" prompt). My "1–3" counted views; ChatGPT's "4–7" counted beats. The product = **1–3 live views + a SHORT, editable default beat-path** (hide/rename/reorder/save). Average teacher follows the path; expert ignores it and drags. Keep the default beats LEAN — the biased-data risk ChatGPT named (teachers can only use what you ship) argues for *fewer* default beats, not more.

**Roadmap reaffirmed (teacher → collaboration → student AI mentor).** Build the teacher instrument NOW; do not let student/AI-mentor requirements lead. The bridge: **teacher shares a time-limited lesson LINK** (teacher-owned replay/explore — teacher chooses which views/controls/duration, optional checkpoints, whether interaction data returns) → later a **student AI mentor trained from teacher data** (state choices, parameter drags, teaching traces, student replay/confusion). Build telemetry quietly now; the instrument's control surface is the shared substrate so V2 is ~free later.

**The right-hand-rule control decision.** Founder asked: can the teacher move/position the 3D hand freely? **Verdict: the hand should AUTO-FOLLOW v and B** (re-orient live to always show the *correct* rule as the teacher drags) — high value, on-strategy. **Reject free-arbitrary placement** (the hand's whole value is being correct; "any orientation" lets it show a wrong rule and teaches nothing). Optional fast-follow: a green/red "you try" mode (grab the hand, it flags correct-vs-yours). Honest caveat: a rigid hand mesh re-oriented to arbitrary angles looks awkward; the v/B/F **vectors** often teach direction better than the hand (a mnemonic, not the most accurate picture).

> ### The prototype + the discipline (no premature scale-up)
> Built `magnetic_force_moving_charge` **8 states → 4 live bookmarks across 2 views** in an **isolated git worktree** (`physics-mind-proto`, branch `proto/mfmc-instrument`, served :8090) so the founder could click/feel it before committing. ~12 lines of renderer (`isTrusted` slider guard + on-entry slider sync); the rest JSON; the lorentz engine was already a live instrument. Founder reaction: "clean, less stains, perfect compared to old." **My honest verdict: it's the right SHAPE and a real improvement — but "clean prototype" ≠ "validated that teachers love it."** Two reasons not to rebuild all 40 yet: (1) this was the *easy* concept (it fit the existing engine); the model must survive a *harder* archetype (CONTRAST/ASSEMBLY — `gauss_law_sphere`/`biot_savart`); (2) **zero real teachers have touched it** — the one question that decides this (does an average non-expert teacher feel confident with a few live bookmarks, or want more hand-holding?) only a real teacher answers. **Next discipline: ONE real teacher (Asmi) teaches with the prototype + ONE hard concept built this way — THEN roll out.** Not 40 rebuilds on a one-data-point prototype.

## Session 77 — The EXIST consultation + Asmi reframe the product: teacher-facing V1 (teachers ARE the paying customers); the student tutor is the VISION, not the mission; and the reviewer site is already ~80% the teacher product (2026-06-22)

**Two conversations, one conclusion.** Today's EXIST funding consultation and a same-day pricing/feature call with Asmi (the reviewer-teacher) converged independently on the same verdict: PhysicsMind is a stronger product for **teachers** than for students.

**The consultant's read.** Sims + AI professor are good, but: Indian students won't buy (they already have institutions / professors / private tuition; those who can't afford tuition can't afford this either); a student-facing B2C *across countries* is hard because student behaviour and the right *teaching* differ country to country ("how will you handle that?"); "generalise the physics for any curriculum" makes the product easy for a competitor to copy; **building for the German market raises EXIST funding odds**; and the homework — **get clarity: which country, and is this a student-facing or teacher-facing product?** His verdict: "really good for teachers — you can do it for teachers, and fuel revenue in India."

**Asmi's read (the actual buyer).** If ~25 of ~30 physics chapters were built at this quality, would she buy? — "Definitely." Price: ₹500/mo is *cheap* (teachers bill ₹600–1000/hr in private tuition; she floated ₹10k), and she'd pay even running just 15–20 students. The feature she actually wants: not only simulations but a **platform to teach** — reorder/hide the simulation states for her own flow, and **draw on the sims / a whiteboard / sketch pen** to derive over the moving picture. PhET is "really basic"; ours is the thing she can't otherwise get.

**The decision — teacher-facing V1; teachers are the paying customers.** This **reverses Session 73** on the customer axis (which had recast teachers as co-development partners, NOT customers, with B2C students as first revenue) and **re-aligns with Session 65** ("teachers are the first paying customers"). WHO = teachers. WHERE = India + diaspora first (proven willingness-to-pay), EU/German + Western teachers (e.g. Radhika, who teaches in the USA) as natural expansion — **company stays German for EXIST**. WHAT = the simulation library + a live teaching layer. The B2B/B2B2C *institute-sales* motion stays cancelled (teacher-facing = direct-to-teacher).

**Mission vs vision, re-set (founder directive).** The **student product is the VISION, not the mission.** *Vision (the destination):* a personal science tutor for every student who doesn't have one. *Mission (now):* build the **best, excellent teacher-facing product** — physics simulations + a teaching surface that **a teacher uses to explain concepts to their students.** The student-facing AI tutor is **V2**, earned later and reached *through* the teachers we onboard (their students become a warm funnel, not cold CAC). Everywhere the docs framed the student product as the mission, it is demoted to vision; the mission is the teacher tool. (Strategy page + this project page updated; the CLAUDE.md header still reads "AI physics tutor for Indian students" — flagged for founder decision, not auto-edited.)

**Why it's a pivot, not whiplash.** It ships what the product was already designed and built to be. Rule 24 already says "the sim is the teacher's silent visual." And the reviewer site deployed for Asmi (`build_review_site.ts`) is **already ~80% the teacher product**: a reorderable vertical state rail ("open any state, drag to reorder" + Default-order reset — Rule 25d) and a **draw-on-the-simulation pen + a whiteboard panel** are already shipped. Asmi's two asks are largely already built. The "reviewer tool" and the "teacher teaching instrument" are the same surface — this is productising, not starting over. Genuinely new (small): a per-state **hide/skip** toggle, **save-as-a-lesson** persistence, and a polished **Present mode**.

**It also dissolves the consultant's two hardest objections.** (a) *"Different countries teach differently"* → the **teacher localises**; we supply the accurate moving picture, they supply pedagogy, culture and relationship. (b) *"Generalise the physics = copyable"* → physics is universal (one engine, global teacher TAM); the moat is **sim-library depth** (~400–500 sims, 7–9 iterations each) **+ teacher-workflow lock-in**, not curriculum-specific student hand-holding. A generic student chatbot is copyable; a 25-chapter teaching engine teachers build their lessons on is not.

**What makes it excellent — and the discipline.** Where teachers struggle: invisible/3D/abstract concepts (fields, Gauss's law, RHR geometry — the core strength); prep time; **live improvisation** ("what if the angle were 90°?" → live `PARAM_UPDATE`/slider; PhET can't be driven on command); sequencing to their own flow (hide + reorder); live annotation (the pen); and **classroom reliability** (must not stutter on a projector in front of 30 students). Priority order: (0) reliability + a real Present mode; (1) hide-states [new] + reorder [done] + save-as-lesson [new]; (2) live-control superpowers (param/what-if + draw-on-sim [done] + the voice/agent driver from Sessions 72–76); (3) library / my-lessons / lesson-sharing; (4) later, the AI lesson-prep copilot (the Session 72 teacher companion). **The hard truth:** features are ~80% there — what turns Asmi's "I'd buy" into a payment is **coverage (~25/30 chapters) + it-doesn't-break-in-class**, not more features; content velocity (~4 diamonds) is still the bottleneck. Don't build a whiteboard IDE — the differentiator is the sim, not the canvas.

**EXIST framing.** Don't distort the company to one reviewer's "build for Germans" heuristic. The honest, stronger story: a German-built deep-tech engine (geography-neutral IP) + a global teacher market with a credible EU / international-school + Western-teacher path (Radhika/USA proves it — physics is universal, zero rebuild) + proven willingness-to-pay + clear monetisation. If he stays fixated on selling-to-Germans, add a German/EU teacher design partner — don't abandon India revenue.

**Open / next.** Validate ₹500–1000/mo with **3–5 more teachers** (Asmi is one friendly, selection-biased data point). Pour energy into the chapter library. Spec the "Teacher Mode" V1 (hide-states + save-lesson + Present mode on `build_review_site.ts`). Reconcile the strategy page + pilot playbook (still student-B2C-first inside), and decide the CLAUDE.md header wording.

## Session 76 — The voice-professor control layer is generalized: a new simulation becomes professor-drivable as DATA, not engine code (Rule 28). Built data-driven on an isolated branch to protect tomorrow's B2C demo (2026-06-22)

**The founder's question.** Before authoring the next sim (electric field due to an electric dipole), the founder asked the architectural question that had been nagging: the magnetic-force B2C demo's operations / grammar / camera-control machinery — do we **reuse it** for every future sim, or **re-create it each time**? And: write the answer as a rule so every future sim follows it.

**The analysis (magnetic_force_moving_charge).** Two agents mapped the system. Verdict: the control layer was **~80% generic already** — operation vocabulary + validation leash (`operations.ts`), camera-framing engine (`framing.ts`, fully concept-agnostic), beat streaming + API route, client beat player, and the generic field_3d renderer message-handlers are all build-once. But a thin slice was **hardcoded to magnetic**: the knob names/bounds (`theta_deg/v/B/q_sign`), the `set_param`/`freeze` handlers, and the colour legend (`v=orange,B=blue,F=green`). So "reuse vs recreate" was really "mostly reuse, with a per-sim wiring tax."

**The decision: generalize now (data-driven), not branch-per-sim.** Rather than add an `if-dipole` fork for every new concept (re-doing ~15% each time), make the professor's runtime whitelist a per-concept **`ControlSurface`** auto-derived from the concept JSON, so a new sim is **pure data** for the brain. Honest caveat surfaced to the founder: the **3D picture itself** still needs drawing code for a genuinely new scene — that's normal sim-building, not professor work; once the picture exists and lists its knobs/objects/colours, the professor drives it with zero brain-side code.

**Demo-safety mechanic.** Tomorrow's appointment shows the magnetic B2C professor, which runs on the exact files being refactored — and the working demo was **uncommitted** (no revert point). So: (1) committed the working demo as a protective checkpoint (`dfc99f0` on `feat/voice-professor`); (2) did the entire refactor in an **isolated git worktree** `C:\physicsmind-voice-generalize` on branch `feat/voice-professor-generalize` (commit `6681e28`), leaving the demo worktree physically untouched; (3) **merge after the appointment.**

**What shipped (2 files).** `operations.ts`: `GlowTarget`/`ParamName` → strings; `ControlSurface {glow, params, readouts, freeze, resetTrajectory}` threaded through `validateOp`/`validateBeats`/`buildBeatsSchema`/`clampParam`; conditional schema blocks. `professorBrain.ts`: `buildGrounding` auto-derives the surface (knobs from `variables` min/max, glow from per-state + bundle tokens, readouts from `computed_outputs`) with optional `bundle.control_surface` overrides; op-vocabulary prose + colour legend generated from the surface. Renderer deliberately **not** touched.

**Verified magnetic-identical.** `magnetic_force_moving_charge` declares no `control_surface` → auto-derives to the same values. `tsc` 0 errors; deterministic regression 0 ops dropped on all 3 reviewed moves (5/2/8 beats, 27 ops on the full lesson); live brain 0 dropped ops across generative / fast-path / out-of-scope / escalation turns.

**Codified as CLAUDE.md Rule 28** — "Every new field_3d sim ships a Professor Pack as DATA, never new engine code": generic-verbs-over-named-objects (no per-sim ops/grammar), the (28a) concept-JSON / (28b) renderer-scenario / (28c) voice-bundle checklist, and the Definition-of-Done gate (draft → teacher review → lock, Rule 17/18). Extends Rule 27 (addressable primitives). Next: build the dipole as data on this foundation (after merge), plus its reviewer-site version for Asmi.

## Session 75 — Camera-aware AI professor: "the engine is the eyes, the AI is the director." Deterministic framing engine + live auto-reframe on a doubt, built for `magnetic_force_moving_charge` (2026-06-20)

> Context: founder's next ask after the sheet music — the AI professor should *know the viewer's perspective*. If it's about to explain the velocity but the velocity arrow points at the camera (so it reads as a dot), it should recognize that and turn the camera to a view where you can actually see it, saying "let me turn this so you can see the velocity," then explain. "It should judge, okay this camera angle isn't perfect — yeah, this one is, now you can see what I'm explaining."

> ### Decision — the architecture that makes this work (and the one that doesn't).
> The tempting build is "AGI looks at the rendered screen and free-flies the camera until it looks good." **Rejected** — that's vision-in-the-loop (slow, breaks the Session-70 cost/latency model), non-deterministic (every student gets a different camera → the same inconsistency we just killed for the lesson), and it can pick *bad* angles (occluded/degenerate). It's the Rule-18 / Session-72 "free control is the danger" trap pointed at the camera.
> **The right build: the engine is the eyes, the AI is the director.** The renderer already owns the camera *and* every object's 3D direction, so "is this object visible from here?" is a **deterministic geometric fact** — no vision model. An arrow that points along the camera's view axis projects to a dot; one perpendicular to it is fully readable. So `framing badness = |objectDir · viewAxis|` (1 = dot/worst, 0 = full/best). The AI never reasons in pixels — it just picks the best **named** view (a closed, vetted set) and narrates the move. Same "AI picks the input, the verified engine computes the result" safety model as everything else. More advanced than the AGI version, and it's *faster, cheaper, and can't show a wrong angle.* "Advanced" = richness of the view vocabulary + the visibility engine, not a bigger model.

> ### Built this session (worktree `feat/voice-professor` / `C:\physicsmind-voice-professor`, UNCOMMITTED).
> - **`src/lib/voiceProfessor/framing.ts`** (new, deterministic, no renderer dependency, unit-tested) — the view catalog (the renderer's 6 named views as axes), a per-concept object-direction registry (for moving_charge: `v = sinθ·x̂ + cosθ·B̂`, `B̂=+Y`, `F ∝ v×B̂`, `v∥=B̂`, `v⊥=x̂`), `framingBadness`, and `bestViewForObject()` → returns a reframe only when the object is genuinely foreshortened (badness ≥ 0.6) AND a named view improves it by a margin (≥ 0.25); else null (leave the camera alone). Doubles as the offline THE-EYE authoring gate.
> - **The renderer reports the real camera** (`field_3d_renderer.ts`): added `emitCameraView()` → posts `CAMERA_VIEW {axis}` on orbit/zoom-end and when a camera move settles. This was the key honesty move — the renderer has manual drag-orbit, so the only way to know "what the viewer actually sees" is to have the renderer say so. Now the framing judges the *real* viewpoint (manual drag included), not just AI-set named views.
> - **Threaded `currentViewAxis`** client → route → brain (`ProfessorInput`), and a `maybeFramingBeat()` pre-step on the reviewed-cluster doubt fast-path (both `runProfessorTurn` + `runProfessorTurnStream`): if the doubt's focal object (`cluster.glow`) is hidden from the student's current view, prepend a deterministic beat `{ say: "Let me turn this so you can see the <object> clearly", ops: [set_state, set_camera <bestView>] }` BEFORE the reviewed answer. A state-jump doubt judges framing from the state it lands on (its authored camera), not the pre-jump view. The generative path is also made view-aware (a coarse "the student is viewing it <label>" prompt hint).
> - **Rule-18-safe by construction:** the *judgment + view-pick* are deterministic (engine geometry), the *narration* is a fixed safe line, the *answer* stays reviewed. Nothing the AI says about physics is generated.
> - **Verified:** `tsc` 0 errors; framing unit-check — from a top-down view, asking about `v_parallel` (STATE_6) reframes to `edge_on` (badness 1.00→0.00); `v_perp` (already visible), `v` from the authored 3/4 camera, and degenerate `F` at θ=0 all correctly DON'T reframe. **Live demo (founder):** on `:3005`, drag the camera to look straight down the field, then ask "where does the sin theta come from" → the professor says "let me turn this so you can see the part of the velocity along the field," swings to the side view, then explains. Ask about an in-plane object → no reframe.

> ### Staging (same discipline as Sessions 72/74).
> This is the cheap deterministic brick. The same `framing.ts` becomes the offline authoring gate (flag "you're explaining v but it's foreshortened in this beat" when building sheet music) — near-free, compounds. The full live conversational camera-direction loop only gets *exercised* once the student-facing professor exists; keep it deterministic, never vision-in-loop. Deferred: generalizing the object-geometry registry to other concepts (added per-concept as field_3d concepts land), new named-view *positions* (the 6 cover see-the-circle vs see-along-B), and any free spatial reasoning (explicitly out — wrong architecture).

> **Candidate CLAUDE.md rule (pending founder approval — do NOT auto-add):** *"Camera/view is a verified operation, not free AI control. Object visibility is computed deterministically (engine geometry); the AI names the object to frame and the engine computes the angle — it never free-flies the camera or reasons in pixels."* Generalizes to every field_3d sim; forget→build-wrong (vision-in-loop is the tempting trap); corroborated by Rule 18 + Session 72's verified-operation-vocabulary doctrine.

> ### Addendum (same day) — made it FULLY FLEXIBLE: continuous angle + any concept (founder: "did you hardcode it or is it flexible?").
> The first cut was bounded two ways the founder rejected: it snapped to **one of 4 named views**, and it only worked for moving_charge (object-direction formulas hand-written in `framing.ts`). Upgraded to remove BOTH hardcoded layers — and it's a clean upgrade, not a rebuild, because the renderer already accepts arbitrary camera positions (`animateCameraTo`) and already computes every object's direction each frame.
> - **Continuous angle (new `frame_object` op).** The AI names an object (`frame_object {object}`); the renderer reads that object's LIVE world direction and computes the *exact* camera perpendicular to it (object lies broadside = fully visible), closest to the current camera for a minimal glide — with a clean-perpendicular fallback for the degenerate "looking straight down the arrow" case (the founder's demo). Not a preset; a bespoke computed angle per object. (`set_camera` named views stay for the lesson + "look down the field".)
> - **Any concept (renderer-reported directions).** The renderer caches each object's direction per frame and reports them (`OBJECT_DIRS` postMessage, ~3/sec + on state change). `framing.ts` **deleted its per-concept geometry** (`CONCEPT_GEOMETRY` + the named-view picker) and now decides off the reported directions — concept-agnostic. A new field_3d concept works once its scenario caches the directions it already computes (moving_charge now; biot/electric/wire later, per-scenario).
> - **Still Rule-18-safe:** the AI picks the *object*; the engine computes the *angle* from verified geometry. "Flexible" ≠ "AI guesses raw camera coordinates."
> - **Files:** `operations.ts` (frame_object op), `field_3d_renderer.ts` (objectDirections cache + OBJECT_DIRS emit + `frameObject()` continuous-angle math + FRAME_OBJECT handler — all backtick-free, inside the template literal), `framing.ts` (generalized to reported dirs; per-concept geometry removed), `professorBrain.ts` (`ProfessorInput.objectDirections`; `maybeFramingBeat` → frame_object; generative prompt gains frame_object), `route.ts` + `VoiceProfessorClient.tsx` (carry `object_directions`).
> - **Verified:** `tsc` 0; `framing.ts` no longer references the concept (grep clean); frameObject geometry unit-checked — vertical `v_parallel` from top-down → side cam (badness 1.0→0.0), v@45° → a *continuous* `[0.50,−0.50,7.97]` (proves non-preset), already-visible objects → near-zero move. **Pending founder live THE EYE** (drag to look down the field, ask "where does the sin theta come from" → continuous glide to a side angle; frame two different objects → two different bespoke angles).
> - **Known nuance:** `v`/`f` directions rotate as the particle orbits, so framing them uses the direction at request time (approximate); the stable targets (`v_parallel`, `v_perp`, `b`) frame exactly.

> ### Addendum 2 (same day) — visibility coupled to EMPHASIS: the engine guarantees you can see what's being explained (founder screenshot: v sin θ explained but foreshortened).
> The founder caught the real gap: in the *generative* explanation of State 6, the professor glowed and explained `v sin θ` but from the dragged camera that arrow was nearly a dot — "the AI doesn't know the viewer can't see it." Root cause: framing fired only on *doubts* (the cluster pre-step), never on the lesson/generative explanation, and only once. **Principle adopted: *to point at an object is to guarantee it's visible* — couple framing to FOCUS (glow/emphasis) and enforce it continuously in the engine** (the only place with the live camera + live directions every frame).
> - **Built (renderer-only, additive):** `autoFrameEmphasis(tokens)` fires on every `SET_GLOW`/`SET_EMPHASIS` — so it covers lesson + generative + doubt uniformly, no AI effort. It frames the highlighted **SET** (`set_glow` already accepts arrays — multi-highlight was always supported and is used, e.g. the decomposition glows `["v_perp","v_parallel"]`). `bestCameraForDirs(dirs)` samples the sphere (~264 candidates) for the view whose WORST-foreshortened object is acceptable, nearest the current view (minimal move); for a **coplanar** set (the decomposition) the plane-normal view wins → all components visible at once. `frame_object` refactored onto the same engine.
> - **Gated so it's never jumpy:** two thresholds (hysteresis) — `FRAME_GATE 0.5` = only move when an object is meaningfully hidden; `FRAME_TARGET 0.34` = a destination must be genuinely well-framed (verified: it lands ~0.26 = ~97% of each arrow's length visible). Plus: skip while the user is **dragging**, skip for ~1.5 s after a **deliberate** `set_camera`/`frame_object` (so authored/AI camera choices win — `explicitCameraAt`), debounce the same set (`lastFramedKey`, reset on state change). Per-state authored cameras do NOT suppress (a default may still be rescued if it hides the focus).
> - **Verified:** `tsc` 0; `bestCameraForDirs` unit-checked — decomposition `{v_parallel,v_perp}` and the 3-vector coplanar set from a bad top-down view → a view where every component is ~0.26 badness (clearly visible); single vector likewise. **Pending founder live THE EYE** (run Whole-concept / generative to the decomposition → camera auto-turns so v sin θ + partners are clearly visible, the screenshot fixed; drag to a bad angle mid-explanation → next glow self-corrects; authored `face_on` beats not overridden).
> - **Tuning point flagged:** rapid successive single-object glows (v → v∥ → v⊥) can swing the camera between them; the gate + minimal-move + debounce keep it reasonable, but if it feels busy in THE EYE, raise `FRAME_GATE` or add a short cooldown. Co-glowing the set (one `set_glow` array) frames all at once with no swing — the preferred authoring.

> ### Addendum 3 (same day) — every doubt is TAUGHT carefully from the first ask (was: terse first answer, careful only on a repeat).
> Founder: after the concept finishes, a doubt got "a short answer with not much controlling/manipulating." The careful "slow down, pause/play, highlight, change camera, explain" treatment was gated behind *escalation* (asked twice) or an explicit "slow down." Wrong: it should be the **default on the very first ask** for any question; escalation then goes *even further*. Changed (`professorBrain.ts`):
> - **Known doubt (verified-cluster match): `richClusterBeats`** replaces the single terse beat — jump to the state, highlight the focal element (the renderer auto-frames the camera onto it), **pause** to hold the key moment while explaining the first point, **resume** with the rest. The spoken words stay the founder-verified answer, paced across measured beats. Still $0, deterministic, guaranteed-correct — just delivered as a mini-lesson.
> - **Unknown / manipulation doubt (generative):** every doubt now runs at **effort "high"** (was "medium") with a generous budget (2800 tok; escalation 3500), and `buildUserTurn` carries a **careful-teaching protocol on every doubt** (go to the state, pause, highlight→auto-frame, *show* with set_param/sweep_param, several measured beats, resume) — not only on a repeat. A matched cluster answer is passed as faithful **grounding** even when we go generative.
> - **Escalation** = re-teach from a *different* angle, even slower (set_speed 0.3–0.4), more concrete analogy, more small steps — "maximum effort," not a repeat of the first explanation.
> - **Cleanup:** the separate `maybeFramingBeat` doubt pre-step (spoken "let me turn this…") was removed as dead — the renderer's auto-frame-on-glow now handles the camera for doubts uniformly. Verified `tsc` 0; `richClusterBeats` produces the pause→explain→resume structure. Pending founder live THE EYE. (Cost note for the future student-facing phase: every doubt is now careful/high-effort; a fast terse tier can return then if cost demands.)

> ### Addendum 4 (next day, 2026-06-21) — known doubts were still SHORT: serve the verified answer as GROUNDING, teach it generatively in depth (not verbatim).
> Founder screenshot: asked "what is the reason of the helical path?" → got a 3-sentence answer (the sim moved, but the words were thin). Cause found: the question matched the verified cluster `other_angles_helix`, so it hit the reviewed-cluster path → `richClusterBeats` **paced** the verified answer — but the verified answers are intentionally **one-liner-brief**, so pacing a short answer is still a short answer. The careful *depth* only lived on the generative path (unknown questions); a *known* question got the concise verified text. **Fix:** route EVERY doubt (known or unknown) to the **deep generative teaching**; the matched verified answer is passed as **grounding** (keeps the physics faithful) and is the **graceful fallback** if generation yields nothing — it is no longer served verbatim as the primary response. Also strengthened the teaching protocol to demand a *thorough step-by-step that builds the idea up* (5–8 beats, explain the WHY, "err on the side of explaining too much"), and raised the beat-count guidance (doubt = 5–8 beats, never one terse beat). `tsc` 0. **The principle:** verified clusters are the *correctness anchor + fallback*, not the *teaching* — the teaching is the generative mini-lesson grounded in them. (Reinforces the teacher-facing "GENERATIVE · TEACHER-VERIFIED" model; cost = one Sonnet call per doubt, accepted for the demo.)

---

## Session 74 — The two-phase delivery model: the full-concept explanation is an AUTHORED deterministic "sheet music" performance (not live-generated); the doubt session stays live. First sheet music built for `magnetic_force_moving_charge` (2026-06-20)

> Context: founder watched the voice-professor demo and called the state narration "lame" — the professor's voice plays *over a looping sim that doesn't do anything in response to the words*. Worse, clicking **▶ Whole concept** routes to the generative brain, so every student gets a *different* explanation and a *different* manipulation. This session diagnosed the root cause, settled the delivery model, defined who authors/verifies the lesson, and built the first one.

> ### Decision 1 — Two phases, and which one is fixed is the whole point.
> - **Phase 1 — "Explain full concept" = ONE fixed, authored, reviewed performance ("sheet music").** The AI professor speaks each line *while driving the sim* (glow, sweep θ, dim, announce engine-computed values, camera), state by state. **Identical for every student.** Deterministic, served verbatim, $0.
> - **Phase 2 — the live doubt session** (after; interruptible-then-resume, founder's pick) stays the *generative* brain — per-student variation is fine and wanted there.
> The founder's worry ("every student gets a different manipulation") is true **only if Phase 1 is generated**. The fix is one line: *the full-concept explanation must be PERFORMED from authored content, never generated* — which is already the architecture (Rule 18; Session 69 Decision 3 "one professor voice performs verified content for the lesson, orchestrates the live tail for doubts"). The founder re-derived it. The implementation simply hadn't realized it: **`intent: "explain_whole"` was wired to the generative path.** That *is* the bug.

> ### Decision 2 — Why it felt "lame": authored states *reveal*, they don't *manipulate*.
> The authored EPIC-L states show static glow reveals; the only real manipulation lived on the lone explorer state (STATE_8 sliders) and inside the *doubt* moves. So the lesson read as a voiceover on a loop. The sheet music makes **manipulation the baseline lesson** — the θ-sweep that opens the circle into a helix, the slow-motion equal-arc trail, the v-decomposition — performed as the spine, not as a reaction to a typed doubt.

> ### Decision 3 — Governance: who writes the sheet music, who verifies it.
> Same three roles as diamond authoring, pointed at a new artifact. **Claude drafts v1** (seed-then-refine, Session 68 — you can't capture usage of a performance that doesn't exist yet) → **Asmi is the pedagogy gate** (she catches *wrong-teaching-order-with-right-physics*, the defect only a practising teacher sees) → **Pradeep approves** (Rule 17). Key realization: the sheet music *is a richer version of what Asmi already produces* — her recommended state-flow + per-state notes (Session 71 Decision 4) become the manipulation choreography, not just the state order.

> ### Decision 4 — The review process evolves with it.
> Today Asmi screen-records teaching a student, then audits each static state. The shift: **her manual teaching IS the sheet music performed by a human** — her hand on the slider, her state jumps (Session 72: teacher-drivable IS machine-drivable; "the explorer slider is Asmi's hand today, the AI's tool call later"). So her screen-recording becomes the *source* of the performance, not just a bug-audit; her review question shifts from "is this state correct?" to "is this *performance* right — right manipulation, right order, right beat?" **Cheap-now:** transcribe her manipulation from the recording by eye. **Instrument-later:** log her ops directly. Her ₹500 buys *more* now — her teaching motion becomes the reusable, machine-replayable lesson.

> ### Built this session (uncommitted, worktree `feat/voice-professor` / `C:\physicsmind-voice-professor`).
> - **`full_lesson_walkthrough` move** authored in `src/data/voice_professor/magnetic_force_moving_charge.json` — 8 beats (one per state, authored order 0°→90°→no-work→Fleming→45°→why-sinθ→10°→interactive). Each `say` condensed from the already-reviewed `teacher_script` (pure transcription, no new physics); each beat = `set_state` first, then manipulation: STATE_2 face-on circle + announce F; STATE_3 dim-except + slow-mo (no-work); STATE_5 **sweep θ 90→45** (circle opens to helix); STATE_6 dim-except v⊥/v∥ (decomposition); STATE_7 **sweep θ 45→10** (helix stretches); STATE_8 sliders handoff.
> - **Reroute** in `src/lib/voiceProfessor/professorBrain.ts` — added `resolveLesson()`; `intent: "explain_whole"` now resolves to the reviewed move and is served verbatim via the existing move fast-path (deterministic, $0, server-side `attachReadout` so announced numbers can't drift), in both `runProfessorTurn` + `runProfessorTurnStream`. The old generative `explain_whole` config remains as an unreachable graceful fallback (missing move → generate).
> - **No client change needed** — the existing **▶ Whole concept** button already calls `runTurn("explain_whole")`; the SSE consumer + `playBeats()` already play streamed reviewed-move beats.
> - **Verified:** `npx tsc --noEmit` = 0 errors; sidecar JSON valid; all 8 beats within MAX_BEATS=12 / ≤6 ops / ≤300 chars; every op uses a valid verb/target/range/state/readout (validateBeats drops nothing → plays full). **Pending:** founder THE EYE (live visual — does each manipulation land on the right word, do the two θ-sweeps read) → then Asmi's review → lock.

> ### Constraints flagged.
> - **MAX_BEATS = 12 ceiling** (`operations.ts`). First cut condenses to 8 beats; if Asmi says the condensation loses too much (STATE_2/STATE_6 each have ~14 authored sentences), raise the limit *for authored reviewed moves* (founder-approved content doesn't need the runaway-generation guard).
> - **Barge-in mid-walkthrough DEFERRED** — this builds the deterministic spine that plays through, then opens doubts. Wiring pause-on-speech→handle→resume is the Phase-2 follow-up (founder chose interruptible-then-resume).
> - **STATE_8 sweep caveat** — θ sweeps re-render the trajectory in STATE_2/3/5/7 but NOT STATE_8 (its on-screen sliders own θ); climax sweeps correctly live in STATE_5/STATE_7. Sweeps pass an explicit `from` because `set_state` resets the override.
> - **Scope:** existing `magnetic_force_moving_charge` only. New diamonds get richer manipulation natively (Rule 27); no retrofit of other diamonds.

> **Candidate CLAUDE.md rule (pending founder approval — do NOT auto-add):** *"The full-concept walkthrough is authored + deterministic — one reviewed performance, identical for every student; only the doubt session may be live-generated."* Likely already covered by Rule 18 + Session 69 Decision 3; flag for the founder to decide whether it earns its own rule number.
...
> ### Decision 1 — A teacher-facing AI companion is the SAME product (a new retention surface on the same substrate), not a second product — and it's deferred until teachers feel the prep friction.
> Founder's question: we build sims for teachers; can we add an AI *companion for the teacher* (distinct from the planned B2C student professor)? Is it a new product? **First, the conflation to clear:** the Session 68–71 "AI professor" is **student-facing** (orchestrates the sim + answers the student). A teacher companion is a **different user, different job** — it helps the *teacher*, not the student. Don't blur them.
> - **Same product, new surface, not a new codebase.** It runs entirely on assets we already manufacture — the concept dependency graph, misconception clusters, reviewer state-flows (Asmi's notes), the sim library. Architecturally it's a thin chat/sidebar layer that *reads* the substrate. Strategically, though, treat it as its own surface with its own question: *does it make a teacher keep paying?* → it's a **B2B-India retention/stickiness lever.**
> - **What it could realistically do, sorted by fuel:** buildable today on data we already have — **lesson prep** ("teaching magnetic force tomorrow — which sims, what order, what do students trip on?" answered from the graph + misconception_watch + Asmi's recommended flow; the strongest one because it just serves back authored data), **worksheet/quiz generation** per concept (cheap text-LLM, cacheable, no physics-in-loop risk if pulled from verified definitions), **misconception-trap coaching.** Illusion right now (needs data we don't have) — **"which of my students struggled"** (zero students → a dashboard on an empty table), **real-time co-teaching whisper** (that's the student-professor problem wearing a teacher hat; premature).
> - **Verdict — don't build now.** Same fuel-starved trap the 2026-06-18 stage rule and Rule 17 already name. Its best features are starved until teachers use sims at classroom volume and feel the prep friction; the cheapest version (lesson-prep helper) is near-free to bolt on *once a teacher asks for it*, so waiting costs nothing and cornering is zero. Trigger to build the lean prep companion: a teacher in a real deployment says "I wish this also helped me plan / quiz / see where they trip."

> ### Decision 2 — Fully-dynamic, per-primitive control for the voice professor is possible and is the right architecture — but the flexibility comes from a rich VERIFIED-OPERATION vocabulary, not free-form AI control of geometry.
> Founder's question: should every primitive (charge, field, force vector, helical path, the right-hand-rule hand — smallest to largest) carry a "placeholder/handle" so the voice professor can move/orient it freely at runtime to explain to the student? Is highly flexible dynamic control possible? **This is exactly Capability A (Session 68 addendum).** Yes — with one reframe that decides whether it works or becomes a disaster.
> - **The dangerous word is "freely."** If the agent can orient the RHR hand anywhere, it can orient it *wrong* for a given current and teach incorrect physics live — the Rule 18 hard floor. What's actually possible and safe: **each primitive exposes a set of verified *operations* (not free control); the agent invokes operations; the operations are physics-constrained so they can only produce correct poses.** Flexibility = richness of the operation vocabulary, not a free hand on the geometry.
> - **The RHR example, concretely:** the hand doesn't get "move anywhere." It gets gestures bound to the physics already in the scene — `show_rhr_for(current_id)` (snaps to the *one* physically-correct pose, determined by the current vector that already exists), `rotate_camera_to_face(plane)` (shows that same correct hand from a clearer viewing angle — *this* is the "move here and there" the founder means: moving the viewpoint, not faking new physics), plus `align_to`, `bring_to_common_origin(v,B)`, `draw_angle_arc(θ)`, `decompose_into_components`, `highlight`. Vectors are real; only the *display gesture* is dynamic.
> - **What "a placeholder on every primitive" actually requires (the buildable spec):** (1) a **stable addressable ID** per primitive (`charge_1`, `B_field`, `force_vector`, `helix_path`, `rhr_hand`); (2) an **exposed set of parametric operations** surfaced through the existing postMessage / `PARAM_UPDATE` / slider contract; (3) **physics constraints baked into each operation** (deterministic layer / mathjs computes numbers; the operation enforces geometry). **We already have the seed:** the `biot_explorer` pattern — a dedicated hidden-by-default object set, RESET-to-built-pose on every state entry (so edits don't leak), driven by sliders (I/r/θ → moves P, circle, element, r̂, dB) — *is* per-object addressable, physics-constrained dynamic control. The explorer slider is Asmi's hand today; it's the AI's tool call later. Teacher-drivable IS machine-drivable (Session 69, Bucket 1).
> - **Honest limits (no illusion):** coverage = richness of the operation library, **never 100%** (can only show what an operation expresses — e.g. can't show the field produced *by the charge itself* if the sim models only the external field; biot already hit this); grows monthly via confusion→new-operation (Rule 17 meaning 1). **3D is where it sings, 2D is cramped** — the dynamic-control future is a `field_3d` future. It's a **multi-year harness build, fuel-starved today** (zero students) — Session 68 already called it tractable-not-trivial / multi-year.
> - **Current reality (verified 2026-06-19):** Label/annotation primitives in `scene_composition` DO have stable IDs (`theta90_label`, `circle_label`, `r_T_label`, `no_work_label` in `magnetic_force_moving_charge`) — `focal_primitive_id` references them. But the actual physics objects — the proton, velocity vector `v`, magnetic field `B`, force vector `F`, RHR hand, helical trail — are **renderer-internal**: referenced only as glow tokens in `teacher_script` (`glow: "v"`, `"b"`, `"hand"`, `"trail"`) and never appear as schema-level primitives with addressable IDs. The `biot_explorer` object set IS per-object addressable + physics-constrained + controllable, but it's bespoke to that one state — not a general system every object inherits. Minor note: `scene_composition` in this diamond is stored as a numeric-keyed JS object (`{"0":{...}, "1":{...}}`) rather than a `{primitives: [...]}` array — a schema inconsistency that doesn't break serving but contradicts Rule 19's framing. **Consequence:** the discipline starts NOW with new work only; don't retrofit existing diamonds (fuel-starved, low ROI). The correct build rule: build new manipulable objects the **explorer way** (named set, RESET-to-built-pose on every state entry, slider/postMessage driven) rather than baking one-offs into the renderer as hardcoded tokens.
> - **Verdict — don't build the agent control engine now; build the addressable-primitive discipline now (near-free while authoring anyway):** every diamond from here on gives its primitives **stable IDs + key parameters exposed through the slider/postMessage contract** (the explorer pattern). Static drawings with no handles corner us; addressable + parametric primitives make every shipped sim *also* a brick in the dynamic-control wall, with no rework. Build the engine when there's a student professor to use it.

> **Net:** both questions resolve the same way — real future layer, architecture already correct, author the cheap foundation now (lesson-prep is near-free later; addressable primitives are near-free now), light up the engine when fuel (students / a deployed teacher's stated need) exists. Neither changes today's authoring except Decision 2's standing discipline.

> **Rulebook promotion proposed (pending founder approval):** Decision 2's addressable-primitive discipline is a candidate **CLAUDE.md build rule** — *every new primitive ships with a stable addressable ID and exposes its key parameters through the postMessage/slider control surface* — it generalizes to all sims, forget→build-wrong (un-addressable primitives corner the dynamic-control future), and is corroborated by the existing `biot_explorer`/explorer-state pattern + Session 68 Capability A. Decision 1 (teacher companion) stays in DISCUSSIONS.md only — it's a deferred-with-trigger product call, not a per-sim build rule.

---

## Session 71 — The playback model (master clock, mute/pause decoupled from TTS), the TTS-review policy, and the review-app UX (vertical state rail + reorder) (2026-06-17)

> Context: prepping the first electrostatics sim (**electric field due to a point charge + field lines** — Asmi teaches it to a student *tomorrow*) surfaced how a teacher actually *operates* a sim while teaching, and how reviewers feed data back. Four decisions, all extending the Session 69 "sim is the teacher's silent visual" law and the reviewer→rulebook filter.
>
> ### Decision 1 — The reviewer protocol + the ground-truth inversion (the data engine).
> **The teacher's method is the ground truth; the simulation adapts to them — never the reverse.** A teacher explains in *their own* style/derivation/order, on their own whiteboard, and opens the sim to whatever state they want to *show* the student what's happening. The sim is a visual aid they point at, **not a script they follow.** If a teacher ends up teaching "according to our sim," the product has failed. Consequence: **every state must read correctly standalone** (teachers jump states in their own order). The reviewer fleet (built over ~3–4 months): **2 "deep" reviewers** (Asmi is #1) — teach a student with the sim → record → **plus** a per-state Excel/notes audit (present / missing / correct / wrong); **3 "usage" tutors** — just use the sim in normal teaching → record the class only. 5 total. We mine all 5 recordings + the 2 Excels for patterns → update sims. **Now: 1 (Asmi).**
>
> ### Decision 2 — Playback model: one master clock, audio rides on top.
> The root issue: reveals were conceptually tied to TTS playback — the same coupling behind our headless-capture timing bugs. **Fix:** each state has its own timeline clock (`PM_simTimeMs`, already present on field_3d) that drives **all reveals/motion/choreography**; reveals never *wait* on a TTS event. Audio rides on top. **Mute** = silence audio output only (clock + reveals continue). **Tap-to-pause** = freeze clock *and* pause audio together. **Tap/resume** = both continue from the same position (TTS resumes where it left off). field_3d reveals already run on the sim clock → already mute-safe; only tap-pause/resume + audio-only-mute need wiring. Refinements adopted: per-state **scrubber/progress bar**, **mute is the default and persists across states**, **replay** button, **spacebar = pause/resume**.
>
> ### Decision 3 — TTS authoring + review policy.
> **Author the `teacher_script` for every sim; keep it OFF (muted) by default; it is never the teacher's script** (Rule 24d). It's worth authoring because it's (a) the B2C seed — one AI-professor voice replaces TTS later, (b) the reveal-timing skeleton, (c) free expert QA. **The simulation is ALWAYS the primary review.** TTS review splits by reviewer type: the **2 deep reviewers** give the script a *separate, optional, secondary* correctness pass explicitly labeled *"audit this for physical correctness — this is our future AI voice; you will NOT teach from it"* (kept a distinct checkbox so it never bleeds into "is the sim right?" and never tempts teaching-from-script); the **3 usage tutors ignore TTS entirely** (mute and teach).
>
> ### Decision 4 — Review-app UX: vertical state rail replaces next/prev.
> Kill next/previous navigation (waste of time). Replace with a **vertical, scrollable rail of state cards** (label + tiny preview); click → canvas jumps to that state instantly (existing `SET_STATE` any-order contract). **Drag-to-reorder** the rail → that's the teaching order; **"Apply"** sets it, **"Default"** resets to canonical; order persisted locally (per-professor profiles deferred until 5 tutors exist). Safe *because every state is standalone* (Decision 1). **The default-flow loop:** each reviewer writes a **"recommended state flow"** in their Excel/notes → we bake it into the JSON as the **canonical default order** → later professors open that default and locally reorder. The reviewer→content compounding loop, made concrete.
>
> ### Sequencing.
> Don't block tomorrow's class on the UX refactor. **Ship `electric_field_point_charge` on the current viewer for tomorrow** (after verifying mute-safe reveals + tap-pause work — likely already fine on field_3d). **Then** the review-app UX upgrade (vertical rail + reorder + scrubber + persistent mute) as the next workstream — a one-time change to the review-site template that benefits *every* sim and every re-review.
>
> **Rulebook promotions proposed (pending founder approval):** new Rule 26 (playback clock + mute/pause contract), new Rule 27 (every state standalone), a §6 state-order schema note (canonical default order ← reviewer flow). Reviewer-fleet model + TTS-review policy → `reviews/README.md` (process, not per-sim build). Review-app UX → build doc / PROGRESS.

---

## Session 70 — Voice-professor unit economics: the cost + latency models, decomposed from a real call (2026-06-16)

> Context: founder shipped a real AI voice receptionist (Vapi, restaurant "Due Fratelli", German) and pulled the live numbers — a **2m46s call cost $0.22**, the assistant config shows **~$0.08/min** and **~1,200ms** voice-to-voice. Stack: **Deepgram nova-2 (STT) + Claude Haiku 4.5 (LLM) + Azure TTS (voice)** — deliberately NOT ElevenLabs. Founder's question: in ~1–1.5y, if the agentic *professor* uses the same kind of TTS/STT/LLM stack, does a 30–40 min real-time concept session cost the same / have the same latency — or does strong harness engineering change it? Both turned out to decompose the same way, and **both reduce to one lever.**
>
> ### Cost — the platform tax is the whole story.
> The dashboard itemizes $0.08/min: STT $0.01 + LLM $0.01 + TTS $0.01 = **~$0.03/min of actual AI**; the remaining **~$0.05/min (~64%) is Vapi's orchestration platform fee.** So the receptionist isn't paying for intelligence — it's paying *rent*. (Azure-over-ElevenLabs already saved ~10× on the voice line.)
> - **Naive professor (same Vapi stack, 40 min):** 40 × $0.08 = **$3.20/session** — the scary number, and why this feels expensive.
> - **Why it isn't that:** a receptionist is the *worst case* — it can't cache anything (every call is a unique live-generated conversation). The professor is the opposite: an **orchestrator of pre-authored, cached assets**, which restructures 3 of the 4 cost lines:
>   - **Platform fee → ~$0:** self-host the transport (LiveKit / Pipecat) instead of renting Vapi. Kills the dominant ~$0.05/min.
>   - **TTS → mostly cached:** ~60–70% of the professor's speech is *authored narration* → synthesize ONCE, serve cached audio forever; live TTS only for off-script doubts.
>   - **LLM → intermittent:** one cheap planner call picks "play sim state 3 + its cached audio," then **no LLM runs during playback**; Haiku-default, Sonnet only for hard doubts.
>   - **STT → VAD-gated** to when the student actually talks (~40% of a lecture).
> - **Engineered 40-min session (estimates):** STT ~$0.08 + live TTS ~$0.12 + LLM ~$0.30–0.50 + self-hosted transport ~$0.20–0.40 ≈ **$0.70–1.10 (~$0.02–0.03/min)** — **3–5× cheaper than naive, despite a far longer session**, and it *drops further as the cached library grows* (the receptionist's never does). A well-built sim also does the visual heavy-lifting, so real clarifications are often 10–15 min, halving it again.
> - **The one trap:** do NOT move to a realtime *multimodal* LLM (GPT-4o-realtime / Gemini Live) — they bill **audio tokens** (~$0.06/min in + $0.24/min out ≈ $0.30+/min just for the model), turning a 40-min lesson into $12+. The **cascade** (audio→text→text-LLM→audio) the founder already chose is the correct, cheap architecture. Keep it.
>
> ### Latency — bimodal, and the same lever solves it.
> Receptionist's 1,200ms decomposes as STT ~100 + **LLM ~800 (the bottleneck — and that's *full-response* time)** + TTS ~200 + endpoint/net ~100. Reference: <500ms feels human, 600–900ms natural, >1,300ms laggy. **Free win, zero stack change: stream + sentence-pipeline** — fire the first clause into streaming TTS the moment it's ready, so audio starts while the model is still generating → perceived ~1,200 → ~700ms.
> - **The professor's latency is bimodal** (like its cost): **scripted playback (~70% of the lesson) = cached-audio fetch ≈ 0–50ms**, instant and smooth, no STT→LLM→TTS loop at all; the *only* latency-critical path is the **~30% off-script doubt/interruption turns.** You're not fighting 1,200ms across 40 min — only on a handful of live turns.
> - **Doubt fast-path budget (~600ms):** stream+pipeline · **smart/semantic endpointing** (don't over-wait to confirm they're done — saves 200–400ms) · **Haiku-first + acknowledgment filler** (tiny cached "Good question —" played instantly to mask think-time, like a human) · **predictive pre-fetch** (we already have the **misconception clusters per state** — pre-compute + cache the 3–4 most-likely doubts at each sim state → a predicted doubt answers *instantly*) · **co-located / regional infra** (for Indian students, US-hosted model endpoints add ~150–250ms each way — pick Asia/India endpoints or self-host regionally) · **barge-in <100ms** (transport-layer job — LiveKit/Pipecat do it natively; another reason to drop Vapi). Budget: endpoint 200 + STT 100 + Haiku TTFT 350 + TTS TTFB 150 + net 100 ≈ **~700ms**, tightening to **~550ms** with pre-fetch hits.
>
> ### The unifying insight (orchestrator-not-generator pays off a THIRD time).
> **The quality lives in the cached assets (authored offline, verified — Rule 18). So the *live* model only has to be FAST, not smart.** You run Haiku live (fast TTFT, cheap) with no quality loss, because correctness + depth were baked into the verified sims/scripts beforehand; the rare Sonnet-grade doubt hides its extra ~500–800ms behind an acknowledgment filler. **Cost and latency are not two problems — they're the same lever: caching what's authored.** Cascade + cache + self-host + model-route gives **instant on ~70% of turns and ~600ms on the rest, at ~10× lower cost** than a uniform realtime-multimodal stack. The more the professor leans on the pre-authored verified library (the moat), the cheaper AND faster it gets; a live-generating receptionist stays stuck at $0.08/min and 1,200ms forever. (Directly extends Session 68's orchestrator architecture + cost addendum.)
>
> **Open build items flagged:** (1) the narration-audio cache layer (synthesize authored `teacher_script` once, key by concept|state|voice); (2) the self-hosted realtime transport choice (LiveKit vs Pipecat) when the professor build starts; (3) per-state doubt pre-fetch wired off the existing misconception clusters; (4) regional model-endpoint selection for India latency. None needed now — they bolt onto today's authored substrate later (Bucket 2, Session 69).

---

## Session 69 — The V1 design law ("the sim is the teacher's silent visual"), the B2C delivery decision (one professor voice replaces TTS), and the reviewer→rulebook promotion filter (2026-06-16)

> Context: choosing the next concept (A15, force on a current-carrying wire F=IL×B — the direct child of the moving-charge sim in the Ch.36 dependency graph, and the prerequisite for A18 parallel-currents) triggered a deeper question — *what should a V1 sim actually contain, given the audience is teachers who teach WITH it?* That unfolded into three durable decisions, all promoted to doctrine.

> ### Decision 1 — The V1 design law: the sim is the teacher's silent VISUAL, not their script or whiteboard.
> A teacher teaching with our sim brings three things: **their voice** (the explanation), **their whiteboard** (the algebra/derivation), and **our sim** (the moving, accurate, manipulable picture). The mistake we were half-making was letting the sim try to be all three — on-canvas prose statements (= doing the teacher's talking) and text derivation walkthroughs (= doing the teacher's whiteboard). Asmi's feedback was exactly this: *"don't write physics statements, I know physics, I'll just look at the sim"* — equations and derivations are fine, prose sentences are not. The resolution:
> - **Derivation:** out as algebra, in as a *picture*. Don't show F=IL×B as lines of math (whiteboard = teacher's job); DO visualize the physical picture the derivation is about (charges drifting → per-charge q(v×B) arrows summing to one wire force → the result equation). The teacher derives on the board *while pointing at the sim that shows why it's true.*
> - **Concept vs derivation:** concept-driven visuals, equation shown but lean. The high-value A15 content (L–B angle trap, ∫dL=displacement / bent-wire-becomes-its-chord, loop net-force-zero) is *inherently visual* — exactly what a sim does and a whiteboard can't.
> - **TTS:** off by default for teachers (they narrate; Asmi called TTS "too fast"), and the visual must read correctly with sound OFF. **BUT always author the `teacher_script` + reveal-bindings even unplayed** — it's the cheapest seed of the B2C orchestration-policy corpus and near-free to author alongside the sim. Carry, don't force.
> - **The catch we caught:** on-canvas prose + auto-TTS are features built for *a student watching alone* (the B2C destination, which doesn't exist yet) — dead weight in the teacher-with-sim product. Asmi flagged exactly this. → **Promoted to CLAUDE.md Rule 24.**

> ### Decision 2 — Does the lean V1 hurt the future B2C AI professor? No — it BUILDS its foundation, with one rule.
> The AI professor is an ORCHESTRATOR of verified assets (Session 68), not a live physics generator. So the only question is whether lean-V1 starves or feeds the assets it will later play. Three buckets:
> - **Bucket 1 — day-one load-bearing, and lean-V1 *strengthens* it:** the control API (teacher-drivable IS machine-drivable — the same postMessage/slider surface the AI steers), the verified visuals, and the authored `teacher_script` + reveal-bindings (the teaching-policy seed).
> - **Bucket 2 — deferrable, additive, zero cornering:** full algebra-derivation templates (the B1 capability), mathjs substitution, the orchestrator harness, real-time voice, TTS playback. All bolt on top of the verified substrate later with no rework.
> - **Bucket 3 — the ONE trap:** don't let "lean" mean *delete* the narration script + bindings. That's the only choice that would actually hurt B2C; it's near-free to keep, so we keep it.
> **Verdict:** lean teacher-V1 is the *manufacturing process* for the AI professor's substrate, not a competitor to it. The only discipline: author the script + bindings even when unplayed.

> ### Decision 3 — B2C delivery: one continuous AI-professor voice that does the lesson AND the doubts; TTS is demoted, not staged.
> Founder's question: in B2C, is it "no TTS, only a real-time voice professor that explains by driving the sim," or "student uses TTS first then the AI professor for doubts"? **Answer: one continuous professor voice — it *performs the authored script* in natural real-time voice (replacing TTS) AND handles off-script doubts in the same voice, no seam.** The two-stage "TTS-then-professor" model puts a seam exactly where it hurts most — you can't barge in on a TTS track, so every doubt becomes a clunky mode-switch; the magic of a real tutor is interrupting them mid-sentence and getting answered *right there*. The professor doesn't free-generate the lesson — it *performs verified authored content* (safe) and only orchestrates/verifies for the novel tail. **TTS survives only as a cheap/offline tier** (narrated playback, no interactivity) for users who can't afford or don't need the live agent. Crucially: **this changes nothing we author today** — script + bindings authored once; *how* it's delivered (TTS now, professor voice in ~2027) is a swappable runtime layer. Choosing "professor replaces TTS" costs nothing now.

> ### Decision 4 — The reviewer→rulebook promotion filter (how one reviewer's feedback becomes rules for ALL sims).
> Founder's process question: when we discuss important things, do we refer from DISCUSSIONS.md or write into CLAUDE.md? Resolution — **different homes by purpose:** DISCUSSIONS.md = the WHY (reasoning archive, not auto-loaded); CLAUDE.md = the terse WHAT-I-MUST-DO (the only doc loaded every session — so anything that's a standing build rule MUST land here or it gets silently violated); memory = recall safety net. The promotion test (all three must pass to enter CLAUDE.md): **generalizes to all sims · forget→build-wrong · corroborated** (2nd reviewer hits it independently OR grounded in existing doctrine). Asmi's first review sorted into: this-sim → `feedback.md`; visual-gate checks → CLAUDE_TEST.md §5.6b (M1 path-renders-fully, M2 timing-faithful, M3 pause-discoverable); **promote-now → Rules 24 + 25** (both also grounded in this session's doctrine, so they pass the corroboration test despite being one reviewer); **candidate patterns (hold for a 2nd reviewer)** → TTS-pacing-too-fast, break-state-into-sub-states. **Guard against overfitting to one reviewer** (Session 68): promote where multiple tutors hit the same wall; log idiosyncratic taste as noise. The filter is now documented in `reviews/README.md` so every future professor's feedback runs it — the rulebook compounds deliberately instead of bloating.
> → **Promoted to CLAUDE.md Rule 25** (pedagogical sequencing is a correctness requirement: 25a foundation-before-complexity, 25b never-reference-an-untaught-concept, 25c co-locate explanation with its visual) — the *wrong-teaching-order-with-right-physics* defect class Session 68 named as the thing only a practicing teacher catches.

> **Net for the build:** A15 will be authored as a silent teacher-driven visual + clean equation + the concept states (charges→arrows→equation hook, the L–B angle trap, bent-wire-becomes-chord, loop-net-force-zero), with a full `teacher_script` + reveal-bindings authored but TTS off — shipping the sword for Asmi and laying the B2C brick in one pass. No ring-tension / non-uniform-field integration in V1 (advanced problem-classes deferred, per conceptual-depth-first).

---

## Session 68 — The voice AI professor is achievable as an ORCHESTRATOR (not a generator); India data = the teaching-policy corpus; first-reviewer value check (2026-06-15)

> Context: first reviewer **Asmi Singh** completed her first real review (sim `magnetic_force_moving_charge`) via a 2-part recording — 37-min Zoom call + 7.5-min screen recording. Founder asked two strategic questions off the back of it.
>
> ### Topic 1: Is the first reviewer worth ₹500? — Yes, not close.
> Claude's verdict: at ~₹500 (~$6) for ~1.5–2h of an experienced physics tutor's time, the value is wildly asymmetric. She caught things **only a practicing teacher catches** — chiefly a *pedagogical-sequencing* flaw (the sim introduces the helix/θ=10° case before θ=90° and θ=45°, so a student can't understand *why* a helix forms). That's not "wrong physics," it's "wrong teaching order" — invisible to an AI reviewer, a physics PhD, or the founder reviewing his own work, yet exactly the kind of defect that makes tutors bounce. She also found a real physics-timing bug (at θ=45°, v·cosθ=v·sinθ so circle-vs-drift should be balanced; the animation shows them unequal) and a recurring trajectory-rendering bug. **Caveats kept honest:** she is a strong *pedagogy/UX* reviewer, not a flawless physics oracle (two of her own statements were off — "θ=0 particle loses energy and stops" contradicts magnetic-force-does-no-work; the θ=90° radius-vs-period mix-up), so physics correctness is still cross-checked by us. And it's a sample of one sim. Decision frame: pay it without hesitation and treat it as buying *the next sim's review too* — the real question isn't "worth ₹500" (obviously) but **"does she scale to 10–20 sims at this sharpness and cadence."** Steer her toward her highest-value output: the teaching *sequence* + the "where a student gets confused" map (the reorder), not bug-spotting (which we'll automate). All feedback + raw transcripts are now archived in `reviews/reviewers/asmi-singh/magnetic_force_moving_charge/` (master `feedback.md` + immutable `sources/`) — the reviewer data-engine folder convention, reviewer-first, scalable to every future professor.
>
> ### Topic 2 (the strategic gem): can a voice AI professor really do what Asmi does — drive the sim AND "write on the whiteboard" AND derive formulas AND explain like a human — for students in UAE/West?
> **Yes — but only once "voice AI professor" is defined correctly: an ORCHESTRATOR of pre-authored, verified assets, NOT a live generator of physics.** A human writes on the whiteboard because they generate the explanation live in their head; the AI must NOT do that (live-generated physics can be wrong in front of a student — the Rule 18 hard floor). Instead it sequences verified assets — the sim, authored derivations, authored annotations, authored remediation branches — in the right order, at the right moment, in natural voice. That is a *fundamentally easier and safer* problem than "an AI that understands and teaches physics from scratch," and it is the architecture PhysicsMind already has. Decomposing "what Asmi does":
> 1. **Talk in natural voice** — ✅ today (real-time speech-to-speech ≫ the robotic TTS Asmi called "too fast").
> 2. **Drive the simulation** (set θ=90°, change v, freeze a frame) — ✅ today, and the moat: sims already expose a control API (postMessage SET_STATE / sliders / the freeze-frame just built); the AI emits tool calls = Asmi dragging the slider, automated. LLMs are good at tool-calling.
> 3. **Synchronize voice + visuals** (point at v·sinθ *while* saying it) — ✅ near-term; `teacher_script` already binds sentences to glow/math-reveal/hand-phase; the AI produces that orchestration dynamically. Pure orchestration.
> 4. **Whiteboard / derive formulas** — ⚠️ split it: **(a)** reveal + narrate *authored* derivations (`derivation_sequence` with handwriting `animate_in`) and annotate the sim — doable now, SAFE (human-verified), and covers ~90% of what the whiteboard is *for*; the student doesn't care if the equation appeared by the AI's "hand" or a triggered reveal. **(b)** true free-form live handwriting — hard, risky (can write wrong physics), low marginal value over (a) → **consciously do NOT chase this early.** It's the one capability that's both hard and unnecessary.
> 5. **Answer questions / detect confusion / re-explain** — ⚠️ partial, improving: voice Q&A doable now; branching to authored remediation is what EPIC-C/drill-down is built for; *true* human-level "did they actually get it?" sensing is the genuine unsolved frontier (ship good-enough — Feynman-back, quiz-back, voice cues — improve with data).
>
> **Why the architecture makes this work when "AI teacher" generally doesn't:** most attempts fail by asking one model to understand + generate + render physics live, all unverified. PhysicsMind already split those: physics-correctness lives in verified sims + authored derivations (offline, reviewed); the *teaching act* becomes orchestration the AI learns. **The AI never has to be right about physics — only good at sequencing/timing verified assets in response to a student.** Tractable with today's models.
>
> **The insight that ties it to the India strategy:** the orchestrator's missing knowledge is the **teaching policy** — when to pause, what to say here, which derivation to reveal next, where to point, and what to do when a student is confused about X. That is *exactly* what the two India data streams capture: Asmi-type reviews → the correct teaching *sequence* + confusion map (the 0→90→45→10 reorder IS a policy correction); tutor recordings → real "teaching-with-this-sim" traces. So "collect data in India" is precise, not vague: **India supplies the orchestration policy the AI can't get from the verified sim itself.** The physics is already in the sim; the *teaching* is what India teaches the AI. (Direct continuation of Session 66's teaching-trace corpus + "data outlives models.")
>
> **Honest hard parts (build eyes-open):** real-time loop latency (talk + drive + listen + respond at conversational speed — tractable, not trivial); genuine student-understanding sensing (the real frontier — ship good-enough, improve with data); free-form handwriting (skip); and it's a **multi-year build** where the architecture choices now (control-API completeness, authored-derivation coverage, orchestration-as-data) decide whether it's possible later — and they're already pointed the right way.
>
> **Bottom line for the pitch:** the final reviewer-hardened sim is not the product — it's the **verified substrate** the orchestrator plays on. The version that fails generates physics live; the version that works orchestrates verified assets and learns its teaching policy from India traces. Voice tech itself commoditizes by ~2027; the defensible asset is the pedagogy policy + the sim-coupling, both manufactured by the India phase.
>
> ### ADDENDUM (same session) — the unscripted 10–15% tail: it's an enumeration + harness problem, not an open-intelligence problem; the concrete orchestrator architecture
> Founder's sharper question: the 80–85% happy path (orchestrate pre-authored states) is fine, but what about the **10–15% where a student asks something off-script** — concretely: *"I don't get where θ is — is it the angle between the charge and B, or between v and B? Bring the two vectors together and SHOW me on the sim"* (a state we never authored), AND *"now slowly derive F = qvB·sinθ, substitute θ=90 → sin90=1 → max, θ=0 → 0, θ=45 → 1/√2, explain each line"* (live whiteboard derivation). Can the AI do this unscripted, in ~1–1.5y of backend engineering, using ≤ Sonnet 4.6? **Verdict: yes for most of it — once you split the tail into two capabilities, each splitting again into "composable from verified pieces" (large, achievable) vs "genuinely novel" (a tiny verification-gated residue).**
>
> **Capability A — dynamic sim manipulation ("show me the angle between v and B").** Achievable IF you invest in a **rich primitive/operation vocabulary**, NOT pre-authored states. The AI never writes render code live (Rule 5); it **composes verified primitives** exposed as tool calls — `bring_to_common_origin(vA,vB)`, `draw_angle_arc(θ)`, `decompose_into_components(v,B)`, `highlight()`, `rotate_camera_to_face(plane)`, `overlay_label("θ")`. Crucially the real v and B (with real directions) **already exist in the current sim state** — the AI invokes a "show the angle that's physically already there" gesture; the vectors are real, only the *display gesture* is dynamic. Coverage of this whole class = **richness of the primitive library** (extend the existing PCPL + postMessage contract into a full sim-manipulation API). Honest limit: it can only show what a primitive can express (e.g. can't show the field produced *by the charge itself* if the sim models only the *external* field) — never 100%, but grows monthly via the compounding loop (Rule 17 meaning 1: confusion → new primitive → richer engine).
>
> **Capability B — live derivation / whiteboard.** Split: **B1 (known, curriculum-bound result — the majority):** author the derivation ONCE as a verified **parameterized template** with substitution slots; the AI **performs** it step-by-step (slower / repeat on command); and **every number is computed by a deterministic math engine (mathjs, already a dependency), NEVER by the LLM** — sin90=1→F=qvB, sin45=1/√2→F=qvB/√2 are *computed*. The single most important safety move: **LLM controls pacing + narration; a deterministic layer produces every symbolic step and number.** "Whiteboard writing" = revealing authored `derivation_sequence` lines (handwriting `animate_in`) + the verified computed values. **B2 (genuinely novel derivation never authored):** LLM proposes → **verification gate** (symbolic/physics checker or adversarial 2nd-model review) → shown only if verified; if not verifiable the AI does NOT bluff — it defers gracefully ("let me get that shown to you properly"), routes to founder/expert, and the question **becomes authored content** (residue shrinks every time). Because the product is **curriculum-bound (NCERT/JEE/A-level), the askable derivation set is finite** → once authored, B2 is a trickle.
>
> **The reframe:** the unscripted tail is mostly an **enumeration + harness problem, not open-ended intelligence** — the curriculum bounds what a student can ask. So the founder's instinct is exactly right ("the key is the best harness architecture + engineering investment"). You almost never need the AI to invent unverified physics; you need (1) a great orchestrator, (2) a rich library of verified assets (primitives + authored derivations + states), (3) a deterministic compute layer (mathjs) for every number, (4) a verification gate that converts misses into authored content.
>
> **Concrete orchestrator architecture (model-routed):** *intent/perception* (cheap fast model — Haiku/Gemini Flash — classifies: navigate / explain-deeper / show-on-sim / derive / substitute / confused-about-X) → *planner-conductor THE BRAIN* (**Sonnet 4.6**: given intent + sim state + tool API + authored-content index, plans the sequence of tool calls) → *verified capability layer* (sim-manipulation API + derivation templates + deterministic math + authored-content retrieval — **no LLM inside**) → *verification gate* (anything not from the verified library is checked before display) → *real-time voice* (prosody + barge-in) → *student memory* (adapt). **Why "≤ Sonnet 4.6" is correct:** intelligence lives in the **orchestration + asset library**, not a bigger model — the deterministic layer (not a smarter brain) guarantees physics correctness; Sonnet 4.6 is ample to *plan which verified tools to call in what order*. The bet is harness + verified assets, not frontier model size.
>
> **Doable in ~1–1.5y?** Yes — bounded engineering, no unsolved research (tool-calling, retrieval, deterministic compute, verification gates, real-time voice all exist today). Build list: (1) rich sim-manipulation API over PCPL; (2) author curriculum derivation/primitive set + wire mathjs as substitution engine; (3) orchestrator harness; (4) verification gate + route-to-human→author loop. **Honest "no" parts:** the truly open-ended "explain anything" tail is never 100% (handle gaps gracefully — route+author, never hallucinate; *bluffing physics is the one unforgivable failure*); real-time latency is a genuine challenge; true confusion-sensing stays partial (ship Feynman-back/quiz-back/voice-cues, improve with data); the primitive vocabulary always has edges. **Compounding:** every tail question the orchestrator can't answer becomes a new primitive or a new authored derivation (85%→90%→94%), and the India traces (real students asking real off-script questions to real tutors) are exactly the dataset that says WHICH to build next — **the tail is the roadmap.**
>
> ### ADDENDUM 2 (same session) — GTM doubt: if Western human tutors already teach with our sims and students trust humans more, why release a B2C AI professor at all?
> Founder's doubt: in the West (USA/UK/UAE) the plan is to sell curriculum-adapted sims B2B to private tutors who teach students with them — so those students already get *human* tutoring on our sims. Since students **trust a physical professor more than an AI tutor**, why also release the B2C AI-voice-professor? Won't the trusted human channel make B2C pointless / cannibalized?
>
> **Verdict: keep B2C — the doubt is correct only under a hidden false assumption (that B2C targets the SAME student who already has a trusted human tutor). It doesn't.**
> - **What's right in the doubt:** for the *premium 1-on-1 segment*, students genuinely do trust a human more (today, for deep mentorship). So never fight the human channel head-on for the same affluent customer — you'd lose. That instinct is correct and should drive *positioning*, not the build decision.
> - **The hidden assumption:** students taking human tutoring on our sims are a **small, affluent subset** (Western 1-on-1 is expensive + supply-constrained — structurally serves a thin slice). The B2C AI professor is for the **massive majority who are NOT in that subset**: can't afford $40–80/hr, need help at 11pm, smaller towns w/o elite tutors, need help on *every* homework daily (no human is affordable enough for that). Same mission as India ("a personal tutor for every student who doesn't have one") at Western willingness-to-pay. **Not cannibalization — coverage of the 90% the human channel can never reach on price/availability.** Two products for two barely-overlapping markets, not two products for one.
> - **Trust is not binary + is use-case-dependent:** human wins on mentorship/accountability/emotional read/the weekly relationship; **AI wins on 24/7 availability, infinite patience (ask the same "dumb" Q 20× no judgment), near-zero cost (architecture built for ~$0.0001/query), and no-embarrassment.** A human tutor is there ~2h/week; the AI covers the *other 166h*. So even tutored students become B2C users for homework/practice/between-session help → **complementary, not competing.** A Western tutor can even *distribute* the AI as their homework-support layer → B2B channel becomes a B2C acquisition channel. And trust **follows outcomes** (Khan/Duolingo/Photomath earned trust with zero human teachers); by launch (~2027–28) the in-school generation treats AI tutors as normal — the gap narrows fast.
> - **Why B2C MUST exist (the economic resolution):** B2B tutor sales are **linear + capped** (finite tutors, fixed seats) — real revenue, but tops out; never venture-scale. **B2C AI professor is the scalable high-margin engine** (millions × subscription, marginal cost ≈ 0) — *the entire moat/architecture is built FOR this scale*; stopping at B2B tutoring would mean over-building the moat for a business that doesn't need it. **Structure: the tutor channels (India + Western B2B) are the BRIDGE — they earn revenue, trust, and the teaching-policy data that makes the AI professor good enough to trust; the B2C AI professor is the DESTINATION where the company becomes large.** B2C isn't *instead of / against* the human channel — the human channel *earns the right* to launch B2C, and B2C is what makes the whole thing worth the investment.
> - **The one thing the doubt SHOULD change — positioning, not existence:** never market B2C as "fire your trusted human professor" (that's when the doubt comes true and you lose the affluent segment). Position it as **"the professor for everyone who doesn't have one — and the 24/7 backup for everyone who does."** Aim at the underserved majority + the gap-hours; then the trust objection mostly evaporates because you're not asking anyone to give up the human.
> - **Bottom line:** keep B2C; just never point it head-on at the trusted-human segment. Tutor channels = revenue + trust + data bridge; B2C AI professor = the scalable destination the architecture exists to reach. (Consistent with the Session-66 curriculum ladder India→UAE→US and the "personal tutor for every student who doesn't have one" mission.)
>
> ### ADDENDUM 3 (same session) — YC-partner interrogation → find the knife → the "sword" bar + the seed-then-refine loop
> Founder asked for a brutally honest **YC-partner-grade** evaluation (using Wispr Flow as the mirror), then to pressure-test the wedge, then brainstormed the product bar himself. Arc captured:
>
> **The YC lens (reality-based, not cheerleading).** Wispr Flow = the shape YC funds: one sharp wedge, **daily use, value obvious in 30s, clear payer, viral loop.** PhysicsMind as pitched = the opposite: broad vision (3 subjects × 3 regions × B2B+B2C+AI-professor), long multi-year build, **no users, no revenue, no retention, solo founder.** A YC partner's verdict on the company *as pitched today = PASS* (pre-user, pre-revenue, solo, brutal edtech monetization, moat still a *thesis* not a fact, content business not yet proven to scale) — **but lean in on the FOUNDER** (ships fast solo, real architectural insight "orchestrate verified assets / never generate physics live," the "a sim needs a teacher beside it" clarity, the data-engine thesis). The hard questions to expect: *what's working right now? why hasn't Khan/Brilliant/PhET/Google done it & why can't they crush you? edtech monetization is brutal—who pays? this is a content business, not software—why does it scale? you're describing a 5-year vision—what does a user do next MONTH? you're solo—why?* Flip-to-yes triggers: (1) one wedge with **weekly retention**; (2) the **cost-per-sim curve bending down** (content→software); (3) a 2nd person; (4) ~40% "very disappointed" on the Sean-Ellis test. **Key distinction: EXIST funds the cathedral (deep-tech, long, pre-revenue — exactly YC's reject profile); YC funds the knife. Don't confuse the two audiences — the Dresden deck can be the cathedral; a future YC pitch must be the knife.**
>
> **The knife (wedge pressure-test).** Rubric for a real wedge: repeated weekly use · clear payer/WTP · validate in weeks · spread loop · **honors the "needs a teacher" insight (a wedge that needs the AI professor first is the cathedral, not a wedge)** · manufactures the moat data. Three candidates scored → **Winner: the Tutor's live teaching tool** (tutors teach their lessons *with* the sims + a lesson-recorder). It's the rare wedge that is BOTH a knife AND a brick in the cathedral (retention proof → fundable traction; recordings → teaching-trace corpus; tutor relationships → India B2B channel; student exposure → telemetry). **Rejected: (B) student "night-before-exam" self-serve — disqualified because it VIOLATES the core insight (a student alone can't learn from a bare sim); it only works once the AI professor exists = the cathedral in a knife costume. (C) viral sim content — keep as the DISTRIBUTION engine, not the wedge (zero retention/revenue).** Sharpest framing of the winner: **own ONE chapter completely** (Magnetism Ch.26 — you already have ~5 diamonds ≈ a near-complete chapter) so you're *indispensable for that chapter*, then land-and-expand. **Node zero already exists: Asmi (teaches + reviews magnetism = first pilot user, not just reviewer).** The reframe: stop saying "I'm building an AI professor / content library"; start saying **"I'm getting 15 tutors to teach every magnetism lesson with my sims and measuring who can't stop."** Same company, completely different fundability.
>
> **The 3-week retention experiment (the instrument that flips PASS→YES):** recruit 10–15 JEE/NEET magnetism tutors (Asmi + network + tutor groups; small per-lesson incentive) → give them the magnetism sims + a dead-simple live-use path + the lesson-recorder (= usage proof AND trace capture) → use it for every magnetism lesson for 3 weeks → **measure the ONE number: % who return next week UNPROMPTED** + run Sean-Ellis ("how would you feel if you could no longer use this?"; ≥40% "very disappointed" = PMF signal). Strong number = traction, not vision; weak number = learned cheaply in 3 weeks before building the cathedral.
>
> **Founder's own brainstorm — the "sword" bar (correct, with 3 sharpenings).** Founder independently articulated the bar: a teacher should feel that *without* the sim they're **missing something important** — the product should feel like a **sword/weapon** to them — and that you can't build that from imagination; you build it from tutor screen-recordings + reviewer feedback. **Verdict: right bar (= the can't-live-without / Sean-Ellis test) and right method (build from real teaching, not imagination) — these two correct instincts ARE the moat (anyone makes one sim; almost no one builds the machine that turns real teaching into indispensable sims).** Three sharpenings adopted:
> 1. **Seed, then refine — it's a LOOP you seed, not a one-way "build from data."** Cold-start: you can't record usage of a sim no one uses yet → build v1 from judgment + Asmi (good enough to get used), THEN recordings refine it toward sword. Data is the *refiner*, not the seed.
> 2. **Lead the leaps — beware the "faster horse" trap.** Teachers ask for incremental fixes, rarely the magical thing (they don't know to). Reviews tell you what's *wrong*, not what's *unmissable*. Data shows where you fall short of sword; *founder judgment* invents the leap.
> 3. **Measure the OUTCOME, not the artifact.** A sword isn't the sim's polish — it's the *result the teacher gets that they can't get without it* (student aha faster than they could produce alone / looks brilliant / saves prep / finally explains a hard concept). When watching recordings, hunt the *moment a teacher gets a result they couldn't get on a whiteboard* — that, repeated and reliable, is the sword.
> Plus: the founder's **"ecosystem to teach with" instinct is right** — indispensability may come from the teaching **workflow** (sim + whiteboard/annotation + frictionless drop-in to a live lesson), not just sim graphics; a great sim that's awkward to teach with is NOT a sword. **Guard against overfitting to one reviewer** — build where 4-of-5 tutors hit the same wall (signal), log one-off idiosyncratic tastes (noise). The 3-week pilot is the *instrument* that reveals what "sword" empirically means (watch where tutors light up / work around it / would panic to lose) → fold "build the teaching workflow, not just sims" into the pilot.
...
>
> **ADDENDUM 12 (2026-06-12) — the synchronized derivation whiteboard ("the AI writes the formula beside the sim, like a teacher"): YES, possible — the engine is half-built already, and the written math must be AUTHORED, never LLM-composed live.** Founder's observation: concepts split into archetypes — (a) **derivation-led** (Biot-Savart: teaching the concept IS forming the equation) vs (b) **single-formula** (explain the phenomenon, state one formula, end with a parameter-interaction section). In real classrooms (India, 1-on-1 or group), the tutor derives the formula on the board BESIDE the running sim. For the AI professor, we need a whiteboard/sheet surface where the AI writes the equation step-by-step, synchronized with sim states and voice. Is real-time sync possible? Decisions:
> 1. **Yes — and the renderer already exists in the codebase:** board mode's `derivation_sequence` engine (primitive-by-primitive `animate_in: "handwriting"`, answer-sheet canvas, MVP at `public/board-mvp.html`) IS the whiteboard surface. Board MODE stays deferred (Rule 20 suspension — that product is exam-answer formatting); the ENGINE gets a second life as the **conceptual derivation panel**. Two purposes, one renderer — nothing un-deferred.
> 2. **Three synchronized tracks, one conductor:** sim canvas + whiteboard + voice. The whiteboard is just another deterministic state machine with a `REVEAL_LINE(n)` verb; each authored derivation line carries a `linked_state` (the line "dB ∝ I·dl" reveals exactly when the sim highlights the current element). The AI professor emits SET_STATE + REVEAL_LINE + speaks its own words — the same conductor pattern as Addendum 10, no new physics risk surface.
> 3. **Hard rule (Rule 18 extended to written math):** SPOKEN words are generated live; WRITTEN math is authored content only. The AI selects and reveals authored lines, and may substitute student-given numbers into authored templates (the WHAT-IF pattern); it NEVER composes new derivation steps at runtime. Math that persists on screen is correctness-critical → content, not behavior — the same knowledge/behavior split as Addendum 9.
> 4. **Authoring discipline (g) adopted:** tag every concept by archetype — `derivation_led` / `single_formula` / `phenomenon_only`. For derivation-led concepts, the physics block's derivation steps are stored as structured data from day one (`derivation_steps[]`: latex line + `linked_state` + tts beat) — author-compatible NOW, panel UI built LATER (Phase-2 voice-conductor demo or after). The physics-author stage already produces derivations; this only fixes the storage shape.
> 5. **Teacher-mode bonus (free-hand doctrine intact):** the same panel, teacher-driven — a steppable, clean typeset derivation beside the sim on the projector. Optional; the teacher who prefers his own blackboard ignores it. (Matches observed real-classroom practice: sim + board derivation side-by-side.)
> 6. **Feasibility, honest:** rendering = KaTeX with progressive/stroke reveal (handwriting animation already specced for board mode); sync = the same 1–3s voice-turn latency budget as the conductor; the hard part is AUTHORING DISCIPLINE (structured derivation steps with state links), not technology.
>
> **ADDENDUM 13 (2026-06-12) — the Contributor Program: free sims + two paid data tracks (recording bounty + per-state review job), open recruiting, and the speed loop. Founder's proposal adopted with three incentive fixes.** Founder: give sims FREE to professors/tutors/YouTubers; Track A — optional recording bounty (₹20–50 per recording of themselves teaching with the sim); Track B — part-time review job (~₹200–400/hr: walk each state, flag, reason, improvement note); recruit openly (LinkedIn/Reddit/etc.), interview, hire part-time. Main doctrine: SPEED — create sims fast → release to the free+paid professor pool → analyze data → update → repeat; the loop, not per-release perfection, builds the asset. Verdict: this is the correct operational form of Addenda 1–3 (Lesson Recorder, signal stack, review panel), with fixes:
> 1. **Fix 1 — review pay unit: per COMPLETED CONCEPT REVIEW, never per hour** (re-affirms Addendum 3: per-hour buys rubber-stamping, per-flag manufactures fake problems). Founder's rate instinct sets the PRICE, the artifact sets the UNIT: a concept review ≈ 45–60 min → ₹300–500 per completed review with a forced verdict on EVERY state (good / minor / major + reason). Same money, aligned incentive.
> 2. **Fix 2 — the recording bounty needs a "valid trace" definition + auto-validation, or ₹20–50/recording invites junk** (fragments, silent uploads, farming). Valid trace = ≥15 min teaching + audible teacher voice + sim event density above floor (the recorder KNOWS: state changes are logged) + 1 per concept per session; auto-approve payment from recorder metadata, spot-check 1-in-10 by ear, cap per teacher per week. Teacher-mic + sim events only (Addendum 1 — no student audio/video), so consent friction stays near zero. Bounty ₹50–100 for a valid trace is the better band (₹20 undervalues the act of consenting + remembering to press Record).
> 3. **Fix 3 — the interview IS a calibration test, not a conversation:** candidate reviews 1 diamond against the panel's gold answers; hire those who catch the planted obvious defects AND write specific reasons ("state-4 arrow unreadable on projector" yes, "make it engaging" no). Screens for the only skill that matters: specificity.
> 4. **Payment doubles as IP capture:** paying (even small amounts) + a one-page contributor agreement = contracted ownership of recordings and feedback for training/improvement use. Goodwill data is legally fragile; paid data is corpus. Rights assignment + consent text in onboarding, non-negotiable.
> 5. **Pool size is governed by founder throughput, not supply:** start 3–5 reviewers + 5–10 recording tutors; grow ONLY when the proposal queue runs consistently empty. A 50-person pool feeding a one-founder approval gate = backlog death (the bottleneck is approve+fix, not data). YouTubers are a different deal shape — brand/distribution play (free tool, public content, badge visible), usually no payment needed; don't mix the tracks.
> 6. **Speed doctrine adopted WITH the gate re-shaped, not skipped:** "release fast to professors" is sound precisely because the paid pool IS the professor gate, parallelized (Stage-④ stops being a one-expert bottleneck). Order preserved: machine gates (validator + visual eyes) INTERNAL → professor pool EXTERNAL → students LAST. Never spend paid reviewer hours on sims that fail machine gates — burns money and pool credibility.
> 7. **Budget math:** 10 tutors × 4 valid traces/wk × ₹75 ≈ ₹3k/wk + 5 reviewers × 4 concepts/wk × ₹400 ≈ ₹8k/wk → **~₹45–50k/month total** — consistent with Addendum 3's ₹60–90k/chapter, trivially affordable for what it buys (corpus + review + the Addendum-9 style-tagged ingestion applies to all incoming traces).
>
> **ADDENDUM 14 (2026-06-12) — the bundled "Full Packet" (₹400/sim: teach-record + per-state flags in one deal) — ADOPTED as the flagship tier, with the honest sufficiency answer: enough for TEACHABILITY, not yet for BEST QUALITY until the student-outcome piece (already built) is attached.** Founder simplifies Addendum 13's two tracks into one package: sim is free; contributor who BOTH (a) records themselves teaching a real student with it AND (b) afterwards flags every state with reason + improvement note gets ₹400 per simulation. Question: is this data really enough to drive existing sims to best quality? Decisions:
> 1. **The bundle is BETTER than split tracks for review quality — adopted as the flagship.** A reviewer who has just taught the sim to a real student flags from EVIDENCE ("my student asked X at state 4"), not armchair opinion — and the recording lets us VERIFY the flags against what actually happened. Teach-first-then-review is the strongest grounding design available; one artifact, one payment, simpler ops.
> 2. **Keep two lighter tiers alongside** (don't make the bundle the only door): recording-only bounty (tutor won't do paperwork) and review-only (contributor has no active students; also the 1 RIGOR expert stays a separate role — working tutors check teachability, not physics correctness). Bundle requires an active student, which shrinks the pool — tiers keep the funnel wide.
> 3. **Pricing sanity:** ₹400 ≈ ₹160–200/hr effective (1 hr lesson + ~45 min review) — acceptable because the lesson was happening anyway (marginal effort = press Record + review); start at ₹400, watch the acceptance rate, move to ₹400–600 if recruiting stalls. Acceptance defined PER COMPONENT (valid trace per Addendum-13 Fix 2 + all-states verdicts per Fix 1) so one junk half doesn't poison the payment dispute.
> 4. **The sufficiency answer (honest): this packet covers 3 of the 4 signal sources — teacher experience (fully), audible student confusion (partly), AI trace analysis fuel (fully) — but NOT the one that defines "best quality": did the student actually UNDERSTAND?** A sim teachers love to teach with ≠ a sim students learn best from. The missing closure costs almost nothing: **attach the existing per-concept `assessment` block as a 2-minute exit quiz** — tutor has the student answer it at lesson end (or homework link). Every question already maps to a state (`teaches_state`/`coverage_map`), so a missed question LOCALIZES the weak state. This converts the packet from opinion ("I think state 4 confuses") to evidence ("student failed the state-4 question") — the Full Packet becomes trace + flags + OUTCOME.
> 5. **Saturation logic:** the first 2–3 packets per sim catch ~80% of teachability defects; expert-style review saturates fast (Addendum 3 #7 — recurring findings become validator gates). After round 2, the marginal signal shifts to outcomes + telemetry — so the program's center of gravity moves from flags to quiz results as the catalog matures. ≥2 independent packets per sim preserves the agreement-strength signal (Addendum 3 #2).
> 6. **Verdict line:** YES — this packet (with the exit quiz attached) is sufficient fuel for the round-1/round-2 quality lift across the catalog; without the quiz it polishes the teacher's experience while staying blind to the student's understanding.
>
> **ADDENDUM 15 (2026-06-12) — session capstone: founder names the fear.** "This is a new innovation product… I'm scared, I don't know if it goes… but I'm really confident that if we execute well, it works really good. We need to achieve our vision." Claude's response, on the record: the fear and the confidence are both correct and attach to different objects — fear belongs to the OUTCOME (unknowable in advance; that is what the lift study and phased roadmap exist to answer cheaply), confidence belongs to EXECUTION (controllable, and the only variable that matters in a category where the science is settled and the moat is grind). The session's 14 addenda are precisely the machine that converts one terrifying bet into a chain of small, answerable ones — the founder never needs to know the whole vision works, only to win the next test. Worst case is bounded (~6 months + a few lakh → still owns the factory, the corpus, and the lesson); the asymmetry favors starting. The one risk the fear SHOULD attach to: the still-open SAP-vs-PhysicsMind fork — a scared full-time founder beats a confident part-time one.
>
> **ADDENDUM 16 (2026-06-12) — first contributor signed: the IIT-Delhi tutor (Asmi, via UrbanPro→WhatsApp; IIT-JAM ~AIR 246, MSc Physics IIT Delhi, ~2 yrs tutoring). Role, day-1 protocol, pricing, anti-gaming constraints.** She's ready to start immediately and will charge. Decisions:
> 1. **Role fit (honest):** rigor expert + recent-topper student-proxy lens — NOT the veteran-classroom-teacher lens (only ~2 yrs, 1-on-1). She's the ideal FIRST panel hire (Addendum 3 composition); the "students get confused HERE" wisdom still needs a veteran JEE/NEET teacher as hire #2. Strategic bonus: her reviews on the first 2 diamonds become the GOLD ANSWERS for calibration-testing every future reviewer (Addendum 13 Fix 3).
> 2. **Access blocker flagged (real):** the app is localhost-only and the security audit found NOTHING is protected (proxy.ts dead code, unguarded LLM routes) — do NOT rush a public deploy. Day 1 runs entirely over Zoom (founder hosts, screen-share + remote control so she drives; Zoom cloud/local recording = the recorder). Her independent access this week via a time-boxed tunnel (founder online) or after the Batch-C minimum (middleware rename + route guards) ships.
> 3. **Day-1 session (~90 min, ONE concept):** (a) 15-min brief + one-page agreement (consent, IP/work-for-hire assignment for training use, payment terms — BEFORE first payment; her engagement is contractor work for PhysicsMind, unrelated to UrbanPro tutoring); (b) **think-aloud FIRST viewing** — she watches narrated auto-mode cold, reacts aloud, founder silent, recording ON (the richest artifact, captured exactly once per person per concept); (c) **mock lesson** — she teaches founder-as-student her own way, TTS muted, recorded (tagged `mock_student` — founder isn't a real learner; its value = process test + her teaching style + usability); (d) homework: the per-state review form.
> 4. **Review form built TONIGHT (Google Form/Sheet, machine-readable rows):** per state — verdict (good/minor/major, forced), category dropdown (pacing/crowding/missing-visual/wrong-emphasis/physics-error/confusing-narration), mandatory reason, mandatory suggested fix for minor/major, timestamp reference into the recording.
> 5. **Pricing: per SIMULATION (artifact), never per hour** (Addendum 13 Fix 1 stands). Full packet (think-aloud + mock lesson + complete per-state review) = **₹500/concept** offer (founder's ₹400 + a respect premium for her caliber; accept up to ~₹700 — the first gold-standard contributor sets the quality bar, don't penny-pinch her). Pay via UPI promptly after artifact validation — prompt payment is retention.
> 6. **Anti-gaming constraints (founder asked for no loopholes):** per-artifact pay (no time-stretching incentive); forced verdict on EVERY state (can't skip); specificity rule ("make it engaging" rejected — must name what + why + concrete fix); **the recording is the lie detector** (every flag must trace to a moment in the recording); validity floors auto-checked (duration, audible voice, sim-event activity); founder fully QAs the first 2–3 packets (calibration conversation on specificity); commission only 2 concepts before judging quality; ≥2 independent reviewers per sim once hire #2 exists (agreement = signal strength).
> 7. **Scale path:** Day 1 concept #1 → Day 2–3 concept #2 → QA gate → if good, commission the 5-diamond magnetism spine as packets → then real-student Full Packets (her own UrbanPro students, with exit quiz per Addendum 14) → her gold reviews calibrate hires #2–3.
>
> **ADDENDUM 17 (2026-06-12) — multi-professor allocation strategy: OVERLAP, don't divide chapters. Calibrate with full overlap, then run 2-of-3 rotating per sim.** Founder asked: with 2–3 professors, do we allocate divided chapters to each (coverage) or send the SAME sims to all (catch common mistakes)? Decisions:
> 1. **Never divide chapters by ownership for the REVIEW track.** Chapter allocation = one person's taste owns a whole chapter: no agreement signal (can't tell truth from taste), undetected drift, single-lens coverage (the rigor lens and the veteran-confusion lens are PER PERSON — both should touch every sim, that's the Addendum-3 panel design), and bus-factor risk. Coverage-by-division optimizes for speed at the cost of the one thing review exists for: confidence.
> 2. **Phase A (calibration, when reviewer #2/#3 join): FULL overlap on the same 2 diamonds, strictly independent** (no discussing before submitting — independence is sacred). Purpose: align the rubric, benchmark each reviewer against the others + founder QA, mint the gold answers. 3× cost on 2 concepts = cheap insurance.
> 3. **Phase B (steady state): each sim goes to exactly 2 of the 3, pairs ROTATING** (A-B, B-C, A-C across sims). Why 2 not 1: a single reviewer's flag is unverifiable taste. Why 2 not 3: once calibrated, the third vote rarely changes a verdict — 50% more spend for marginal signal. Rotation prevents stable-pair shared blind spots and keeps every reviewer benchmarked against every other.
> 4. **Per-state agreement matrix = the real output:** both say good → confident pass; both flag the same state → certain, fix it; one flags, one doesn't → founder judges, or 3rd reviewer tiebreaks, or the state is marked for live-test/telemetry data to settle it. (Addendum 3 #2 operationalized.)
> 5. **Agreement stats double as reviewer QA (anti-gaming continued):** a reviewer who never flags = rubber-stamping; one who consistently disagrees with both peers AND student outcomes = miscalibrated. The rotation makes both visible without extra cost.
> 6. **Exception — the teaching-trace/recording track needs NO overlap:** tutors record whatever lessons their real students need; coverage is opportunistic, redundancy adds nothing there. Overlap discipline applies only where flags drive content edits.
> 7. **Cost math at ₹500/packet:** 5 magnetism sims × 2 reviewers = ₹5,000/round (vs ₹7,500 full-overlap). **Now, with only Asmi signed:** run her solo with founder QA as the de-facto second reviewer; move to 2-of-3 when the veteran-teacher hire #2 lands; full calibration round when #3 arrives.
>
> **ADDENDUM 18 (2026-06-13) — Contributor Program operational artifacts SHIPPED; founder's together-then-solo ramp approved with one gate and one new loophole closed.** Built tonight at `docs/contributor_program/`: CONTRIBUTOR_GUIDE (.md + send-ready .html — packet definition, golden rules, step-by-step, rejection table, good-vs-bad flag examples), CONTRIBUTOR_AGREEMENT (.md + .html — recording consent, work-for-hire IP for product improvement + AI training, ₹500/accepted packet, confidentiality, WhatsApp "I AGREE" signature; lawyer review before scaling), FOUNDER_RUNBOOK (internal — tonight checklist, 90-min session script with stay-silent rule, per-packet QA checklist, red-flag table, payment ledger), review_form_template.csv (per-state rows: forced verdict dropdown + category + specific reason + suggested fix + recording timestamp). Decisions: (1) founder's plan — sims 1–3 together on Zoom, then solo — APPROVED with an explicit gate: solo only after **two consecutive packets pass QA without rework**; solo packets still require self-recording (the recording stays the lie detector when unobserved). (2) **New loophole closed: the preview.** A contributor who opens the sim before the First-Look session destroys the first-impression artifact — rule added everywhere: link is shared only at session start; previewed sim = First-Look invalid. (3) Document beats PPT for the contributor guide (reference material, WhatsApp-friendly, searchable); PPT only if a group onboarding ever happens.
>
> **ADDENDUM 19 (2026-06-13) — founder's workflow-gap questions (timestamps with no clock; no board beside the sim) answered ZERO-BUILD for session 1; testing-phase build list prioritized.** (1) **Timestamps:** the sim has no clock and needs none — the convention is **voice markers**: she says "FLAG: state N — …" aloud; the recording carries the marker, the STATE NUMBER is the address, and exact mm:ss gets copied (approximately) from the video player's scrubber when she fills the sheet afterwards. No screenshots ever. (2) **The board:** Zoom Annotate lets her draw directly ON the shared sim (and those annotations are themselves data — wherever a teacher must draw, the sim is missing a visual → feeds Addendum-12/Addendum-2 signals), plus a blank Excalidraw tab kept open as the blackboard for derivations. (3) **Build list for the testing phase, priority-ordered:** ① visible state badge in the player if absent (tiny); ② tunnel/secured access before her solo phase; ③ in-sim 🚩 flag button (the Addendum-2 teacher flag button) replacing FLAG-aloud; ④ Lesson Recorder v0 (Addendum 1, mic + time-aligned sim event log) replacing Zoom — which makes timestamps AUTOMATIC and dissolves this whole problem class. Guide/runbook/CSV updated accordingly; nothing needs building before tomorrow's call.
