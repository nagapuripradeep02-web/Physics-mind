...
**Status: 0a + 0b COMPLETE · amended after founder-proxy Checkpoint A cycle 1 (2026-08-01).
0c NOT DISPATCHED — awaiting Checkpoint A cycle 2.**
...
| Concept | State → rows |
|---|---|
| `hydrogen_bonding` | S1 B,C,D,**N** · S2 A,B,C,D,E · S3 A,B,D,E,M · S4 A,B,C,E,M · S5 A,B,C,E,H,M · S6 A,B,E,H,M · S7 A,B,M,**O** · S8 A,B,C,E,H,M |
| `ionic_bonding` | S1 C,M,**N** · S2 C,K,M,**N** · S3 A,C,M · S4 A,C,I,M · S5 A,I,M,**P** · S6 A,C,I,J,M · S7 A,C,H,I,M · S8 A,C,I,M,**Q** · S9 A,I,M,**O** · S10 A,C,H,I,J,M,**Q** |
| `bond_polarity_dipole_moment` | S1 B,C,D,**N** · S2 B,C,D,F,M · S3 B,D,F,M · S4 B,D,F,M · S5 B,D,F,M · S6 B,D,F,L,M · S7 B,D,F,L,M · S8 B,C,D,F,L,M |
| `metallic_bonding` | S1 A,I,M · S2 A,C,G,I,**N** · S3 A,G,I,M · S4 A,G,I,M · S5 A,C,G,I,J,M · S6 A,G,I,M,**O** · S7 A,C,G,H,I,J,M |
...
state.bonding_scene = {
  ...
  links:   { enabled, delta_min, form_pm, break_pm, angle_window_deg, show_count },
...
`species · molecule · ligand · angle · delta_chi · temperature · count · separation · lattice_size ·
valence · field · shift · spin`.
...
N per frame; every other unit carries colour only.**
...
| Element colours | `MG_ELEMENTS` (`:44797`) | one palette, tuned for the deep-blue field background |
...
repeats except the one declared contrast pair. Delta cues are basic literal English (Rule 41) —
five were rewritten at Checkpoint A for personification or an ambiguous referent.
...
| 7 | ext | The anomaly you can now explain | `trend-break` | "Water breaks the trend" | — | H₂S 213 · H₂Se 232 · H₂Te 271 K, trend → ~180; H₂O **373** |
| 8 | core | Explore | `interaction_complete` | — | species (core) · count (core) · T (**ext**) | live link count |
...
Coherent-when-cut (Rule 38a): hiding S6–S7 leaves what an H-bond is, that it is weak, what it
needs, and that it is a live network; the explore sandbox loses its `T` slider with them.
...
| 6 | ext | **Why it shatters** — shift one layer one site, like charges meet | `layer-shift-snap` ⇄ | "One shift, it splits" | shift | **like-charge contacts 0 → 6** |
| 7 | ext | Why it melts so high: the whole lattice must go at once | `melt-the-lattice` | "Heat frees the ions" | temperature | 788 kJ/mol · 1074 K |
| 8 | ext | **Molten conducts, solid does not** — the same field on both | `field-on-both` | "Free ions carry charge" | field | conductivity, solid vs melt |
| 9 | **adv** | Charge and size set the strength | `melt-race` | "Double charge, far stronger" | — | MgO 3791 kJ/mol · 3125 K vs NaCl 788 · 1074 |
| 10 | core | Explore | `interaction_complete` | — | ion pair (core) · spin (core) · T (**ext**) · shift (**ext**) · field (**ext**) | lattice enthalpy · m.p. |
...
**S10's controls are ring-gated.** S2 labels both radii "atomic radius" and states the metallic /
covalent basis once, so the 186 vs 99 pm comparison is honest.
...
| 8 | core | Explore | `interaction_complete` | — | molecule (core) · angle (core) · ligand (core) | live μ in debye |
...
| 7 | core | Explore | `interaction_complete` | — | metal (core) · field (core) · shift (core) · valence (**ext**) | drift · ΔH_at |
...
| **E3** | lattice layer | `placement:'lattice'` · rock_salt/fcc/bcc/hcp · growth beat · **row P interior reveal (D-5)** · coordination highlight · electron transfer + linear-pm radius re-scale · **derived** layer-shift outcome + `like_contacts` readout · electron sea + drift · **row Q ion drift** · melt | `ionic_bonding`, `metallic_bonding` | Desk 2 |
...
6. Link criterion is **derived**: sweeping δ across `delta_min` flips link formation with no authored
   change; O passes the threshold, S fails it; `form_pm` < `break_pm` so there is no flicker at the
   boundary.
7. Rock-salt coordination = 6 for every interior site; fcc = 12; bcc = 8; hcp = 12.
8. Layer-shift outcome is **derived** — flipping the lattice's charge pattern flips split ↔ hold, and
   `like_contacts` reads 0 → 6 on the ionic pattern and 0 → 0 on the cation-only one, with no
   authored change.
...
- **Cycle 2: PENDING.** E1 must not be dispatched until it returns `DESIGN_OK`.
