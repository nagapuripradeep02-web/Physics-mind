...
| ~~**F-5**~~ | ~~P2~~ | **FIXED 2026-08-27.** Its grading was UNMEASURABLE as written — rates 1.1/0.9 stopped existing when the audio pinned the pacing. Re-measured on the real timeline: `s4_3a` runs 6619-10971 ms, `Lcut` reveals at 12050, so the glow is empty for **100%** of its window at the only rate there now is. One key deleted. All 24 glows audited: `s4_3a` is the ONLY one that can never light; the rest (8-46%) name objects that appear DURING their sentence — ordinary reveal choreography, left alone. |
...
| ~~—~~ | ~~P3~~ | ~~`focal_primitive_id` has **no consumer at all**~~ — **PREMISE WRONG, GRADED AND CLOSED 2026-08-27.** It IS consumed: `smoke_visual_validator.ts` (`extractFocalPrimitiveIds`, ~:48-57) ships `focal_primitive_ids` to the PAID vision gate. **No authoring change is correct** — STATE_6 `arc1` / STATE_7 `arc_normal` are semantically right (the arc IS the focal element there) and repointing them at a stamped-but-wrong object to make them resolvable would degrade the authoring to paper over a renderer limit. The real finding is narrower and now filed: `angle_arcs_are_the_only_vg_object_kind_never_stamped_so_no_mechanism_can_address_one_by_its_authored_id` — every pool the display pass fills is `stamp()`ed except the arc loop (~15106-15129). Latent, not live: **96 field_3d concepts scanned, ZERO glows name an arc.** But an arc is exactly what an author reaches for when the taught thing IS the angle, and a glow naming one produces the C-1 signature (scene dims to 0.4, nothing lit) with no gate firing. Engine fix deliberately NOT taken here — `field_3d_renderer.ts` belongs to the concurrent #8 session (`docs/CONCURRENT_SESSION_HANDOFF_2026-08-27.md`). |
| ~~—~~ | ~~P3~~ | ~~STATE_7 preposition drift~~ — **FIXED 2026-08-27.** The on-canvas caption (the ≤5-word delta cue, the most-read text on screen) was the sole outlier: one occurrence of "from" against three of "to" in title, `field_3d_config.label` and `s7_4`, plus `s7_2` "the angle **to** the normal". "to" is also the physically correct reading — you measure to the normal, then subtract from ninety. Caption now "Measure to the normal" (4 words, still inside the Rule 32c budget). |
