...
-- ============================================================================
-- Stage 8 · transformer (Ch.7 §7.9, concept 8/8) — NEW field_3d scenario_type
--   build (prefix tfr_, clone-sibling of lc_oscillation). Clean build: NO defect
--   discovered; verify chain green (check:renderer-syntax + tsc 0 + validate
--   132/132). The row below is a probe_definition capturing the scenario's
--   BINDING invariants so the deferred cumulative-EYE (orchestrator, Amendment 4)
--   has automatable checks. FILE only — no DB write (trial).
-- ============================================================================
INSERT INTO engine_bug_queue (bug_class,title,severity,owner_cluster,root_cause,prevention_rule,probe_type,probe_logic,status,concepts_affected,fixed_in_files,discovered_in_session,row_type) VALUES
('field3d_transformer_scenario_binding_invariants',
 'transformer (NEW tfr_ scenario): the build-time invariants a cumulative-EYE run must confirm on the frozen frames — (1) TWO-ZEROS distinction: S1 secondary reads 0 because the loop is mechanically OPEN (secondary switch blade tilted open, secondary beads hidden), S3 secondary reads 0 because the closed loop sees dPhi/dt=0 (blade closed, beads present but static) — the two must render VISIBLY DIFFERENT (Preamble B4). (2) NO-BRIDGE: the primary and secondary bead loops never share a point; no bead ever crosses between the left and right circuits, at any state. (3) S5 THUMB-LOCKSTEP: during the scripted N_s ramp the DOM N_s slider thumb + label move 100->200 in lockstep with the growing secondary winding (round(N_s/10) rings). (4) S3 flux numeral HIDDEN: only the qualitative badge dPhi/dt=0 renders, never the AC-peak 0.090 Wb. (5) S6/S8 gauges glow is PANE-LEVEL (whole power pane brightens, never one bar of the Pp|Ps pair). (6) subscript compose covers all three text paths incl. LETTER subscripts Vp/Vs/Ip/Is/Np/Ns/Pp/Ps (U+209A/209B). (7) S7 line-loss renders at 3dp (3.200 -> 0.032) ONLY here.',
 'MODERATE','peter_parker:renderer_primitives',
 'NEW scenario_type has no prior EYE baseline; these seven invariants are the design decisions most at risk of silent regression on a future clone or edit. Not a discovered defect — a forward probe definition for the deferred cumulative-EYE (this build''s own verify chain is green: check:renderer-syntax OK, tsc 0 errors, validate:concepts 132/132 PASS).',
 'Any future edit to the tfr_ scenario (or a clone of it) re-verifies all seven invariants before seal. A NEW field_3d scenario declaring two electrically-disjoint loops must prove the no-bridge invariant and any dual-zero (open vs closed-dead) distinction on the frozen frames, not just in live play.',
 'manual',
