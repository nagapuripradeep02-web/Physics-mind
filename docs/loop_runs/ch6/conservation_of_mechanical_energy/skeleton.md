...
3. **`E_dissipated` is a STATE FUNCTION, not a path integral** *(union — #10)*: `E_dissipated = E_total(t₀) − K − U_grav − U_spring`, where `E_total(t₀)` is captured once at state entry (and re-captured on `RESET_TRAJECTORY`). Fold-exact for free — it inherits the derived bars' frame-rate independence.
