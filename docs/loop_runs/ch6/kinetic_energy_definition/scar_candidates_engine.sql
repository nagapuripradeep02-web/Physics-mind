...
-- ═══════════════════════════════════════════════════════════════════════════
-- 2026-08-03 · field3d_surgeon · arrow-label glyph height (routed Checkpoint B E3)
-- The live engine_bug_queue row for the bug_class below is already OPEN; on merge
-- it should flip to status='FIXED' with fixed_at = now().
-- ═══════════════════════════════════════════════════════════════════════════
INSERT INTO engine_bug_queue (bug_class, title, severity, owner_cluster, root_cause, prevention_rule,
  probe_type, probe_logic, status, concepts_affected, fixed_in_files, discovered_in_session, row_type)
VALUES
('field3d_arrow_label_sprite_renders_at_under_half_the_body_label_glyph_height',
 'Force-arrow label sprites carried their own glyph height (0.26) with nothing in the file relating it to the body billboard (0.4), so mg / N / F / f rendered at 43 percent of the ink height of the label beside them',
 'MAJOR', 'peter_parker:field3d_surgeon',
 'Every camera-facing label in the newtons_laws_body rig is a 128px-tall canvas carrying a 76px font, so a sprite world scale.y IS its on-screen glyph size. The body billboard was built with a bare literal 0.4 at its build site and the arrow/component labels with an independent NLB_ARROW_LABEL_H = 0.26 — two numbers, no relation between them anywhere in the file, so nothing could notice the force labels had drifted to 65 percent of the billboard em. MEASURED on kinetic_energy_definition STATE_3 frozen at 1280x720: the mg label ink box is 12 x 6 px (13 x 7 at a lower ink threshold) against the cart billboard 72 x 14 — a 0.43 ink-height ratio, on the one state whose job is to introduce mg, where the weight arrow is the only rendered physical cause. Confirmed fleet-wide on normal_force STATE_4, where N, mg, F and f_s are all illegible beside a crisp DOM HUD. This is a SIZE class and is independent of the SEAM Q ink floor: the same labels measure 7.29:1 and 8.42:1 against their backdrop (they sit over the dark page, not the lit slab), well clear of the 4.5:1 text floor. They are not dim, they are small. Fixed by making the billboard height an explicit reference constant (NLB_BODY_LABEL_H) and stating the arrow-label height AS it, so the two can never drift apart again.',
 'A label size is only meaningful RELATIVE to the other labels in the same frame, so never author two label heights as independent literals — name the reference and state the others as it (or as a fraction of it). Three things this fix had to get right. (1) Do NOT compare label sizes by ink bounding box alone: the box is glyph-set dependent, so "mg" (x-height plus descender) and "N" (cap height only) measure 0.69-0.75 em while "m1 = 4 kg" (ascender through descender) measures 0.94 em. Two labels at IDENTICAL sprite scale in this rig already measure a 0.71 ink ratio ("flag" 18x10 vs "cart = 3 kg" 71x14, both at 0.40, kinetic_energy_definition STATE_5) — so an "ink height >= 0.7 x the nearest body label" invariant IS, in practice, "equal glyph size", and any target above ~0.75 would force annotation labels LARGER than the labels naming the bodies. (2) A separation floor stated as a bare number is silently invalidated the moment the thing it protects grows: NLB_LABEL_MIN_SEP was swept and validated as 1.154 x the then-current glyph height, so it is now written that way and tracks it (at the historical 0.26 it still evaluates to exactly the swept 0.30). (3) Distinguish the sprite QUAD height from the INK height when reasoning about lane gaps — the canvas is mostly transparent padding, so the tallest ink a label can draw is 0.594 x the quad height, which is why a 0.40 quad still clears a 0.34 lane gap.',
 'js_eval',
 'For every frozen frame of a newtons_laws_body concept, connected-component the coloured ink inside the 3D viewport (8-connectivity, merge across gaps <= 6 px) and match each component to its sprite by hue and screen position. FAIL if any component belonging to elementType nlb_arrow_label / nlb_comp_label / nlb_cp_label / nlb_mk_label / the theta-arc label has an ink HEIGHT below 0.7 x the ink height of the nearest visible nlb_body_label in the same frame. Static backstop, cheaper and stricter: assert every pmCreateAutoLabel call inside the nlb region passes a height derived from NLB_BODY_LABEL_H, and FAIL on any bare numeric literal there. Collision backstop, which must run in the same pass: FAIL if any two components whose hues match two DIFFERENT label sprites merge into one component — bigger labels collide more easily, and nlbDeCollideLabels only de-collides WITHIN one body, never across bodies.',
 'OPEN',
 ARRAY['kinetic_energy_definition','normal_force','positive_negative_zero_work','work_done_by_constant_force','block_on_incline','connected_bodies','free_body_diagram','friction_force','newton_first_law','newton_second_law','newton_third_law','rolling_friction','tension_force']::text[],
 ARRAY['src/lib/renderers/field_3d_renderer.ts']::text[],
 'ch6-concept3-arrow-label-glyph-height', 'incident');

-- BLOCKING side effect found while verifying the row above. Filed separately
-- because the root cause is NOT the label size — the label size only EXPOSES it.
INSERT INTO engine_bug_queue (bug_class, title, severity, owner_cluster, root_cause, prevention_rule,
  probe_type, probe_logic, status, concepts_affected, fixed_in_files, discovered_in_session, row_type)
VALUES
('nlb_work_bar_track_tops_lose_collinearity_when_a_3d_label_size_changes',
 'A 3D sprite constant silently reaches the DOM work-bar instrument: raising NLB_ARROW_LABEL_H breaks the zero-line collinearity contract of nlb_wk on work_done_by_constant_force STATE_5',
 'MAJOR', 'peter_parker:field3d_surgeon',
 'The nlb_wk work-bar row documents a STRUCTURAL alignment contract (field_3d_renderer.ts, the comment above the nlb_wk_bars div): every track is the first child of a column flex at flex:0 0 auto under align-items:stretch, so every track TOP — and therefore every zero line drawn at top:50 percent inside it — is collinear by construction, because the caption is the only growing child. That contract is what a signed bar is read against; the comment records that the previous violation put 28 J of phantom deflection on the normal-force bar. CAUSALLY ESTABLISHED, not inferred: with NLB_ARROW_LABEL_H = 0.26 the work_done_by_constant_force STATE_5 frozen frame is byte-identical to the approved baseline (0 px differ); with it = 0.40 the slot-0 track top drops ~15 px while slot 1 stays, and three diff regions appear at x = 25..70 (46x16 at y=93, 46x38 at y=164, 46x33 at y=278). Everything else in that frame diff is arrow-label ink in the 3D viewport. The A/B was run through the EYE path both ways with a cache re-seed each time, and the reverted run reproduced the baseline exactly, so this is not run-to-run noise: STATE_5 is deterministic across same-code runs (0 px), while STATE_6 is NOT (1758 px between two identical-code runs — it is the explore state, Rule 37 free-run). The mechanism was not located before the dispatch ceiling. The suspect neighbourhood is the measure-and-reflow fit ladder that the work bars deliberately share with the energy panel, plus the per-frame nlbDodgeBodyLabels pass which calls getBoundingClientRect on the panels every frame; a 3D sprite scale has no legitimate path into a DOM flex layout, so whatever the path is, it is the defect.',
 'No 3D scene constant may be able to change a DOM instrument layout. When a renderer keeps both a 3D scene and measured-and-reflowed DOM instruments, the measurement pass must read only DOM geometry, and any ladder that resizes an instrument must be idempotent and independent of scene state. Verification rule that caught this and should be standard for any change to a shared constant: do not classify an H2 delta by its PERCENTAGE — a 0.48 percent delta hid a broken instrument while a 0.92 percent delta on another concept was pure intended legibility. Connected-component the diff and classify every region by WHERE it is; a region outside the 3D viewport is a regression until proven otherwise. And when a region is unexplained, run the causal A/B through the SAME path that produced the evidence (re-seed the cache and re-run THE EYE with the one constant reverted) — a probe against the review-site build showed ZERO panel movement for the same constant and would have exonerated the change wrongly.',
 'js_eval',
 'On every state of every newtons_laws_body concept, read the getBoundingClientRect of each visible div with class nlb_en_trk in both nlb_bars and nlb_wk_bars. FAIL if any two visible tracks in the same row differ in top by more than 1 px, or differ in height by more than 1 px. Run the assertion twice in the same session with a 3D label constant perturbed between the runs and FAIL if any rect changes at all — a DOM instrument must be invariant under scene-only changes.',
 'OPEN',
