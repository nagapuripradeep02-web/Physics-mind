# Skeleton — `angular_momentum` (rotmech · Class 11 Ch.7 · concept #9 · Phase 0d, Desk C) — REV 3

> **Status:** Phase-0d architect skeleton, **REV 3 — ruling-delta pass, NOT a fix cycle and NOT a
> re-review** (dispatch 2026-08-04). Two founder rulings landed after REV 2 and are the ONLY sources
> of change: **(R1)** the rbr camera is a filed P1 engine gap — **F-C4**,
...
> REV 2 (the fix-cycle-1 resubmission; A1–A13 discharged) preserved at `skeleton_rev2.md` · **this
> file = REV 3, the 2026-08-04 ruling-delta pass** — every edit enumerated with its justification in
> the RULING-DELTA table at the end. Everything the reviewer verified
...
- **Authored pose — ONE pose, all five states (S1–S5):** `{radius: 9.6, theta: π/4, phi: 1.16}`, the
  ...
  spin-circle read (which fails toward side-on — the drum circle collapses to a line and the marker
  stripe's sweep turns into an ambiguous oscillation). The oblique pose is the solve that serves
  both at once. It also inherits the build comment's framing obligation (`:50469–50474`): the pose
...
- **Pose-dependent beats, tagged [POSE-OBLIQUE] in the control table (the blast radius if the pose
  ...
  - **Checked and NOT tagged:** **S3** (`show_l_arrow: false`; the beat rides readouts + chips —
    pose-independent DOM surfaces) and the **drum-marker spin legibility** (S1/S3 focal): the marker
    constrains the pose from the OPPOSITE side — it degrades toward side-on, not top-down — and
    reads clearly at 23.5° elevation. It is a pose-TOLERANCE, not a pose-dependence; recorded here
    so any future pose re-solve respects both constraint directions.
...
'applied_force_at_point'` — DECLARED-INERT (`:950–956`, contract §1), consumed by NOTHING here ✓. **Camera: no field exists** — the rbr config surface (`:978–1060`)
...
*Handoff: REV 3 supersedes REV 2 as the working skeleton — a ruling-delta emission only (the
dispatch is explicit: NOT a fix cycle, NOT a re-review; it inherits REV 2's Checkpoint A position).
On `DESIGN_OK`:
