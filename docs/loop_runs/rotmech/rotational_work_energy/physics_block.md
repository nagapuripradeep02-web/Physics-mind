...
>
> 1. **Numbers in narration are SPELLED OUT** — "minus zero point one five", never "−0.15". The
...
>    instrument table is computed-not-spoken. **Rev 4 (τ=0.15):** the full-stop frame (rendered θ, W)
>    is UNMEASURED at the new τ and — per rev 4's item 3 — now occurs inside **no guided state's real
>    timeline at all** (30.60 s stop > every measured guided timeline, ~23–26 s). Board values are
>    θ=22.95 rad / W=−3.44 J (W is τ-independent; θ is recomputed for the new τ); the OLD rendered
>    figures **−3.45 J / 8.62 rad** were measured only at the retired τ=0.40 and must not be quoted at
>    the new τ. **"−3.44 joules" must not be voiced in any state** either — the value is
>    COMPUTED-BY-THE-STUDENT (DoD (l)), never displayed.
...
> ### ⚖️ REV-4 ADDENDUM 2026-08-14 — quality-auditor F1 (BLOCKING) applied; founder-ruled `tau_brake` 0.40 → 0.15 N·m
>
> The RESEED ruling above and all four banner constraints are **UNCHANGED and still binding.** This
> addendum layers in ONE further founder-ruled change, surfaced by quality-auditor's **F1**: the review
> player derives a state's on-screen duration from **narration length**, not the authored `duration`
> (`timelineTotal = Math.max(narrationEnd, duration*1000)`, `build_review_site.ts:1153` — `duration` is
> a FLOOR, not a cap). At the original `tau_brake = 0.40`, the local full stop (11.475 s) sat INSIDE
> every guided state's REAL narration-derived timeline (measured: S2≈26.1 s · S3≈24.7 s · S4≈23.3 s ·
> S5≈23.8 s), so S2–S5 spent roughly half their on-screen life holding a false rest-clamp frame
> (`τ=0.00 · α=0.00 · KE=0.00 · W=−3.45 · θ=8.62`) while narration and the on-canvas claim asserted a
> torque still acting. **Founder ruling: `tau_brake` 0.40 → 0.15 N·m on S2–S5** (S1 stays 0, no brake;
> **S6 stays 0.05**, its own sandbox default — NOT affected). New local full stop: **30.60 s**, beyond
> every measured guided timeline — no guided state reaches the rest clamp at all, and every held END
> frame now DEMONSTRATES the concept's own mirror claim (`KE₀−KE=|W|`) instead of contradicting it.
> Seven items re-derived below, named exactly against skeleton REVISION 4's own change list
> (`skeleton.md` §"Changed in revision 4"); **nothing else in this document moves.**
...
| alpha = -tau/I, tau=0.15 (rev 4, founder-ruled 0.40->0.15) | -0.05 rad/s^2 | -0.04901960... -> -0.05 | OK exact |
| t_stop = omega0*I/tau | 30.60 s | 30.600000... -> 30.60 (2dp) | OK exact |
...
| theta_stop (continuous form) | "board answer 22.95" | 22.950000... -> 22.95 (2dp) | OK exact |
| theta_stop as **rendered** by the engine | ~22.96 rad predicted (theta-bias; UNMEASURED at tau=0.15) | not independently reproducible without the live renderer - the ~0.012 rad grid bias (measured at the old tau=0.40) predicts 22.95+0.012~22.96 at tau=0.15 - ASSUMPTION, probe-before-authoring | flagged, not re-measured this session; MOOT for the guided arc - the 30.60 s stop is beyond every guided state's real timeline |
| W_stop (continuous form) | -3.4425 -> board -3.44 | -3.442500... -> -3.44 | OK exact |
| W_stop as **rendered** by the engine | likely -3.44 J predicted (was -3.45 J, but that was measured only at the retired tau=0.40; UNMEASURED at tau=0.15) | the bias excess scales with tau: 0.15x0.012~=0.002 J, BELOW the dp-2 quantum - predicted to print -3.44, not -3.45 - ASSUMPTION, probe-before-authoring | flagged, not re-measured this session; MOOT for the guided arc regardless |
| S2 chip match: dt from omega=1.50 to 0.75 at alpha=-0.0490 (rev 4, tau=0.15) | "match ~ 16100 ms" (state-local, pad engaged ~800 ms) | dt=0.75/0.0490196=15300 ms EXACTLY; +800 ms engage = **16100 ms** | OK exact - 0.75/(0.15/3.06) reduces to a clean 15.300 s, no rounding ambiguity (unlike the old 5739 ms figure at tau=0.40) |
...
| I*alpha check on S5's rendered rows | "3.06x(-0.05)=0.153->0.15, matches printed tau" | 3.06*(-0.0490196)=-0.150000... -> -0.15 (exact - tau/I round-trips exactly by definition, unlike the old value which only rounded to -0.40) | OK exact |
...
*rendered/measured, not derivable* - the theta-bias-inflated stop values. At the OLD tau=0.40 these were
measured on the live renderer as 8.62 rad / -3.45 J; at the REV-4 tau=0.15 they have **not** been
re-measured this session (no dev server probe executed this pass - the recomputation above is pure
closed-form kinematics) and are PREDICTED, not measured: theta~=22.96 rad, W likely -3.44 J (the
0.15x0.012~=0.002 J bias sits below the dp-2 quantum, so the one-quantum excess that showed at tau=0.40
likely does not survive at tau=0.15 - **ASSUMPTION - probe-before-authoring**, per rev 4 item 3). They
come from the fixed-16-ms-grid theta offset (~half of h*delta-omega ~ 0.012 rad) baked into the live
integrator. **These values are correctly excluded from every quotable instrument per DoD (l) and are not
used anywhere below** (the `computed_outputs` block, the narration) - **and are now MOOT for the guided
arc regardless of their exact rendered value: the 30.60 s stop lies beyond every guided state's real
narration-derived timeline (~23-26 s measured), so this frame is unreachable in S1-S5 no matter what it
prints.**
...
2. **The exact ms of S2's chip match** depends on the exact authored `engage_at_ms`/`pad_travel_ms`
   (skeleton rev 4: ~16 100 ms; mine, assuming a clean 800 ms travel: 16 100 ms - the decay leg alone is
   an EXACT 15 300 ms at tau=0.15 (0.75/(0.15/3.06) has no rounding ambiguity, unlike the old
   5739/5737.5 ms split at tau=0.40); json_author's exact `pad_travel_ms` decides the last couple of ms
   only insofar as the assumed 800 ms travel differs from the built value).
3. **§4's instrument table has no gap relative to the teaching need.** DoD (l)'s two
   COMPUTED-BY-THE-STUDENT values (theta=22.95 rad, W=-3.44 J at the full stop, rev 4 board values -
   was theta=8.61 rad at the old tau=0.40) are correctly excluded
...
```json
{
  "variables": {
    ...
    "tau_brake": {
      "name": "Magnitude of the constant frictional brake torque opposing the spin — this concept's own characteristic guided value is 0.15 (founder-ruled, rev 4/F1: the review player derives a state's ON-SCREEN duration from narration length, not the authored `duration` — a floor, not a cap; the old 0.40 pushed the local full stop to 11.475 s, INSIDE every guided state's real narration-derived timeline (~23–26 s measured), so S2–S5 spent roughly half their on-screen life holding a false rest-clamp frame; 0.15 moves the stop to 30.60 s, beyond every measured guided timeline — skeleton §3, rev 4)",
      "unit": "N·m",
      "min": 0, "max": 2.0, "default": 0.15, "step": 0.05
    },
    "tau": {
...
| Quantity | S1 (static, τ=0) | S3/S4/S5 entry (t=0, τ engaged) | Local full stop (30.60 s, rev 4) — never pinned, now beyond every guided state's real timeline | S6 entry (t=0, τ=0.05) | S6 @ t=30 s |
|---|---|---|---|---|---|
| I (kg·m²) | 3.06 | 3.06 | 3.06 | 3.06 | 3.06 |
| ω (rad/s) | 1.50 | 1.50 | 0.00 (clamp) | 1.50 | 1.01 |
| KE (J) | 3.44 | 3.44 | 0.00 | 3.44 | 1.56 |
| θ (rad) | — (no row) | 0.00 | 22.95 (board) / ~22.96 (rendered, predicted — ASSUMPTION, probe-before-authoring) | 0.00 | 37.65 |
| α (rad/s²) | — (no row) | −0.05 | 0.00 (clamp) | −0.02 | −0.02 |
| τ (N·m) | — (no row) | −0.15 | 0.00 (clamp) | −0.05 | −0.05 |
| W (J) | — (no row) | 0.00 | −3.44 (board) / likely −3.44 (rendered, predicted — ASSUMPTION, probe-before-authoring; the ≈0.002 J bias excess sits below the dp-2 quantum) | 0.00 | −1.88 |
...
| S2 | `{ m: 2.0, r: 0.80, omega0: 1.50, tau_brake: 0.15, engage_at_ms: ≈pad_travel_ms (≈800) }` | ω₀=1.50 at entry (pad travels in first — the only state that shows the travel); brake becomes effectively 0.15 only AFTER contact (rev 4). THE CALCULATOR should treat `tau_brake` as 0 for an entry-scope check and SKIP a post-engage assertion here (no τ/α row is shown in S2 anyway — §4). |
| S3 | `{ m: 2.0, r: 0.80, omega0: 1.50, tau_brake: 0.15, engage_at_ms: 0, theta0_rad: 0 }` | Pad ALREADY in contact (no travel — Rule 32b: only the taught variable's motion changes); brake engaged from t=0 so θ is exactly the angle turned while the torque acted, per the skeleton's own framing. Fresh θ=0 — first state to show the row. |
| S4 | `{ m: 2.0, r: 0.80, omega0: 1.50, tau_brake: 0.15, engage_at_ms: 0, theta0_rad: 0 }` | Identical entry to S3 ("same machine, same brake, same run") — the readout SET differs (τ drops, KE+bar return), not the physics. Fresh θ=0 (S4 also shows the θ row). |
| S5 | `{ m: 2.0, r: 0.80, omega0: 1.50, tau_brake: 0.15, engage_at_ms: 0, theta0_rad: 0 }` | Identical entry again ("the same decay at the same rate as S3 and S4" — skeleton's own words); θ not displayed in S5 but still seeded at 0 for W's own internal accumulation. |
...
state clock, and that **each state's REAL narration-derived on-screen timeline stays clear of the new
30.60 s local full stop** (§3 restates this per-state; measured guided timelines — S2≈26.1 s,
S3≈24.7 s, S4≈23.3 s, S5≈23.8 s — already clear it with wide margin, per the skeleton's own F1
held-end-frame proof). DoD (j) item (4b) additionally requires driving each of S3/S4/S5 to its REAL
timeline end and asserting `ω > 0` and `KE₀ − KE = |W|` within one display quantum there.
...
| ~800 ms | Contact — `tau_brake_Nm: 0.15` engages (rev 4) | brake engagement |
| ~800 → ~31400 ms (local) | ω decays continuously: ω(t)=1.50−0.0490·(t−0.8)s; KE bar falls continuously from 90.6%; live `ω`,`KE` readouts update every frame | α=−τ/I |
...
Controls: **none**. Margin: chip match at 16 100 ms (rev 4 — was 6539 ms at the old τ=0.40), pin any
≥3000 ms — **the chips carry the numbers, the pin need not land on the match** (skeleton's own words,
confirmed: chip labels are static text, unaffected by the live readout's later value). `glow_focal`:
`rbr_brake_pad` (0→contact, "the cause, travelling") then `rbr_drum` (contact→end, "where the torque
acts"); `hold_glow:["omega","KE"]` whole state.

**No A-28 risk in S2** — S2 shows no `W`/`theta` row, so the full-stop quantum mismatch (which is
specifically a `W` vs `KE` ledger issue) does not apply here regardless of timing. **Rev 4: S2's own
local full stop moves to ≈31 400 ms** (≈800 ms engage + 30 600 ms decay) and is no longer reached at
all — S2's measured real (narration-derived) timeline is ≈26.1 s, so at the state's held end frame the
wheel is still visibly turning: ω ≈ 0.26 rad/s, KE ≈ 0.10 J, bar ≈ 2.7% full, both chips latched from
their earlier match. The old framing ("S2 may safely run to or past its own local full stop") no
longer describes reality — the state's real timeline now ends well BEFORE the local stop, not past it.

### S3 — "Work is torque times angle" · core · no live controls · pin inside the W=τθ window · real timeline ≈24.7 s (< 30.60 s local stop, rev 4)

Entry: r=0.80, m=2.0, ω₀=1.50 (reseed, §2), pad already in contact, `tau_brake_Nm: 0.15` (rev 4),
`engage_at_ms: 0`, `theta0_rad: 0`. **No ω/KE row, no `ke_bar`, no KE chip — DoD (k) invariant.**

| t-window | What animates | Driven by |
|---|---|---|
| 0 → open (whole state) | Rod sweeps continuously from ω=1.50 decaying (`show_r_line: true`, breaking symmetry so the turn is countable); τ reads a flat **−0.15 N·m** for the entire state (well clear of the rest clamp — see the margin check below) | τ=−τ_brake (constant while engaged) |
...
Controls: **none**. **Pin-margin cross-check (rev 4 — this session's addition to the skeleton's own
table):** this state's local full stop moves to **30.60 s** (identical to the canonical decay, since it
reseeds ω₀=1.50 with τ=0.15 engaged from t=0 — §2), well beyond S3's REAL narration-derived on-screen
timeline (**measured ≈24.7 s** — F1). Because `duration` is a floor, not a cap, the internal pin instant
is no longer the binding constraint — **the state's real timeline END is** (skeleton §3 F1 proof table,
independently reproduced this session): at t≈24.7 s the rod is still turning (ω≈0.29 rad/s, underlying,
not itself shown), θ≈22.10 rad, τ still reads a flat **−0.15 N·m** (not the rest-clamp 0.00), and
W≈−3.31 J, still climbing more negative — `W=τθ` holds throughout (−3.31≈−0.15×22.10 to display
precision) and the state never freezes into a false "torque is zero" or "angle stopped" frame.
`glow_focal`: `rbr_r_line` (whole state) + `hold_glow:["W"]` — "the pointer that makes the angle
countable."

### S4 — "The work is the energy lost" · core (PRIMARY AHA) · no live controls · pin ≥ 5.0 s, NOT at the full stop · real timeline ≈23.3 s (< 30.60 s local stop, rev 4)
...
| ~400 ms → pin (recommend ≈7000–8000 ms) → open | **`mirror-fall`**: as W grows more negative, KE (row + bar) falls by the same amount, beat for beat — same decay as S3, same τ=−0.15 the whole time | τ,θ,W,KE all continuing from t=0 |

Controls: **none**. **A-28 cross-check (rev 4):** the local full stop moves to **30.60 s**, beyond S4's
REAL narration-derived timeline (**measured ≈23.3 s** — F1), so the "off by one quantum at the full
stop" risk (W=−3.45 vs ΔKE=3.44, measured on 36 of 40 reachable τ including the OLD 0.40) is now MOOT
for the guided arc — the frame is unreachable within any measured guided timeline. Recommend pin
≈7500 ms (unchanged — well inside the ≥5.0 s / NOT-at-full-stop cell and clear of the 30.60 s stop by a
wide margin). **F1 held end-frame (skeleton §3 proof table, independently reproduced this session):**
at t≈23.3 s (the state's real end) KE≈0.20 J (bar mostly emptied but nonzero) and W≈−3.25 J, still
falling — `KE₀−KE = 3.4425−0.1959 = 3.2466 → 3.25`, matching `|W|=3.25` to display precision: the
mirror claim holds at the state's OWN real end frame, not just mid-state. `glow_focal`:
`rbr_brake_pad` (whole state, "the agent doing the work") + `hold_glow:["KE","W"]` — "the two mirrored
rows."

### S5 — "Where the rule comes from" · advanced · no live controls · pin ≥ 6.0 s · real timeline ≈23.8 s (< 30.60 s local stop, rev 4)
...
| ~4000 ms → pin (≥6000 ms) → open | HELD: full equation on screen; I×α = 3.06×(−0.05) = 0.153 → −0.15 checkable against the live τ row (§0 verified this arithmetic exactly) | — |

Controls: **none**. Margin: assembly completes ~4.0 s, pin ≥6.0 s (skeleton's own figure), replay
window to ~8.0 s — comfortably clear of the new **30.60 s** local stop (≥22 s margin even at the
latest pin). S5's REAL narration-derived timeline is **measured ≈23.8 s** (F1) — well short of the
stop, so the decay is still live at every point the state is on screen. **F1 held end-frame
(independently reproduced this session):** at t≈23.8 s, KE≈0.17 J and W≈−3.27 J,
`KE₀−KE=3.4425−0.1700=3.2725→3.27` matching `|W|=3.27` to display precision — the derivation's
closing `Δ(½Iω²)` line points at a `KE` row that is still genuinely falling when the state ends.
...
> form, preserving the sign — e.g. S3's `−0.15 N·m` (rev 4 — was `−0.40 N·m`) becomes "minus zero
> point one five newton metres". **The F-4 requirement is unaffected and still binding**: S3 must
> speak the DISPLAYED value with the minus, never a bare "0.15". Everything else in §4 (word bands,
> symbol expansion, dialect dual-label, the two anchors) stands as written.
>
> **Rev 4 addendum (2026-08-14):** S3's torque phrase below is now authored ALREADY spelled out
> ("minus zero point one five newton metres"), reflecting the founder-ruled `tau_brake` 0.40 → 0.15
> N·m change — the one number this pass touches. Every other spoken number in §4 is UNCHANGED
> (0.75 · 0.86 · 3.06 · 1.50 · 3.44 are all τ-independent) and remains in whatever form it was already
> authored in; this banner's general directive to `json_author` — spell every remaining digit value
> out at build — still stands for them.
>
...
**S3** (54 words — carries the F-4 fix, Checkpoint A cycle 2; rev 4: torque value updated + spelled
out per the founder ruling): "The brake still presses on the wheel's rim — its turning effect, the
torque tau, keeps the wheel slowing. Two counters run together: the angle turned, theta, and the work
done, W, torque times angle. The torque meter reads minus zero point one five newton metres, meaning
the brake is removing energy from the wheel." *(F-4: quotes the DISPLAYED value with the minus named,
now spelled out per the standing correction; names the sign's meaning explicitly, per founder-proxy's
own wording. Dialect (i-7): "turning effect (torque tau)" dual-labelled once, bare "torque"/"tau"
after.)*
...
2. **τ prints the RESOLVED net torque, never the authored schedule value** — it reads exactly 0.00 once
   ...
   state's REAL narration-derived on-screen timeline is kept clear of the local full stop — now
   **30.60 s** (rev 4, τ=0.15; was 11.475 s at τ=0.40) — specifically to avoid this (§3, per state;
   measured guided timelines S2≈26.1 s / S3≈24.7 s / S4≈23.3 s / S5≈23.8 s all clear it with margin).
...
9. **Dialect (Rule 38d) — confirmed applied.** "Wheel" throughout, never "drum" (my own first S2 draft
   ...
   τ/"torque" (§4).
10. **Pin-margin / A-28 cross-check (rev 4 — restated at the founder-ruled τ = 0.15, quality-auditor
    F1 BLOCKING).** With the §2 reseed reading, S3/S4/S5 each share the SAME local full stop, now at
    **30.60 s** (ω₀=1.50, τ=0.15, I=3.06 — verified exactly in §0; was 11.475 s at the old τ=0.40).
    **The binding discipline is no longer the authored pin/duration — it is the REAL narration-derived
    on-screen timeline** (`build_review_site.ts:1153`: `duration` is a floor, not a cap). Measured:
    S3≈24.7 s, S4≈23.3 s, S5≈23.8 s — all well clear of the new 30.60 s stop, so every held END frame
    (not just the internal reveal pin) now demonstrates the concept's own mirror claim: S3 holds
    θ≈22.10 / τ=−0.15 / W≈−3.31 (still climbing, not clamped), S4 holds KE≈0.20 / W≈−3.25
    (`KE₀−KE=|W|` ✓), S5 holds KE≈0.17 / W≈−3.27 (`KE₀−KE=|W|` ✓) — independently reproduced this
    session against the skeleton's own F1 proof table (§3 above). This SUPERSEDES the old ≤10500 ms /
    ~7500 ms pin-margin recommendations, which were designed around the OLD 11475 ms stop and are now
    obsolete.
...
*Physics block complete for `rotational_work_energy` — produced against skeleton REVISION 3 (no cycle
3), re-derived against **REVISION 4** (quality-auditor F1 BLOCKING, founder-ruled `tau_brake`
0.40 → 0.15 N·m, 2026-08-14) per the rev-4 named 7-item change list; nothing else in this document
moved. Handoff to `alex:json_author` is NOT executed this session per the task brief — this file is the
artifact for a later session to consume. First build obligations for that session, in order: (1) DoD
(j)'s Playwright probe — confirm the §2 reseed reading against the live renderer (or correct it) before
trusting any θ/W/ω figure beyond a state's own t=0, INCLUDING the new item (4b) end-of-real-timeline
assertion; (2) verify the **30 600 ms** pin-margin cross-check (§3/§6-10) against the actual authored
`pad_travel_ms`/pin instants once chosen, and reconfirm the REAL narration-derived timelines measured
this session (S2≈26.1 s / S3≈24.7 s / S4≈23.3 s / S5≈23.8 s) once final `text_en` is built (word-count
edits can shift TTS length); (3) verify S3's τ-before-θ reveal-order gap (§3, S3 build note) reads
acceptably under THE EYE, or fall back to a simultaneous reveal; (4) re-probe the two rendered-stop
values (θ, W at the full-stop frame) at the new τ=0.15 only if a future need requires them — currently
MOOT for the guided arc, unreachable within any guided timeline regardless of their exact print.*
