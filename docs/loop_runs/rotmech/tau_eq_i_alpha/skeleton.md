# Skeleton — `tau_eq_i_alpha` (rotmech · Class 11 Ch.7 · Phase-0b, Desk D) — REV 2 (fix cycle 1)
...
> states unbuildable. Since fix cycle 1 the DIAGNOSIS and the canonical signed-torque semantics live
> at **`docs/loop_runs/rotmech/_engine/findings_d.md` §1** — the single definition site for both
> Desk-D skeletons (P1-3) — which D1 consumes by reference; this skeleton remains the authoritative
...
> **Revision history:** REV 1 = `skeleton.md` (the orchestrating session promotes this file over it
> after verifying completeness). Checkpoint-A report: `founder_proxy_A.md` (cycle 1 — the
> authoritative fix list for this revision). **This is fix cycle 1 of max 2.** The four
> SHARED-CONTRACT items in the response table below are adopted VERBATIM from the orchestrating
> session's reconciliation — desk law, not this skeleton's to renegotiate; the sibling
> `rotational_kinematics` is being fixed against the identical text.
...
|---|---|---|
| **SHARED CONTRACT (orchestrating-session reconciliation — adopted verbatim, both Desk-D skeletons)** | (1) α = the per-step finite difference of ω, from the SAME post-step snapshot as I/ω, blanked across re-pins, is the chapter's ONE definition — this side's REV-1 spec survives, is now DECLARED the shared definition, and the sibling is named as consumer (its analytic τ/I form is retired on its side). (2) The sibling's K8 **motor drive wheel** is the surviving chapter actuator; REV 1's floating tangential arrow is DROPPED as the actuator — the rim force arrow now layers ON the wheel's contact patch, and **contact = the engage instant**, on the same timing contract the brake pad uses. (3) The canonical signed-torque semantics moved OUT of both skeletons to **findings_d §1**; D1 consumes by reference and restates nothing (the decay-only diagnosis is folded there too — the definition site moved, not the finding). (4) ω₀ floor: ONE reconciled office ask — lower to 0 at BOTH sites (`RBR_SLIDER_SPEC.omega0.min`, `:49999`, AND the live-write guard `if (!(value > 0)) return;`, `:50075`); explore-state-only effect | §3 readout metrics; D1; D2; D3; D4; item B; scar rows 155, 82 |
| **P1-1** | Drive actuator un-forked (wheel adopted, above) AND the phantom-arrow beat killed: the cause-motion IS the wheel's travel, and the force arrow appears AT contact — no frame ever draws a force while τ reads 0.00 (fixed at S2, S4, S5 both runs, S7; S3's release = the wheel visibly withdrawing). D4 rebuilt around findings_d §4b's verified plumbing fact: the travel animation is gated on the `rbr_brake_pad` mesh and driven by `eng.padEngageMs`, assigned only in the `brake` branch (`:50521–50526`) — the `applied_torque` branch leaves it null (`eMs` = Infinity, nothing travels), so the travel logic must be lifted onto whichever actuator the state's torque source names | §3 S2/S3/S4/S5/S7/S8; D4; §9; §10(b)/(d) |
| **P1-2** | The finite-difference α is kept (this side's D3 won) and is now the single shared metric, recorded at findings_d §1; D3 names the sibling as consumer | §3 readout metrics; D3 |
| **P1-3** | Neither skeleton is the definition site any more: the semantics live at findings_d §1, outside both; D1 cites by reference and restates nothing; scar row 155 re-dispositioned honestly (REV 1 marked it satisfied — it was satisfied by neither) | D1; scar rows 155, 108 |
| **P1-4** | S6 re-taught honestly, patched from REV 1's text (the aborted partial had NOT fixed this): never-reversing is FRICTION's behaviour, not a law of opposing torques. Title "Friction brake: negative α"; the brake contract is stated **frictional — opposes ω** (the CoAM S5 qualifier the mirroring had dropped); narration scopes the claim in one clause; the reversal physics is named REAL (findings_d §1 semantics) but out of scope, owned by the sibling's sandbox. §4's claim that S6 "kills 'negative α spins it backwards'" is DELETED — that belief is correct physics for a driven opposing torque, not a misconception | §2 S6; §3 S6; §4 |
| **P1-5** | The I and τ patches now COMPUTE on on-screen objects: I at S4 = 0.50 (frame) + 0.50 + 0.50 (each 2 kg mass at 0.50 m) = 1.50 — algebra-only, no Σ, S7's derivation untouched; τ at S2 = the rim arrow's own label × the drum radius: 2.78 N × 0.55 m = 1.53 N·m (S4 repeats the recipe: 1.09 N × 0.55 m = 0.60). The founder ruling request on prerequisites stays visible (item A); the proxy's recommendation (accept the unbuilt state with strengthened patches, no re-sequencing) is adopted | item A patch table; §3 S2/S4; §2 cross-checks; §10(b) |
| **P1-6** | New engine row **D10**: `deriveStateMeta` motion declared from the TORQUE, not the ω-seed alone — ships in the SAME change as D1 (findings_d §6b); walk table updated in both directions (S4/S5/S7 consume it) | D10; walk table |
| **P1-7** | S3's entry ω = S2's end value (**5.56**, now stated per P3-2), drives to **6.56**, then the wheel withdraws and ω holds — the seam no longer changes ω without a torque; S2 → S3 reads as one continuous story | §2 tables; §3 S3; pin table |
| **P1-8** | The τ_net tug is KEPT — this skeleton PICKS the structural buy and says so: a second source (a `sources[]` list summing to τ_net, per-source engage windows, `rbrBrakedSeconds` split, every `eng.tau > 0` guard widened for the signed source — the engine today holds ONE scalar `eng.tau` and ONE window, filed at findings_d §1), with a DESIGNED fallback (one signed `tau_app` control [−2.0, +2.0], tug cut) if Desk E rules simultaneity out of scope. ω = 0 with both engaged is DEFINED: a static hold with breakaway — while the brake is engaged and the drive magnitude ≤ τ_brake, L is held at exactly 0 and `sign(L)` is never consulted (no limit-cycle chatter); when the drive exceeds τ_brake the wheel breaks away smoothly. Bring-up probe (e) added | D1; §3 S8; D5 |
...
| **P2-2** | The `external_torque.source` declared/live enum mismatch (declared `'brake'`/`'applied_force_at_point'`/`'torsion_spring'` vs resolved `'applied_torque'`/`'brake'`, `:50518`) closed in D1's same-change list | D1 |
| **P2-3** | Run B → **2.5 s** (end ω **5.80** — the 6.95 collision with CoAM's headline pulled-in ω removed; REV 1 celebrated it as coherence, but it was two unrelated mechanisms sharing one number on one machine); run A ALSO driven 2.5 s (→ **1.25**), so the equal-time speed ratio IS the α ratio: 5.80/1.25 = **4.64**. A cross-APPARATUS collision scan now sits beside the in-state scan; the 4.64 inertia ratio is kept (forced by the contract's own poses — real coherence); 2.0 s was rejected (→ 4.64, colliding with the ratio itself) | §2 ground truth + cross-checks; §3 S5; §4; pin table |
| **P2-4** | Folded into shared item 4: D2 narrowed (an authored `omega0_rad_s: 0` already works today — verified `:50497`/`:49828`; no new field, no APPARATUS_CONTRACT deviation on the authoring path), office ask = contract-reading confirmation + the two-site floor | item B; D2 |
| **P2-5** | Explicit physics_author instruction: S5 reads the 4.64 off the instruments ONLY and never invites an r² computation (the frame inertia dilutes r² — masses 4× closer is 16× less mass-term inertia but only 4.64× total; the why is #6's job) | §3 S5; §5 |
...
the approved teaching order and NONE is in this wave — and per `4b289d4`, **#5/#6 are not among the
eight pre-registered ids at all**, so their absence is permanent, not temporary (founder_proxy_A).
This concept genuinely uses all three as vocabulary, so its `prerequisites` array will name ids with
no concept JSON. **Concrete Rule-25 consequence, state by state — every patch now COMPUTES, never
merely names (P1-5):**
...
re-sequence the wave. (founder_proxy_A P1-5 recommends: accept, do not re-sequence — the qualitative
spine S1/S2/S3/S6 is self-standing and carries the primary aha. Adopted here; the call stays the
founder's.)
...
Two narrow items go to the office (already recorded at findings_d; kept here for the audit trail):
...
2. **The reconciled ω₀ slider ask — ONE position for the whole desk (shared item 4; supersedes
   REV 1's "keep the floor" AND the sibling's one-site request, which contradicted each other):**
   lower the floor to 0 at BOTH sites — `RBR_SLIDER_SPEC.omega0.min = 0.5` (`:49999`) **and** the
...
The torque's agent is the **motor drive wheel** (shared chapter actuator — the sibling K8's
surviving spec, shared item 2): it translates in from a park pose, visibly turning, and **contact =
the engage instant**, on the same timing contract the brake pad uses; the rim tangential force
arrow layers ON its contact patch while the drive is live. State entry is an instantaneous
single-frame re-pose to the authored ENTRY CONFIG (the 0c-1 general rule, CoAM physics block §2 —
same mechanism, no new machinery). HUD (value-only, 2 dp): `I`, `ω`, `α`, `τ` — **and never `L`,
`KE`, `dL/dt` or `F_pull`** (readout lists are opt-in per state, `:1043`; showing L here would
pre-spoil #9 — scar `teach_do_not_prespoil_a_later_reveal`; this is also the
`teach_inverted_scenario_inverts_cutline_flags` lesson applied: this concept SURFACES the
...
| S5 run B | 0.20 | 0.66 | +1.53 (same) | **+2.32** (2.3182) | from rest → **5.80** in 2.5 s (equal drive time) |
...
| S5 | Same torque, more inertia: less α | SUPPORTING AHA: same τ, I × 4.64 ⇒ α ÷ 4.64 — two equal-time runs, one comparison | `misconception_confrontation` | core (quantitative) |
| S6 | Friction brake: negative α | The signed law via a FRICTION brake: a frictional torque opposing the spin gives α < 0; ω falls at a steady rate to rest, and friction then HOLDS it at rest. **Brake contract: frictional — opposes ω** (the CoAM S5 qualifier, restored at P1-4). Honest scope: never-reversing is FRICTION's behaviour, not a law of opposing torques — a sustained driven opposing torque CAN reverse a spin (findings_d §1 semantics; out of scope here, owned by the sibling's sandbox) | *(straightforward beat)* | extended |
...
| S5 | The SAME torque gives less α when I is bigger — α = τ/I | `two-run-compare` (one-shot A→cut→B, then HELD — coined at P3-1: never looping, so CoAM's looping `cycle-compare` name may not cover it) | run A: 0.80 · 0 (rest — item B) · +1.53 (wheel contact = engage at 0.5 s) · 0; run B (at the cut): **0.20** · 0 · +1.53 · 0 | Run A (the reference — reuses S2's τ and home r deliberately, as CONTINUITY, not verification): wheel travels in 0 → 0.5 s, contact = engage at 0.5 s, drives 0.5–3.0 s (2.5 s), ω 0 → **1.25**, α +0.50; the wheel withdraws at 3.0 s and a chip stamps **"run A: ω = 1.25"** beside the ω readout (static — the held comparison value). CUT at 3.5 s: re-pin cue, readouts blank ≥ 0.5 s (`:50053–50064` [LIVE] + D6), masses re-pose to r = 0.20 in the blanked single frame. Run B: the SAME wheel travels back in 4.0 → 4.7 s, contact = engage at 4.7 s, same τ = +1.53, an EQUAL 2.5 s drive (4.7–7.2 s) — ω races 0 → **5.80**, α +2.32; the wheel withdraws at 7.2 s. HELD: chip 1.25 beside live 5.80, α 2.32, I 0.66 — the whole comparison in one frame, and the equal-time speed ratio IS the α ratio: 5.80/1.25 = **4.64**. Narration states the factor: I is 4.64× smaller, α is 4.64× larger. **physics_author (P2-5): S5 reads 4.64 off the instruments ONLY — never invite an r² computation (the frame inertia dilutes r²; the why is #6's job); the one_line_fix stays "I depends on where the mass sits"**. Anchor (~9 words): "a merry-go-round with riders near the edge speeds up more slowly" | **More inertia: less acceleration** | none | 40–55 | core |
| S6 | A FRICTION brake torque is a negative α: ω falls at a steady rate to rest, and friction then HOLDS it at rest | `translate-through` | 0.80 · +1.50 · 0 · 1.53 (pad engages at 1.5 s; slider default/step overridden per-concept via `config.slider_controls`, `:50005–50014` [LIVE]) | CAUSE: the brake pad translates in to the drum (pad machinery [LIVE], `:50623–50631`; R_drum line + r-line drawn as two distinct labelled lines — CoAM's dual-line discipline inherited). EFFECT after contact: α reads **−0.50**, τ **−1.53**, ω falls 1.50 → 0 over exactly 3.0 s — the same rate S2 added speed, opposite sign (narrated). Rest clamp at 4.5 s: ω holds 0.00, α returns 0.00 while the pad stays on. **Honest scope (P1-4): the brake is FRICTIONAL — it opposes ω, and never-reversing is FRICTION's behaviour, not a law of opposing torques** — narration carries it in one clause ("this brake is friction: it opposes the spin, so at rest it only grips — it cannot spin the wheel backwards; a driven opposing torque could"); the reversal physics is real (findings_d §1 semantics) but out of this concept's scope, taught in the sibling's sandbox | **Brake on: negative α** | brake-torque slider, [0, 2.0], step 0.01, default 1.53 *(min_ring: extended)* | 35–50 | extended |
...
| S8 | Sandbox | `drag-sandbox` | 0.80 · +1.50 · 0 · 0 (home pose per contract) | Free-running (Rule 37). **No idle sweep** — the turntable's own steady spin is the until-first-input motion (an idle τ-pulse cycle would grow ω without bound, and an idle r-sweep would perform CoAM's conservation beat and pre-spoil a LATER concept — both rejected, stated here so json_author does not add one). Live semantics: `tau_app` drag (D5) applies the drive along the home spin sense — the drive WHEEL translates in and is in contact whenever tau_app > 0 and withdraws at 0, its rim arrow tracking the dragged magnitude; `tau_brake` drag opposes (extended ring); **both at once = a live τ_net tug — the α readout shows the difference** (the teacher's own net-torque demonstration), with the ω = 0 rule DEFINED (P1-8): while the brake is engaged and the drive magnitude ≤ τ_brake, the wheel is HELD at rest (L pinned 0, α reads 0.00, `sign(L)` never consulted) until the drive exceeds τ_brake and it breaks away smoothly — no chatter at zero. **Declared fallback:** if Desk E rules simultaneity out of scope (D1 structural note / findings_d §1), S8 drops the tug and exposes ONE signed `tau_app` control [−2.0, +2.0] — designed here so the cheap cut is a design decision, not a mid-build improvisation. `r`/`m` drags re-shape I live; `ω₀` change = restart with re-pin cue ([LIVE] `:50074–50078`; floor 0 under shared item 4). α/τ readouts blank across every re-pin (D3) | **Try it yourself** | ALL, ring-gated: `tau_app` *(core — the taught variable, D5)* · `r` *(core)* · `m` *(core)* · `omega0` *(core)* · `tau_brake` *(extended — cut with S6's ring)* | 0 / open | *(explore)* |

**Archetype audit:** null-result-hold ×2 (S1/S3, declared contrast pair), steady-drive (S2),
converge-on-mark (S4), two-run-compare (S5), translate-through (S6), equation-build (S7),
drag-sandbox (S8). No other repeat; no static state; every archetype discharged by the authored
beat with no teacher input (S6's slider is layered ON an authored beat). Chapter vocabulary settled
(P3-1): `converge-on-mark` is the sibling's spelling, adopted; `cycle-compare` is NOT used here
(CoAM defines it as looping).
...
16 ms grid** (`RBR_GRID_MS`, `:49737`) — **the SHARED chapter definition (shared item 1), recorded
at findings_d §1; the sibling `rotational_kinematics` consumes this same metric and has retired its
analytic τ/I form** — equal to τ_net/I exactly whenever I is constant (every guided beat; zero live
noise — the taught deltas 0 → ±0.50 are full-scale), and honest in the one place they differ (an S8
r-drag during a drive, where dω/dt = (τ − ω·dI/dt)/I — the readout shows the true rate, never a
lie). **α and τ BLANK during every re-pin/restart blank window** (the one-step finite difference
across a cut is a meaningless spike — D3). All rows publish from ONE post-step snapshot per frame.
...
| S2 | none — monotone climb by design; pin photographs mid-climb (ω ≈ 2.96, α +0.50, wheel in contact, arrow on) | ≥ 13 s (author 13) | 7.8 s | claim-by-construction ✓ |
| S3 | withdraw + settle ~2.5 s | ≥ 8 s (P3-3) | 4.8 s | 2.3 s ✓ |
| S4 | match + withdraw 6.9 s (+ 0.033 lag) | ≥ 12 s (author 12) | 7.2 s | 0.27 s ✓ |
| S5 | run-B end + withdraw 7.2 s (+ lag) | ≥ 13 s (author 13) | 7.8 s | 0.57 s ✓ — the pin photographs chip 1.25 beside live 5.80 (the archived frame IS the correct comparison — P2-3 numbers) |
...
| **"A constant torque makes it turn at a constant speed"** (the force→velocity carryover — THE pivot) | **S2** | belief: hold a steady push and the wheel settles at some steady rate · visual_counter: the drive wheel presses steadily and its rim arrow holds perfectly constant while ω climbs past 2.00, 3.00, 4.00… and never settles; the CONSTANT readout is α, not ω · one_line_fix: torque sets the rate of CHANGE of spin — constant torque means constantly increasing speed |
| "The same torque always produces the same acceleration — only how much mass, not where it sits" | **S5** | belief: same push, same speed-up · visual_counter: identical drive wheel, identical τ = 1.53, equal 2.5 s drives — and run B (masses near the axle) reaches 5.80 in the time run A reached 1.25 (chip-stamped); α readout 2.32 vs run A's 0.50 · one_line_fix: α = τ/I, and I depends on where the mass sits — 4.64× less inertia, 4.64× more acceleration |
| | | Named primitives for each wrong picture (scar `field3d_rule16a_belief_unbuildable_for_want_of_a_rod_primitive`): S1 needs the τ/α readout rows (**D3**); S2 needs the rendered drive wheel + rim arrow (**D4**) + the climbing ω under **D1**; S5 needs the two-run restart script (**D6**) + the run-A chip ([LIVE]) |

S3, S4, S6, S7, S8 carry NO misconception_watch. *(REV 1's closing claim that S6 "kills 'negative α
spins it backwards' structurally" is DELETED at P1-4 — that belief is correct physics for a driven
opposing torque, not a misconception; S6 now scopes its never-reversing claim to FRICTION and names
the reversal as real, out-of-scope physics.)* EPIC-C branches: **zero** (Rule 20 [D]; EPIC-L-first).

## 5. `has_prebuilt_deep_dive` states (2)

**S2** (the primary aha — the force→velocity confusion is the single most documented dynamics
misconception, linear or rotational) and **S5** (the I-dependence — where exam mistakes
concentrate). Cache-hint only; V1.0 ships zero authored deep-dives (Rule 18). **P2-5 note for the
deep-dive author:** the r² story belongs to the DEEP DIVE (and ultimately to #6), never to S5's
narration — the live S5 reads the 4.64 off the instruments only.
...
`mass_position_matters` (why r², not just m — deep-dive territory per P2-5) · `alpha_formula_use`
(computing α = τ/I in problems).
...
(#3). All in-chapter, NONE shipped, and #5/#6 are not in the pre-registered build at all — **see
Checkpoint-A item A for the Rule-25 consequence and the per-state COMPUTING patches (P1-5).** No
cross-chapter prerequisites (Newton's second law itself is assumed from Laws of Motion, invoked
only in S7's derivation with its own one-clause restatement).
...
the abstract chapter turntable; the pushing agent is ON SCREEN as the rendered **motor drive wheel
with its rim force arrow** (D4 — shared item 2).
...
|---|---|---|---|---|
| Moment of inertia | `I` (HUD `3.06 kg·m²`) | S1 one-breath (full COMPUTING cliff patch at S4 — P1-5) | S1, after its sentence (`readout_at_ms`) | ✓ |
| Angular speed | `ω` (HUD `1.50 rad/s`) | S1 | S1 | ✓ |
| Torque (net) | `τ` (HUD `0.00 N·m`, signed) | S1 one-breath ("a turning push about the axle"); COMPUTED at S2 (F × R recipe — P1-5) | S1, after it | ✓ |
| Angular acceleration | `α` (HUD `0.00 rad/s²`) | S1 ("how quickly the spin rate changes each second") | S1, after it | ✓ |
| Drive actuator | **motor drive wheel** (shared chapter actuator — sibling K8 spec, shared item 2) | S2, the cause sentence | S2 (travels in; visible from its park pose) | ✓ |
| Drive force at the rim | tangential arrow ON the wheel's contact patch, label `drive F = 2.78 N` (S4: `F = 1.09 N`) | S2, the cause sentence | S2 — appears AT contact, never before (P1-1) | ✓ |
...
| Held comparison | chip `run A: ω = 1.25` | S5, run-A close | S5 | ✓ |
| Braked radius | `R_drum` line, distinct style + label from `r` | S6, "at the rim" (R_drum is SPOKEN at S2's τ recipe; the line itself first draws at S6) | S6 | ✓ |
...
scene glow_focal the scene — S2 glows the drive wheel + arrow (cause) while the ω row carries
instrument hold-glow, so the taught relation's two halves are never mutually dimmed (scars
`ecp_glow_targets_missing_primitives`, `state_glow_focal_dims_one_half_of_the_relation…`).
**P3-6:** the two channels are the established chapter pattern, but Checkpoint B must confirm from
an ACTUAL frame that ONE focal reads on screen.
...
revolution → drive wheel travels in (the cause-motion), contact = engage, arrow appears at contact →
unbounded steady climb · S3 climbing entry → wheel withdraws, arrow leaves with it → spin rate
freezes and holds · S4 re-posed rig → I computed from the rig → formula + chip → wheel travels in,
contact = engage → readout meets chip (match latch) → wheel withdraws, hold · S5 run A from rest
(wheel in, drive, wheel out) → chip stamp → blanked cut + re-pose → run B from rest (wheel back in,
equal drive, wheel out) → held comparison · S6 pad translates in → steady decay → rest clamp holds ·
S7 equation assembly + arrow reveals → wheel travels in → slow replay from rest · S8 free-running
sandbox (steady spin until first trusted input; wheel in contact whenever tau_app > 0). No passive
state; every stated agent is a rendered object (drive wheel + rim arrow, pad); every stated number
is produced by the §3 metrics.
...
- **(i-1) Preset-cut coherence:** *Hide advanced (drop S7):* S1–S6 + S8 coherent — no surviving
  ...
  are all positive; the Unicode-minus ledger row is S6-only); S8 drops tau_brake by min_ring (and
  with it the τ_net tug — see the usability walk's core-only answer, P3-7).
...
**Core-only answer (P3-7):** under `core_only` the tug's tau_brake is cut with its ring, and the
demonstration is the first half alone — drag tau_app, α jumps instantly, ω integrates it; still a
demonstration no whiteboard performs live. (3) *Definition precedes use?* Ledger §10(b), enforced
by `readout_at_ms`.
...
ω = αt at S4) each carry a named one-breath patch in the state that hits them; the I and τ patches
COMPUTE on on-screen objects (P1-5), never merely name.
...
stops 1.50 rad/s in 3.0 s; S6's honest friction scope does NOT weaken this item — the magnitude
arithmetic is identical, and the exam's "constant retarding torque carries ω through zero" variant
is precisely the reversal physics S6 now names as real and the sibling teaches). (iv) inverse
proportionality → S5. Conceptual distractor ("the torque stops — does the wheel stop?") → S3. No
missing piece. (M1–M6 magnetism carve-out: N/A.)
...
call the climb "free" — the drive wheel is right there acting; S4's exactness could plant "same τ,
same α always" — S5 detonates it one click later (that sequencing is deliberate); S6 must not plant
"an opposing torque can never reverse a spin" — its one honest clause prevents exactly that (P1-4).

## Block 2 — Aha-moment designation

- **PRIMARY aha, at S2:** *a torque does not set how fast something spins — it sets how fast the
  spin rate CHANGES; hold a steady torque on and the speed climbs without limit.*
...
1. **D1 `[NEEDS-0c-3]` — SIGNED DRIVE TORQUE (the physics gap; the serious one).**
   ...
   both sources feed `eng.tau` through `Math.abs` (`:50520`, `:50532`). **Semantics: canonical at
   `docs/loop_runs/rotmech/_engine/findings_d.md` §1 — consumed BY REFERENCE by this row and by the
   sibling's K1; NEITHER skeleton restates or redefines them (P1-3; shared item 3; scar
   `two_skeletons_sharing_one_engine_build_state_different_semantics_for_the_same_bought_field`,
   row 155).** Engine-shape requirements this concept adds on top of those semantics:
   - The integrator stays a pure closed form of `eng.t_ms`, piecewise-linear in state-local t with
     breakpoints at engage/release/zero-crossing instants (accumulator-free; Rule 36; a
     `SET_TIME_FREEZE` pin re-evaluates; a pinned rewind reproduces the frame — the trace obligation
     of scar `rewind_path_assertion_stops_at_the_function_body_and_never_follows_its_calls`
     transfers to the 0c-3 report, which must cite the terminating line of every function on the
     rewind path, as `rbrThetaAt`'s backwards-rebuild does at `:49958`). Engage/release knobs as
     today (`engage_at_ms`/`engage_cue`/`release_at_ms`/`release_cue`, `:50522–50524` [LIVE]).
   - **Simultaneous drive + brake (S8's τ_net tug) is a STRUCTURAL buy, not a formula tweak (P1-8;
     filed at findings_d §1):** the engine today holds ONE scalar `eng.tau` and ONE engage window
     (`eng.brakeOnMs`/`brakeOffMs`, written once at `:50520–50534`), and the `src` branch picks
     EITHER brake OR applied_torque. The buy is a `sources[]` list (each entry: type, signed τ, its
     own engage window) summing to τ_net, `rbrBrakedSeconds` (`:49927`) split per source, and every
     `eng.tau > 0` guard widened to `!== 0` for the signed source. **This skeleton PICKS: keep the
     tug, price it structurally.** DESIGNED fallback if Desk E rules simultaneity out of scope: one
     signed `tau_app` control [−2.0, +2.0], tug cut (§3 S8 carries the fallback design).
   - **ω = 0 with both sources engaged (P1-8):** static hold with breakaway — while the brake is
     engaged and the drive magnitude ≤ τ_brake, L is held at exactly 0 and `sign(L)` is never
     consulted; when the drive exceeds τ_brake, smooth breakaway. No limit-cycle chatter.
   - **Enum closure (P2-2), same change:** the interface declares `external_torque.source:
     'brake' | 'applied_force_at_point' | 'torsion_spring'` while the implementation resolves
     `'applied_torque' | 'brake'` (`:50518`) — declare `'applied_torque'` as a member and drop or
     explicitly defer the two never-implemented members (scar
     `deferred_enum_members_must_be_declared_not_merely_unimplemented`).
   - Absent-field clause: no `applied_torque_Nm` ⇒ behaviour identical to today. Bring-up probes:
     (a) from rest, τ_app = 1.53, I = 3.06 constant — assert ω(t) = 0.50·t to 1e-9 over 20 s;
     (b) frame-dt fold with the grid h held fixed (20 steps at dt = h vs 10 at dt = 2h), assert
     |Δθ| and |Δω| < 1e-12; (c) drive + brake simultaneously engaged, assert
     dω/dt = (τ_app − τ_brake·sign(ω))/I segment-wise away from ω = 0; (d) drive against the spin
     from ω = +1.50, assert a smooth sign change with NO clamp event; (e) from spin with
     τ_brake > τ_app both engaged — assert L reaches exactly 0 and HOLDS with zero chatter, then
     raise τ_app past τ_brake and assert smooth breakaway. **Without D1: S2, S3, S4, S5, S7, S8 —
     six of eight states — cannot exist.**

2. **D2 `[no engine work — rides D1; office confirmation only]` — start from rest (NARROWED at
   P2-4 / shared item 4).** Authoring `omega0_rad_s: 0` is ALREADY read correctly today
   (`rbrNum` typeof, `:49828`; seed at `:50497`) — a literal 0 is honoured, so S4/S5/S7 need **no
   new field and no APPARATUS_CONTRACT deviation on the authoring path**; the only block is D1's
   decay-only physics (L₀ = 0 never moves). What remains: (a) D1 lands; (b) the office CONFIRMS the
   contract reading (§1 pins the concept-opening pose only — item B.1); (c) the reconciled ω₀
   slider ask (item B.2, shared item 4): floor 0.5 → 0 at BOTH sites — `RBR_SLIDER_SPEC.omega0.min`
   (`:49999`) AND the live-write guard `if (!(value > 0)) return;` (`:50075`). Explore-state-only
   effect; supersedes REV 1's "keep the floor".
...
3. **D3 `[NEEDS-0c-3]` — `alpha` and `tau` readout rows.** `RBR_RO_META` (`:50147–50154`) closes at
   ...
   `ascii_minus_in_oncanvas_math_from_tofixed` sweep discipline). Metrics exactly as §3: **α = the
   per-step finite difference (ω_k − ω_{k−1})/h on the 16 ms grid — the SHARED chapter definition
   (shared item 1), recorded at findings_d §1; the sibling `rotational_kinematics` consumes this
   same metric (its analytic τ/I form is retired on its side)**; τ = the signed authored schedule
   value at t. Both publish from the same post-step snapshot as I/ω; **both BLANK during re-pin
   blank windows** (the cross-cut finite difference is a spike, not a measurement). **Loud-warn
   (P2-1, findings_d §2): an unknown readout token must `console.warn` once per state instead of
   skipping in silence — that warning is worth more than the rows themselves; it converts an
   invisible authoring error class into a visible one for every future rbr concept.** The existing
   `dLdt` row is NOT an acceptable substitute — it prints L-language this concept must not show
   (ledger §10b). `readout_at_ms` (`:1047` [LIVE]) already gates per-row reveal — S1 uses it.
   Scriptable knobs: `readouts` list per state, reveal ms per row, `hold_glow` membership. Absent
   from a state's list ⇒ row not built (today's behaviour, plus the warn).

4. **D4 `[NEEDS-0c-3]` — the MOTOR DRIVE WHEEL, the shared chapter actuator (shared item 2 — the
   sibling K8's surviving spec; REV 1's floating tangential arrow is DROPPED as the actuator).**
   The taught cause must be a rendered object (scars
   ...
   brake (`:50623–50631`). Build ONE mesh with TWO consumers (findings_d §4 discipline): a motor
   drive wheel that translates in from a park pose to contact the drum, visibly turning while
   engaged, **contact = the engage instant**, on the same timing contract the brake pad uses —
   plus, for THIS concept, a tangential force arrow layered ON the contact patch while the drive is
   live (label `drive F = <value> N`; torque = force at the rim × R_drum — the P1-5 recipe; length
   through the existing bounded `rbrArrowLen` map, `:49829` — knee + soft saturation + min floor; a
   single arrow, no ratio pair, so the map's dynamic range is safe). **Verified plumbing fact
   (findings_d §4b): the travel animation is gated on the `rbr_brake_pad` mesh and driven by
   `eng.padEngageMs`, assigned ONLY in the `brake` branch (`:50521–50526`; travel `:50729–50744`);
   the `applied_torque` branch leaves it null, so `eMs` = Infinity and nothing travels — the travel
   logic must be lifted off the brake-pad mesh onto whichever actuator the state's torque source
   names.** Visibility follows the SOURCE like the pad's does (drive active ⇒ wheel in contact +
   arrow on; release ⇒ wheel withdraws, arrow leaves with it) — never a stray overlay (the 0c-1
   stray-overlay lesson, `:50596–50606`). Element types register in the exact-token list
   (`:50586–50592`); `deriveStateMeta` co-edited in the SAME change if reveal keys are added.
   **Force-map note (P3-8, findings_d §4):** D4's arrow and D7's arrows ride the FORCE map
   (`rbrArrowLen`) with two consumers priced as ONE mechanism; the sibling's K4 velocity arrows
   ride their OWN map — correct per scar
   `velocity_arrows_routed_through_a_force_arrow_map_collapse_their_ratio…`; stated here so Desk E
   never reconciles it ad hoc. Scriptable knobs: engage/release (shared with D1's), arrow label
   text, park pose. Absent ⇒ no wheel, no arrow, today's scenes unchanged.
...
5. **D5 `[NEEDS-0c-3]` — `tau_app` slider token.** `RBR_SLIDER_TOKENS` (`:49995`) and
   ...
   **step 0.01** (1.53 and 0.60 must be reachable; note S6 also overrides tau_brake's default 0.92 /
   ...
   (`:50079–50088`); the drive wheel's contact + arrow track the dragged magnitude (D4). **Under
   D1's declared fallback (simultaneity cut), this token becomes the signed range [−2.0, +2.0] —
   one control, tug dropped (P1-8).** Wire into `rbrApplyParam`, `rbrSyncSliderRow`, PARAM_UPDATE
   emit. Hidden rows keep their reserved slot (`visibility:hidden`, `:49991`, `:50137` [LIVE]).
   Absent from `controls_visible` ⇒ hidden as today.
...
7. **D7 `[NEEDS-0c-3]` — per-particle TANGENTIAL force arrows (S7 only).** The existing pull arrows
   ...
   revealed one-per-cue, riding the rotating masses. On the FORCE map with D4 — one
   tangential-vector mechanism, two consumers (P3-8, findings_d §4). Registers in the exact-token
   list; labels decollided from the r-label hysteretically (scars `field3d_label_sprite_overlap`,
...
10. **D10 `[NEEDS-0c-3, ships WITH D1]` — `deriveStateMeta` motion declared from the TORQUE, not
    the ω-seed alone (P1-6; findings_d §6b).** `deriveStateMeta.ts:496–508` declares rbr motion
    from the seed alone (`if (w0 >= 0.05) out[stateId] = true`), so a state seeded at rest falls
    through `undefined` and THE EYE's D5 motion gate silently skips — and this concept authors
    EXACTLY the three from-rest states the gate exists for (S4, S5, S7: `omega0_rad_s: 0` + an
    engaged drive that moves the body a great deal). Required: the motion declaration also
    inspects the torque schedule (any engaged signed source ⇒ motion true), co-edited in the SAME
    change as D1 (findings_d ranks it "HIGH — must ship WITH §1"). Distinct from D4's reveal-keys
    co-edit (that is scar
    `derivestatemeta_new_scenario_key_absent_from_f3d_reveal_keys_falls_through_to_pcpl`; this row
    is the MOTION gate). Absent ⇒ today's behaviour — and THE EYE goes silent on precisely the
    three states 0c-3 exists to enable; that is why it ships with D1, not after.
...
- **F-b (legibility, decision requested, non-blocking):** under a held max drive in S8, ω grows
  ...
  the CoAM corner-case posture; founder_proxy_A concurs), (ii) a soft visual treatment at high ω.
  No engine work assumed.
- **F-c:** item B verbatim (NARROWED at fix cycle 1: contract-reading confirmation + the two-site
  ω₀ floor ask — shared item 4; no deviation on the authoring path).
...
| S2 | D1 (drive), D3, D4 (wheel + arrow), D9 (cues, phases) |
| S3 | D1 (release), D3, D4 (withdraw), D9 |
| S4 | D1, D2 (rest entry), D3, D4, D8 (formula_at_ms), D9 (chip + match latch, r-line), D10 (from-rest motion gate) |
| S5 | D1, D2, D3, D4, D6 (run script), D9 (chips ×2, re-pin cue), D10 |
| S6 | D3, D9 (brake pad + cues + rest clamp + slider override, drum line + r line) |
| S7 | D1, D2, D3, D4, D7 (F_t arrows), D8, D9, D10 |
| S8 | D1 (live drive + net tug + ω=0 hold), D3 (blank on restarts), D4 (wheel tracks tau_app), D5 (tau_app token), D9 (sliders, seize, Rule-37 free-run) |

Reverse: D1 ← S2,S3,S4,S5,S7,S8 · D2 ← S4,S5,S7 · D3 ← all · D4 ← S2,S3,S4,S5,S7,S8 · D5 ← S8 ·
D6 ← S5 · D7 ← S7 · D8 ← S4,S7 · D9 ← all · D10 ← S4,S5,S7. Every D-row claimed by ≥1 state ✓;
every state claims ≥1 row ✓; every primitive named in §3/§4/§10(b) maps to a D-row or a cited
[LIVE] surface ✓.
...
queried: nothing beyond the five commands above. **Fix-cycle-1 re-dispositions: rows 4, 24, 25,
27, 28, 29, 61, 82, 108, 155 (the wheel adoption, the findings_d §1 definition-site move, D10, and
the P2-3 number changes); all other verdicts carry over from REV 1 unchanged on an unchanged
design surface.**
...
| 2 | `architect_authors_a_force_triangle_whose_ratio_exceeds_the_renderer_arrow_maps_dynamic_range` | B-sat — D4 single rim arrow (on the wheel), D7 equal pair; no ratio pair; bounded `rbrArrowLen` cited `:49829` |
...
| 4 | `architect_reuses_a_marker_mechanism_without_diffing_the_side_effects_its_presence_switches_on` | B-sat — chip renders inline in its readout row only (`:50165–50176`); pad visibility follows source+τ (`:50623–50631`), so τ_brake-0 states never show a stray pad; the D4 wheel reuses the pad's TIMING CONTRACT with the travel plumbing gap diffed explicitly (findings_d §4b — `padEngageMs` never reaches the applied source) |
...
| 24 | `deferred_enum_members_must_be_declared_not_merely_unimplemented` | B — every 0c-3 field ships IMPLEMENTED under this spec; no new inert declarations; the one formerly-"implemented" member (`applied_torque_Nm`) re-specified honestly (D1), and the `source` enum's two never-implemented members closed or explicitly deferred in D1's same change (P2-2) |
| 25 | `derivation_principle_applied_to_one_beat_but_not_its_sibling` | B-sat — the rendered-cause principle applied at EVERY torque event: D4 wheel + rim arrow in S2/S3/S4/S5(both runs)/S7/S8, pad in S6, both in S8 |
...
| 27 | `derived_readout_asserted_by_value_without_defining_its_metric` | B-sat — §3 metrics defines all four rows incl. α's finite-difference metric (the SHARED chapter definition, findings_d §1) and its honest explore-case divergence |
| 28 | `derivestatemeta_new_scenario_key_absent_from_f3d_reveal_keys_falls_through_to_pcpl` | 0c-3 — rbr keys landed at 0c-1; D4/D7/D8 co-edit deriveStateMeta in the SAME change if reveal keys added; the MOTION-gate half is its own row now (D10, P1-6) |
| 29 | `directive_no_gate_asks_whether_a_teacher_could_use_it` | B-sat — teacher-usability walk (§10), incl. the τ_net tug AND the core-only answer (P3-7) |
...
| 55 | `field3d_nlb_body_screen_anchor_lands_under_the_dom_slider_panel` | 0c-3 layout note — D4 wheel + arrow anchor at the rim, clear of the bottom-right panel |
...
| 61 | `field3d_rule16a_belief_unbuildable_for_want_of_a_rod_primitive` | B-sat — §4 names each belief's primitives (D3/D4 wheel+arrow/D6 + live chip) |
...
| 68 | `frozen_pin_unbudgeted_on_a_sequential_misconception_state_can_archive_the_wrong_picture` | B-sat — pin table budgets every sequential state; S5's pin archives the COMPLETED comparison (chip 1.25 beside live 5.80) |
...
| 81 | `named_primitive_declared_without_the_surface_that_can_render_it` | **B — the row that forces D3–D8 and D10:** every named primitive maps to a live surface or a D-row (walk table, both directions) |
| 82 | `narration_attributes_an_effect_to_a_cause_the_model_does_not_contain` | B-sat — D1 puts the drive IN the model; D4 renders it as the shared motor drive wheel + rim arrow with contact = engage, so no frame draws a force while τ reads 0.00 (P1-1); the pad renders the brake |
...
| 93 | `nlb_frictionless_state_with_an_opposing_applied_force_reverses_and_unwinds_its_own_work_ledger` | B-analog-sat — the FRICTION brake never reverses (rest clamp, `:49942`; S6 scopes the claim to friction, P1-4); the DRIVE legitimately grows ω and there is no work ledger to unwind (no bars) |
...
| 108 | `paired_skeletons_cite_each_other_by_state_number_across_a_renumbering` | B-sat — CoAM cited by content + REV; the sibling consumes findings_d §1 and this skeleton's D-rows BY NAME, never by state number; the wheel spec is cited as "the sibling K8's surviving spec" with its content restated here, so a sibling renumbering cannot orphan it |
| 109 | `phase0_config_enum_closed_against_the_spec_driver_not_the_served_concept_set` | **B — the headline:** 0c-1's "applied_torque_Nm = #7 with no extra code path" comment (`:947–949`) is false for the driving half; the correction now lives at findings_d §1 (the canonical site); this skeleton is the concept-side scope statement |
| 110 | `phase0_union_lists_capabilities_but_never_which_knobs_are_scriptable` | B-sat — every D-row lists its scriptable knobs |
| 111 | `phase0_union_table_asserted_not_walked_state_by_state` | B-sat — walk table, both directions (incl. D10) |
| 112 | `ppc_probe_points_primitive_new_build` | N/A — precedent record |
| 113 | `prior_rulings_and_internal_ledgers_are_not_re_run_over_the_states_a_restructure_creates` | B-declared-and-executed — this fix cycle re-ran the ledger, cross-checks, pin table, walk table and this audit over the changed states (S3/S5 numbers, wheel choreography); on any future restructure they re-run again |
| 114 | `quantitative_check_state_reuses_the_exact_numbers_and_the_delta_cue_of_the_state_it_verifies` | B — designed against: S4 uses entirely fresh numbers (r 0.50 · I 1.50 · τ 0.60 · α 0.40 · ω 1.20 · F 1.09), none of S2's; every delta cue distinct; S5-run-A's reuse of S2's τ/pose is declared CONTINUITY (the reference run), not verification — and its end value 1.25 is its own (2.5 s, not S2's 8.11 s) |
| 115 | `radius_scenario_F_r_label_kerning_collision` | 0c-3 — D4/D7 label kerning vs the r-label |
| 116 | `ramp_endpoints_multiply_the_taught_variable_by_a_factor_no_rendered_string_claims` | B-sat — the 4.64× factor is narrated in S5 and legible in the held HUD pair + chip (1.25 vs 5.80, equal drive times) |
...
| 120 | `rule31_motion_floor_satisfied_by_a_late_label_reveal_over_a_frozen_canvas` | B-sat — every state's motion is physical spin/climb/decay (+ the wheel's own travel); no label-only state |
...
| 124 | `signed_engine_union_drops_items_its_own_state_table_still_consumes` | B-sat — REV 2 renumbers nothing (same 8 states); the walk table binds both directions incl. the new D10; any future renumbering shows the old→new mapping |
...
| 138 | `state_glow_focal_dims_one_half_of_the_relation_the_state_exists_to_teach` | 0d/B-noted — §10(b): scene focal (drive wheel + arrow) and instrument hold-glow (ω row) ride separate channels, so cause and effect are never mutually dimmed; Checkpoint B reads an actual frame (P3-6) |
...
| 141 | `taught_variable_has_no_rendered_physical_correlate_so_a_text_label_is_the_only_cause` | B — forces D4 (wheel + arrow); α's correlate is the visibly quickening/slackening spin read against the constant cause |
...
| 147 | `teach_do_not_prespoil_a_later_reveal` | B-sat — L/KE/dLdt/F_pull never shown; Σ-form S7-only; negatives S6-only; no ω–t graph (belongs to #4); S6 names the reversal as real WITHOUT performing it (the sibling's sandbox owns the demo) |
...
| 155 | `two_skeletons_sharing_one_engine_build_state_different_semantics_for_the_same_bought_field` | **B — re-dispositioned at fix cycle 1 (REV 1 marked this satisfied; founder_proxy_A showed it was satisfied by NEITHER skeleton — each claimed the definition site):** the canonical signed-torque semantics now live OUTSIDE both skeletons at findings_d §1; D1 here and the sibling's K1 both consume BY REFERENCE and restate nothing (shared item 3). The α metric is likewise single-sited (shared item 1 — this side's finite difference, recorded at findings_d §1, sibling consumes). The drive actuator is single-specced (shared item 2 — the sibling's K8 wheel, consumed here at D4). D8's formula-reveal semantics remain reconciled chapter-wide by Desk E (F-d) |
| 156 | `velocity_arrows_routed_through_a_force_arrow_map_collapse_their_ratio_and_cannot_draw_a_true_zero` | B-checked — no velocity arrows here; D4/D7 are genuine forces on the force map (P3-8 note in D4); a zero drive HIDES the wheel + arrow (visibility), never draws a zero-length one |
| 157 | `verification_via_applystate_bypasses_player_false_hang` | 0d — verification drives the real player |

---

*Handoff: → founder-proxy **Checkpoint A re-review** (fix cycle 1 complete; cycle 2 available if
needed). On `DESIGN_OK`: the orchestrating session promotes this file into `skeleton.md`, then the
physics block for this concept, then the sibling `rotational_kinematics` proceeds against the
identical shared-contract text (it consumes findings_d §1 + D3/D8 by reference and adds the
θ/kinematics rows). The dispatching session also: (1) confirms findings F-a…F-d + items A/B are
reflected in `docs/loop_runs/rotmech/_engine/findings_d.md` for Desk E's 0c-3 scope freeze (the
shared-contract items and §4b are already filed there); (2) appends `tau_eq_i_alpha` to the OPEN
`teach_visual_must_match_narration` row at 0d. NO concept JSON until the 0c-3 PR merges and this
desk syncs.*
