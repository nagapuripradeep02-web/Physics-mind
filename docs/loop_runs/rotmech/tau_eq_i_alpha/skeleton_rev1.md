# Skeleton — `tau_eq_i_alpha` (rotmech · Class 11 Ch.7 · Phase-0b, Desk D) — REV 1
...
> states unbuildable. This skeleton is the authoritative statement of both.
...
|---|---|---|
| torque τ | S1 (the "τ = 0.00" readout), S2 (the drive) | S1: "a torque is a turning push about the axle — right now there is none." S2's rendered rim-force arrow makes it concrete. |
...
| moment of inertia I | S4 (the equation) | S4: "moment of inertia I measures how spread out the mass is — the readout computes it from the masses' positions." |
...
### B. APPARATUS_CONTRACT deviation request — entry ω = 0 for S4, S5, S7 (chapter-wide decision)

The contract pins the home pose at ω = +1.50 rad/s and this concept OPENS there (S1, S2, S6, S8 all
enter at the home pose — the "one continuing machine" rule holds at concept entry and at the explore
state). But the canonical τ = Iα picture — the one every textbook and every exam item draws — is a
torque applied to a body AT REST that then spins up, with α legible as the steady climb of ω from
zero. S4 (the quantitative beat), S5 (the I-comparison) and S7 (the derivation replay) need entry
`omega0_rad_s: 0`. Verified: the config READ path already honours an authored 0 (`rbrNum` is
typeof-resolved, `:49828`; seed at `:50497`, `Math.abs(0) = 0`) — the block is purely D1's
decay-only physics, not a config gap. The ω₀ SLIDER keeps its 0.5 floor (`:49999`) — the deviation
is authored-entry-only, no control change. The sibling `rotational_kinematics` skeleton will need
the same ruling. **Written here per APPARATUS_CONTRACT §4; the dispatching session should copy this
section into `docs/loop_runs/rotmech/_engine/findings_d.md` for the office.**
...
|---|---|---|---|---|---|
| Home / S1, S2, S6, S8 entry | 0.80 | 3.06 | S2 drive **+1.53** · S6 brake **1.53** | S2 **+0.50** · S6 **−0.50** | home ω 1.50; S2 climbs 0.50/s unbounded; S6: 1.50 → 0 in 3.00 s |
| S3 (after-drive hold) | 0.80 | 3.06 | +1.53 then **0** at 2.0 s | +0.50 then **0.00** | entry 3.00 → holds **4.00** |
| S4 (fresh numbers) | **0.50** | **1.50** | **+0.60** | **+0.40** | from rest; predicted ω after 3.0 s = **1.20** |
| S5 run A | 0.80 | 3.06 | +1.53 | +0.50 | from rest → **1.50** in 3.0 s (chip-stamped) |
| S5 run B | 0.20 | 0.66 | +1.53 (same) | **+2.32** (2.3182) | from rest → **6.95** in 3.0 s |
...
2.3182 × 3.0 = 6.9545 → 6.95; I ratio 3.06/0.66 = **4.64** = α ratio 2.32/0.50 = 4.64 (numerically
identical to CoAM's I-ratio — deliberate chapter coherence, and S5's 6.95 equals CoAM's pulled-in ω).
...
|---|---|---|---|---|
| S1 | No torque: the spin does not change | Baseline + null: τ = 0 ⇒ α = 0 ⇒ ω constant; readouts built; kills "a spinning object needs a torque to keep spinning" | *(straightforward beat)* | core (qualitative) |
| S2 | Torque on: the spin keeps speeding up | **THE PRIMARY AHA:** constant τ → ω climbs and never settles; α is the constant thing | *(straightforward beat)* | core (qualitative) |
| S3 | Torque off: the speed stays | The other half: removing τ removes the CHANGE, not the motion | *(straightforward beat)* | core (qualitative) |
...
| S6 | Opposing torque: negative α | The NET/signed law: a brake torque gives α < 0, ω falls at a steady rate, stops and HOLDS (never reverses) | *(straightforward beat)* | extended |
...
**Coined archetypes (two, justified once):**
...
- `converge-to-mark` — a live readout sweeps toward a pre-committed prediction chip and the match
...
| S2 | A constant torque does not set the speed — it sets the RATE the speed changes | `steady-drive` | 0.80 · +1.50 · 0 (engages at 4.89 s) · 0 | Pre-roll: one full slow revolution (2π/1.50 = 4.19 s). CAUSE: the **drive arrow** (D4) appears at the drum rim, tangential, labelled — static for a 0.7 s beat. THEN τ = +1.53 engages: ω climbs at exactly 0.50/s — 1.50 → 2.00 → 3.00 → past every value, to state end, never settling; α readout sits CONSTANT at +0.50 (the steady thing), τ readout +1.53. The spin visibly quickens each revolution. Anchor (~8 words): "like steadily pushing a playground merry-go-round" | **Torque on: speed climbs** | none | 40–55 | core |
| S3 | Removing the torque removes the change, not the motion | `null-result-hold` — **declared contrast pair of S1** (delta names the flip: S1 holds the NEVER-driven speed 1.50; S3 holds a DRIVEN-UP speed 4.00 — absence of torque preserves whatever ω the torque left behind) | 0.80 · +3.00 · +1.53 (engaged at entry) · 0 | Entry mid-drive: arrow on, ω climbing 3.00 → 4.00 over 2.0 s (α +0.50). CAUSE: at 2.0 s the drive arrow visibly retracts (release cue). After a readable beat the EFFECT: α snaps to 0.00, τ to 0.00 — and ω HOLDS at 4.00 for the remainder. Nothing slows it | **Torque off: speed stays** | none | 25–40 | core |
| S4 | τ = Iα is exact: it predicts the outcome before the run | `converge-to-mark` | **0.50** · **0 (from rest — deviation B)** · +0.60 (engages at 3.9 s) · 0 | Fresh pose (masses visibly at mid-rod; r-line labelled 0.50). The formula surface **τ = Iα** appears whole at its defining sentence (D8); the chip **"predicted ω = 1.20 after 3 s"** stamps beside the ω readout (chip form, `:1021–1030`, `:50165–50176` — [LIVE]). CAUSE: drive arrow appears (0.7 s beat), τ = +0.60 engages at 3.9 s. EFFECT: ω sweeps 0.00 → 1.20 over 3.0 s (α readout constant +0.40); at 6.9 s the drive releases exactly as ω arrives — **match cue co-glows chip and readout** (latch, tolerance 0.01) — and ω holds 1.20 | **Equation predicts the speed** | none | 40–55 | core |
| S5 | The SAME torque gives less α when I is bigger — α = τ/I | `cycle-compare` (one-shot A→cut→B, then HELD — not looping) | run A: 0.80 · 0 (rest — deviation B) · +1.53 (engages 0.5 s) · 0; run B (at the cut): **0.20** · 0 · +1.53 · 0 | Run A (the reference — reuses S2's τ and home r deliberately, as CONTINUITY, not verification): drive 0.5–3.5 s, ω 0 → 1.50, α +0.50; at 3.5 s a chip stamps **"run A: ω = 1.50"** beside the ω readout (static — the held comparison value). CUT: re-pin cue, readouts blank ≥ 0.5 s (`:50053–50064` [LIVE] + D6), masses re-pose to r = 0.20 in the blanked single frame. Run B: SAME drive arrow, same τ = +1.53, 3.0 s — ω races 0 → 6.95, α +2.32. HELD: chip 1.50 beside live 6.95, α 2.32, I 0.66 — the whole comparison in one frame. Narration states the factor: I is 4.64× smaller, α is 4.64× larger. Anchor (~9 words): "a merry-go-round with riders near the edge speeds up more slowly" | **More inertia: less acceleration** | none | 40–55 | core |
| S6 | An opposing torque is a NEGATIVE α: ω falls at a steady rate, reaches rest, and holds — never reverses | `translate-through` | 0.80 · +1.50 · 0 · 1.53 (pad engages at 1.5 s; slider default/step overridden per-concept via `config.slider_controls`, `:50005–50014` [LIVE]) | CAUSE: the brake pad translates in to the drum (pad machinery [LIVE], `:50623–50631`; R_drum line + r-line drawn as two distinct labelled lines — CoAM's dual-line discipline inherited). EFFECT after contact: α reads **−0.50**, τ **−1.53**, ω falls 1.50 → 0 over exactly 3.0 s — the same rate S2 added speed, opposite sign (narrated). Rest clamp at 4.5 s: ω holds 0.00, α returns 0.00 while the pad stays on — the spin NEVER reverses (rest clamp, `:49942` [LIVE]) | **Opposing torque: negative α** | brake-torque slider, [0, 2.0], step 0.01, default 1.53 *(min_ring: extended)* | 35–50 | extended |
| S7 | The law is Newton's second law summed over the body's particles: τ = (Σmr²)α = Iα | `equation-build` | 0.80 · 0 (rest — deviation B) · +1.53 (engages at 4.5 s) · 0 | The equation assembles synced to narration on the single formula surface (D8): F = ma per particle → the tangential force each mass needs is m·(rα) → its torque m·r²·α → summed: τ = (Σmr²)α = Iα. Per-mass **tangential force arrows** (D7) reveal one per sentence, equal by symmetry (0.80 N each), riding the masses. Then the slow drive replay runs 4.5–7.5 s (ω 0 → 1.50) with the arrows on and the ledger narrated: 0.64 + 0.64 + 0.25 = 1.53 (the frame's own share named honestly). Σ/calculus notation here only (Rule 38c) | **Adding up every particle** | none | 40–55 | advanced |
| S8 | Sandbox | `drag-sandbox` | 0.80 · +1.50 · 0 · 0 (home pose per contract) | Free-running (Rule 37). **No idle sweep** — the turntable's own steady spin is the until-first-input motion (an idle τ-pulse cycle would grow ω without bound, and an idle r-sweep would perform CoAM's conservation beat and pre-spoil a LATER concept — both rejected, stated here so json_author does not add one). Live semantics: `tau_app` drag (D5) applies a signed drive along the current spin sense, arrow tracks; `tau_brake` drag opposes (extended ring); **both at once = a live τ_net tug — the α readout shows the difference**, the teacher's own net-torque demonstration; `r`/`m` drags re-shape I live; `ω₀` change = restart with re-pin cue ([LIVE] `:50074–50078`). α/τ readouts blank across every re-pin (D3) | **Try it yourself** | ALL, ring-gated: `tau_app` *(core — the taught variable, D5)* · `r` *(core)* · `m` *(core)* · `omega0` *(core)* · `tau_brake` *(extended — cut with S6's ring)* | 0 / open | *(explore)* |
