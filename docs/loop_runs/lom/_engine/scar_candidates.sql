...
-- ═══════════════════════════════════════════════════════════════════════════
-- PORT SEAM 3 (lom-a 2b6f503 -> feat/lom-third-law-port): spring_action phase
-- machine + release-window dt scaling + slow-motion badge — 2026-07-30.
-- TEXT ONLY, nothing applied. Ported in substance from the lom-a rows.
-- ═══════════════════════════════════════════════════════════════════════════

-- The dispatched bug_class. FIXED here for the timing/phase half; the coil-
-- geometry half (compression stroke + post-release ring) is port seam 4.
-- INSERT INTO engine_bug_queue (
--     bug_class, title, severity, owner_cluster, root_cause, prevention_rule,
--     probe_type, probe_logic, status, concepts_affected, fixed_in_files,
--     discovered_in_session, row_type
...
--     ARRAY[]::text[],
--     ARRAY['src/lib/renderers/field_3d_renderer.ts', 'src/lib/validators/visual/deriveStateMeta.ts']::text[],
--     'port seam 3 (lom-a spring choreography seam A) 2026-07-30',
--     'incident'
-- );

-- Found while wiring the slow window. Port seam 4 owns the fix (coil geometry).
-- INSERT INTO engine_bug_queue (
--     bug_class, title, severity, owner_cluster, root_cause, prevention_rule,
--     probe_type, probe_logic, status, concepts_affected, fixed_in_files,
--     discovered_in_session, row_type
...
-- ═══════════════════════════════════════════════════════════════════════════
-- SPRING CHOREOGRAPHY — ENGINE SEAM B (coil geometry) · PORT SEAM 4 · 2026-07-30
--   field3d_surgeon dispatch, ONE bug_class:
--   nlb_spring_coil_geometry_does_not_follow_the_choreography_phases
--   Ported from feat/lom-a 9421ba2 onto master's renderer. TEXT ONLY.
--   NOTHING BELOW IS APPLIED TO THE DATABASE.
-- ═══════════════════════════════════════════════════════════════════════════

-- (1) The seam-A row this dispatch CLOSES. bug_class is the upsert key, so this
--     is an UPDATE of the existing OPEN row above, never a second INSERT.
-- UPDATE engine_bug_queue SET
--     status = 'FIXED',
--     fixed_in_files = ARRAY['src/lib/renderers/field_3d_renderer.ts']::text[],
--     root_cause = root_cause || ' FIXED 2026-07-30 (seam B): with a live spring_action the drawn target is capped at NATURAL length in every phase, so the GAP does the compressing - the coil now shrinks natural->compressed across the scripted compression stroke and springs back out with the carts through the slowed release (measured on master: mesh 0.360 -> 0.800 world over 156 release frames, drawn == gap to 1e-6 on every compress and release frame). The compressed-now boolean survives only on the no-choreography path.'
...
--     ARRAY[]::text[],
--     ARRAY['src/lib/renderers/field_3d_renderer.ts']::text[],
--     'port seam 4 (lom-a spring choreography seam B) 2026-07-30',
--     'incident'
-- );
...
-- ============================================================
-- PORT SEAM 5 (lom-a 76dad78 -> feat/lom-third-law-port, 2026-07-31)
-- WALL-ANCHORED SPRING. Founder finding on newton_third_law STATE_3: "the spring
-- is moving with the block. It should be locked to the wall." Both incidents
-- below were RE-MEASURED on THIS branch's pre-change renderer (the seam-4 build,
-- stashed) with src/scripts/_scratch_nlb_wall_spring_probe.ts and reproduced the
-- lom-a numbers exactly. TEXT ONLY, NOT APPLIED.
-- ============================================================
...
-- (3) Probe doctrine carried over from lom-a - the H2 "0.00%" demand is not well
--     formed. MODERATE, OPEN. NOT re-observed on this branch (this seam ran no
--     EYE pass: the port is statically unreachable for all 145 concepts), carried
--     forward verbatim so the doctrine is not lost at merge.
-- INSERT INTO engine_bug_queue (
--     bug_class, title, severity, owner_cluster, root_cause, prevention_rule,
--     probe_type, probe_logic, status, concepts_affected, fixed_in_files,
--     discovered_in_session, row_type
