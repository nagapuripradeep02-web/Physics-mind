...
| 2 | `every_bond_is_staggered` | **Every C–C bond is staggered** | sight straight down a C–C bond of the chair: the bonds on the two carbons are staggered, all six C–C bonds read the same, so nothing in the chair is eclipsed | **sight-along bond walk** | "Sight down a bond" | none | 44–52 | core | `auto_after_tts` |
...
| 4 | `the_flip_swaps_them` | **Axial becomes equatorial** | **PRIMARY AHA** — follow one axial bond through the flip: it ends up equatorial, and so does every other bond that started axial, and no bond ever broke | **follow-one-bond through the pucker** | "Follow one axial bond" | `pucker` (drag-seize, after the scripted pass) | 45–55 | core | `manual_click` |
...
| **E-4** | `hud_lines` | `+= a_value` | #4 S8 renders `A = 7.3 kJ·mol⁻¹` on the HUD once §I-4 moves it off the formula surface | founder-proxy |
...
| the population relation | formula surface `N(equatorial) : N(axial) = exp(A/RT) : 1` (symbolic, therefore un-falsifiable by the live slider — chemistry block §I-4) | `show_formula` / `formula` | S8 |
| the A-value and the temperature | HUD `A = 7.3 kJ·mol⁻¹ (literature)` and `T = 298 K` — value-only, Rule 34b | HUD members **`a_value`** and **`temperature`** | S8 |
...
7. **Your §I-3, §I-4, §I-5, §I-6, §I-7 and §I-8 are ADOPTED in full** and are reflected in §7, §10 and §14 above: `barrier = 45 kJ·mol⁻¹` replaces `ΔG‡`; S8's surface becomes the symbolic ratio in `A` (founder-proxy P1-6: `A ≡ −ΔG°`, so `exp(ΔG°/RT)` with a positive ΔG° inverted the convention twice) with `A` and `T` moved to the HUD; S7 gains a `C₇H₁₄` surface; S1 carries no surface; the S3 arc is the **C–C–H** angle at 109.46°, not the ax–C–eq angle (which is 107.50°); and "conformation" is dual-labelled at S5.
